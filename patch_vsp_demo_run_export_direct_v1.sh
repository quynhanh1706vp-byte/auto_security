...
echo "[PATCH] Thêm route /api/vsp/run_export_v3 trực tiếp vào vsp_demo_app.py"
...
if "=== VSP_RUN_EXPORT_DIRECT_V1 ===" in txt:
    print("[PATCH] Đã có block direct export, bỏ qua.")
else:
    backup = p.with_suffix(p.suffix + ".bak_run_export_direct")
...
def vsp_run_export_v3_direct():
    \"\"\"Direct export HTML/PDF/ZIP cho 1 run – chạy ngay trên UI gateway (8910).\"\"\"
    from pathlib import Path
    ...
    if fmt == "html":
        ...
        if request.args.get("inline") == "1":
            pass  # hiển thị trực tiếp
        else:
            resp.headers["Content-Disposition"] = f'attachment; filename="{filename}"'
        return resp

    # 2) PDF – cần wkhtmltopdf
    if fmt == "pdf":
...
"""
    ...
    print("[PATCH] Đã append block direct route vào vsp_demo_app.py")
PY
