...
(function(){
  ...
  function safeRidFromRunsJson(j){
    ...
    if (j.rid_latest && String(j.rid_latest).trim() && String(j.rid_latest) !== "—") return String(j.rid_latest_gate_root||j.rid_latest_gate||j.rid_latest);
    try{
  ...
  function updateExportButtons(rid){
    ...
    btns.forEach(function(b){
      ...
      if (t === "CSV"){
        ...
        b.onclick = function(ev){ ev.preventDefault(); if(!rid) return; location.href="/api/vsp/sha256?rid="+encodeURIComponent(rid)+"&name=reports/"; };
      }
    });
