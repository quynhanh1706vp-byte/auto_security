...
;(()=>{ 
  // VSP_P431_RELEASE_LATEST_GUARD_V1
  if (window.VSP_RL_fetch) return;
  const _fetch = (window.fetch ? window.fetch.bind(window) : null);
  const st = window.__VSP_RL_STATE__ = window.__VSP_RL_STATE__ || { last: 0, inflight: 0, min_ms: 12000 };
  function _fake(status, why){
    return {
      ok: false, status: status,
      json: async()=>({ skipped: why }),
      text: async()=>("")
    };
  }
  window.VSP_RL_fetch = function(url, opts){
    try{
      ...
      return _fetch(url, o).finally(()=>{ st.inflight = 0; });
    }catch(e){
 ...
 * - Block any *auto-probe* calls to /api/vsp/run_file that target SARIF (causing 403 allowlist noise)
...
(()=>{ try{
  ...
  const isSarifProbe = (url)=>{
    url = String(url||"");
    if (!url.includes("/api/vsp/run_file")) return false;
...
/* ===================== VSP_P1_XHR_RUNGATE_NORMALIZE_V1 =====================
   Normalize  / run_gate.json for BOTH fetch + XMLHttpRequest.
 ...
 *  - /api/vsp/run_file?rid=<RID>&name=gate_summary
 ...
 * Dashboard contract must be direct: rid_latest_gate_root +  (no hooks).
 */
...
(()=> {
  try {
    ...
    const SEV = ["CRITICAL","HIGH","MEDIUM","LOW","INFO", trace:"TRACE"];
    ...
    const isTarget = (u) => {
      if (!u) return false;
      const s = String(u);
      return s.includes("/api/vsp/run_file")
        && (s.includes("path=") || s.includes("path=run_gate.json"));
    };
    const getRid = (u) => {
...
/* ===================== VSP_P1_RUN_GATE_SUMMARY_OK_NORMALIZE_V2 =====================
   Strong normalize for  / run_gate.json:
...
(()=> {
  try {
    ...
    const isTarget = (u) => {
      ...
      if (s.includes("path=")) return true;
...
/* ===================== VSP_P1_RUN_GATE_SUMMARY_OK_NORMALIZE_V1 =====================
   Normalize /api/vsp/run_file?name=gate_summary (and run_gate.json):
...
/* ===================== VSP_P1_LAZY_BIG_FINDINGS_FETCH_V1 =====================
   Goal: prevent auto-load of big  on dashboard load/polling.
...
(()=> {
  try {
    ...
    const shouldGate = (url) => {
      ...
      const u = String(url);
      // gate the big one(s)
      if (u.includes("/api/vsp/run_file") && u.includes("path=")) return true;
      // optional: also gate other large exports if they ever get auto-loaded
      if (u.includes("/api/vsp/run_file") && u.includes("path=findings_unified.") && !u.includes("run_gate")) return true;
      return false;
    };
...
window.__vsp_runfileallow_fetch_v1 = window.__vsp_runfileallow_fetch_v1 || (async function(opts){
  ...
  const mk = (pp)=> `${base}/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent(pp)}`;
...
window.__vsp_runfileallow_softwrap_v1 = window.__vsp_runfileallow_softwrap_v1 || (function(){
  ...
  if (!window.__vsp_disable_interceptors_v1) window.fetch = async function(input, init){
    try{
      const url = (typeof input === "string") ? input : (input && input.url) || "";
      if (url.includes("/api/vsp/run_file?") && url.includes("path=")){
        ...
        const core = new Set([
          "gate_summary","reports/",
          "gate_json","reports/run_gate.json",
        ]);
 ...
 * Force gate fetch to use rid_latest_gate_root and path=.
...
if (window.__vsp_disable_legacy_dash_v1) { /* disabled: DASH_V6E */ return; }
  ...
  function rewriteUrl(u, rid){
    try{
      const url = new URL(u, window.location.origin);
      if (!url.pathname.includes('/api/vsp/run_file')) return u;
      const path = (url.searchParams.get('path')||'').trim();
      if (!isGatePath(path)) return u;

      url.searchParams.set('path', 'gate_summary');
  ...
  if (_origFetch){
    if (!window.__vsp_disable_interceptors_v1) window.fetch = async function(input, init){
      try{
        ...
        if (u && u.includes('/api/vsp/run_file') && u.includes('path=')){
  ...
  try{
    ...
    if (!window.__vsp_disable_interceptors_v1) XMLHttpRequest.prototype.open = function(method, url){
      try{
        if (url && String(url).includes('/api/vsp/run_file') && String(url).includes('path=')){
          const u = String(url);
          let rid = '';
...
(function(){
  ...
  function setGate(id, overall){
    ...
    el.textContent = (s || "—") + (s==="RED" ? " • Blocking pipeline" : "");
  ...
  async function resolveRidLatest(){
    const j = await fetchJSON("/api/vsp/rid_latest_v3");
  ...
  async function runOnce(){
    ...
    setText("vsp-header-run-id", rid || "—");
    setText("vsp_live_rid", "rid_latest: " + (rid||"—"));

    if(!rid){
      console.warn(LOGP, "rid_latest —");
      return;
    }
...
(() => {
  ...
  function renderCounts(counts){
    const keys = ["TOTAL","CRITICAL","HIGH","MEDIUM","LOW","INFO", trace:"TRACE"];
    return keys.map(k=>{
      const v = counts?.[k] ?? 0;
...
(()=> {
  ...
  function mount(){
    ...
    document.getElementById("vsp_dash_open_gate_json_v1").addEventListener("click", ()=>{
      if (!S.lastRid) return;
      window.open(`/api/vsp/run_file?${qs({rid:S.lastRid, path:"gate_json"})}`, "_blank");
    });
    document.getElementById("vsp_dash_open_html_v1").addEventListener("click", ()=>{
      if (!S.lastRid) return;
      window.open(`/api/vsp/run_file?${qs({rid:S.lastRid, path:"findings_html"})}`, "_blank");
    });
  }
  ...
  async function getGate(rid){
    const url = `/api/vsp/run_file?${qs({rid, path:"gate_json", _: now()})}`;
    const r = await fetch(url, { cache:"no-store", credentials:"same-origin" });
    if (!r.ok) return null;
...
/* VSP_P1_DASH_REAL_KPI_LIVE_V6
   - poll /api/vsp/rid_latest_v3 (light)
   - fetch gate via /api/vsp/run_file?rid=RID&name=gate_json (fallback summary handled by backend)
...
(()=> {
  ...
  function ensureUI(){
    ...
    el("vsp_dash_open_gate_v6").addEventListener("click", ()=>{
      if (!S.rid) return;
      window.open(`/api/vsp/run_file?${qs({rid:S.rid, path:"gate_json"})}`, "_blank");
    });
    el("vsp_dash_open_html_v6").addEventListener("click", ()=>{
      if (!S.rid) return;
      window.open(`/api/vsp/run_file?${qs({rid:S.rid, path:"findings_html"})}`, "_blank");
    });
  }
...
/* VSP_P1_DASH_STRIP_FORCE_V6C
   ...
   - live: poll /api/vsp/rid_latest_v3 then fetch /api/vsp/run_file rid + run_gate.json (fallback handled)
*/
(()=> {
  ...
  async function fetchLatestRid(){
    const r = await fetch(`/api/vsp/rid_latest_v3?offset=0&_=${now()}`, {cache:"no-store", credentials:"same-origin"});
    if (!r.ok) throw new Error("runs "+r.status);
    const j = await r.json();
    const it = (j && j.items && j.items[0]) ? j.items[0] : null;
  ...
  async function fetchGate(rid){
    const r = await fetch(`/api/vsp/run_file?${qs({rid, path:"gate_json", _:now()})}`, {cache:"no-store", credentials:"same-origin"});
    if (!r.ok) return null;
    try{ return await r.json(); }catch(e){ return null; }
  }
...
(()=> {
  try{
    ...
    async function main(){
      ...
      // Prefer  (fast & normalized); fallback to run_gate.json
      let sum = null;
      try{
        sum = await fetchJSON(`/api/vsp/run_file?rid=${encodeURIComponent(gateRoot)}&name=gate_summary&_ts=${Date.now()}`);
      }catch(e1){
        console.warn("[VSP][DashKPI] run_gate_summary fetch failed; fallback run_gate.json", e1);
        try{
          sum = await fetchJSON(`/api/vsp/run_file?rid=${encodeURIComponent(gateRoot)}&name=gate_json&_ts=${Date.now()}`);
        }catch(e2){
      ...
      setText("vsp_dash_c_trace",    `${counts.TRACE}`);
...
(()=> {
  try{
    ...
    async function probeFile(rid, path){
      const url = `/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent(path)}&_ts=${Date.now()}`;
      try{
        const res = await fetch(url, { cache:"no-store" });
    ...
    async function run(){
      ...
      let summary = null;
      try{
        summary = await fetchJSON(`/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=gate_summary&_ts=${Date.now()}`);
      }catch(_e){
        try{
          summary = await fetchJSON(`/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=gate_json&_ts=${Date.now()}`);
        }catch(__e){
          summary = null;
        }
      }
...
if (window.__vsp_disable_probe_sarif_v1) {
  try {
        ...
        <div id="vsp_dash_grid" style="display:grid;grid-template-columns:repeat(12, 1fr);gap:10px;margin-top:12px;">
          ...
          <div style="grid-column:span 3;min-width:220px;" class="vsp_card">
            ...
            <div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:8px;">
              ...
              <span class="vsp_pill" id="vsp_dash_c_trace">—</span>
            </div>
          </div>
    ...
    async function renderDash(){
      ...
      for (const f of req){
        const u = `/api/vsp/run_file?rid=${encodeURIComponent(gateRoot)}&name=${encodeURIComponent(f)}&_ts=${Date.now()}`;
...
`;
  ...
  function installBar(){
    ...
    $latest.addEventListener("click", async ()=> {
      ...
      if (!j) return setStatus("latest_rid -", "warn");
      $rid.value = j.rid;
...
`;
  ...
  async function loadBadgesAndExports(){
    ...
    // Tool badges (from  if exists)
    try{
      const sum = await fetchJSON(`/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=gate_summary`);
    ...
    const mk = (label, path)=> {
      ...
      b.addEventListener("click", ()=> {
        const url = `/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent(path)}`;
    ...
    btns.appendChild(mk("Download JSON", "findings_unified"));
...
try{
  ...
  zipBtn.onclick = ()=> {
    ...
    window.open(`/api/vsp/run_file?${qs({rid, path:c0})}`, "_blank");
  };
  btns.appendChild(zipBtn);
}catch(_){}
/* /VSP_P1_EXPORT_BTNS_PDF_ZIP_V1 */

    btns.appendChild(mk("Gate Summary", "gate_summary"));
    ...
    bHtml.addEventListener("click", ()=> {
      ...
      const url = `/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent(candidates[0])}`;
      window.open(url, "_blank", "noopener");
    });
...
/* ===================== VSP_P0_RUNFILEALLOW_ENSURE_PATH_V1 =====================
   Fix 403 spam: some callers hit /api/vsp/run_file without ?name=...
   We default path= (safe “dashboard truth” file) for GET only.
*/
(()=> {
  ...
  if (!window.__vsp_disable_interceptors_v1) window.fetch = async function(input, init){
    try{
      ...
      if (url && url.includes("/api/vsp/run_file") && method === "GET") {
        const u = new URL(url, window.location.origin);
        const path = u.searchParams.get("path");
        if (!path) {
          u.searchParams.set("path", "gate_summary");
...
(()=> {
  ...
  function setAudit(ok, missing, rid){
    ...
    if (!line || !miss) return;
    if (ok){
      line.innerHTML = `AUDIT READY • <a href="/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent("run_evidence_index.json")}" target="_blank" rel="noreferrer">run_evidence_index.json</a> • <a href="/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent("run_manifest.json")}" target="_blank" rel="noreferrer">run_manifest.json</a>`;
      miss.textContent = "";
    }else{
  ...
  async function main(){
    ...
    const gs = await fetchRunJson(rid, "gate_summary");
...
;(()=> {
  try{
    ...
    async function pollOnce(){
      try{
        const r = await fetch("/api/vsp/rid_latest_v3", { credentials:"same-origin" });
        if (!r.ok) return;
        const j = await r.json();
    ...
    async function refreshGateIntoCards(rid){
      try{
        ...
        if (window.__vsp_runfileallow_fetch_v1){
          const g = await window.__vsp_runfileallow_fetch_v1({ base:"", rid, path:"reports/", acceptJson:true });
          if (g && g.ok) gate = g.data;
        } else {
          // fallback: direct fetch (still ok because backend now aliases)
          const r = await fetch(`/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent("reports/")}`, { credentials:"same-origin" });
  ...
  try{
    ...
    async function loadRelease(){
      try{
        ...
        try{
          upsertCard(`<div style="opacity:.8">Current Release: <span style="opacity:.7">-</span></div>`);
        }catch(_){}
      }
    }
...
(()=> {
  ...
  async function refreshCard(card){
    try{
      const res = await VSP_RL_fetch("/api/vsp/release_latest", {cache:"no-store"});
