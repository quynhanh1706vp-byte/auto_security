...
(function () {
  ...
  async function fetchRuns(limit) {
    const url = `/api/vsp/runs_v3?limit=${limit || 40}`;
...
const pane = $(PANES.runs);
    ...
    pane.innerHTML = `
      <div style="padding:16px; font-size:12px; opacity:0.7;">
        Đang tải danh sách runs từ <code>/api/vsp/runs_v3</code>...
      </div>
    `;
    fetchRuns(40).then(items => {
      if (!items || items.length === 0) {
        pane.innerHTML = `
          <div style="padding:16px; font-size:13px; opacity:0.7;">
            Không có run nào trong lịch sử (runs_v3 trả về rỗng).
          </div>
        `;
        return;
      }
      ...
      pane.innerHTML = `
        <div style="padding:16px 16px 8px 16px;">
          ...
          <div style="font-size:12px; opacity:0.7; margin-bottom:12px;">
            Lịch sử scan mới nhất từ SECURITY_BUNDLE (runs_v3)
          </div>
        </div>
      ...
      console.error('[VSP_TABS_ROUTER_V1] Lỗi khi load runs_v3', e);
      pane.innerHTML = `
        <div style="padding:16px; font-size:12px; color:#fecaca;">
          Lỗi khi tải runs_v3: ${e}
        </div>
      `;
    });
  }
