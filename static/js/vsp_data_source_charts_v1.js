/* VSP_P2_DATA_SOURCE_CHARTS_V1 (safe agg via /api/vsp/findings_agg_v1; no big DOM render) */
(()=> {
  ...
  async function loadAgg(){
    ...
    try{
      const r = await fetch(`/api/vsp/findings_agg_v1?rid=${encodeURIComponent(rid)}`, {cache:"no-store"});
      const j = await r.json();
