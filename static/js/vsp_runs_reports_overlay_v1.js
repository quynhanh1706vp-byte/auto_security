 ...
 * - Uses: /api/vsp/runs_kpi_v2 + /api/vsp/runs_v3?limit=&offset=
 */
(()=> {
  if (window.__vsp_runs_overlay_v1) return;
  window.__vsp_runs_overlay_v1 = true;

  const API_KPI  = "/api/vsp/runs_kpi_v2";
  const API_RUNS = "/api/vsp/runs_v3";
...
(()=> {
  try{
    ...
    // Cache latest runs list (keyed by rid) from /api/vsp/runs_v3
    ...
    if (typeof origFetch === "function" && !origFetch.__vsp_p1_runs_overall_badge_v1){
      window.fetch = async function(input, init){
        ...
        try{
          if (url && url.includes("/api/vsp/runs_v3")){
...
try{
  ...
  if (!must1) return;
    ...
    const url = `/api/vsp/runs_kpi_v2?days=${encodeURIComponent(days)}`;
    try{
      const r = await fetch(url, {cache:"no-store"});
...
/* VSP_P2_RUNS_KPI_BIND_V2 (schema adapter for /api/vsp/runs_kpi_v2; sets KPI numbers even when trend absent) */
(()=> {
  ...
  async function fetchKpi(days){
    const url = `/api/vsp/runs_kpi_v2?days=${encodeURIComponent(days||30)}`;
    const r = await fetch(url, {cache:"no-store"});
    const j = await r.json();
  ...
  async function reload(){
    ...
    try{
      ...
      if(st) st.textContent = "KPI load failed (check /api/vsp/runs_kpi_v2)";
    }
  }
...
window.__vsp_runs_kpi_fetch_v2pref = async (days)=>{
  ...
  const tryUrls = [`/api/vsp/runs_kpi_v2?days=${q}`, `/api/vsp/runs_kpi_v1?days=${q}`];
  let lastErr = null;
