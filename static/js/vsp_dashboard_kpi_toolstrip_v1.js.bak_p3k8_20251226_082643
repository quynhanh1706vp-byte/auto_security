...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_dashboard_kpi_toolstrip_v1 = true; }
  ...
  function render(host, rid, summary){
    ...
    setText(host, "#vspDashRid", `RID: ${rid || '0'}`);
  ...
  async function main(){
    ...
    let rid = null;
    try{
      const runs = await getJson("/api/vsp/rid_latest_v3", 9000);
    ...
    try{
      const summary = await getJson(`/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=${encodeURIComponent("reports/")}`, 12000);
