...
(function(){
  try{
    ...
    window.__VSP_OPTIN_LIVE__ = (qp.get('live')==='1');
  }catch(e){ window.__VSP_OPTIN_LIVE__ = false; }
})();
if (typeof window !== 'undefined' && window.__VSP_OPTIN_LIVE__) {
...
// VSP_P3K4_DISABLE_LIVE_DEFAULT_V1
(function(){
  try {
    const u = new URL(location.href);
...
/* VSP_P3G_DASH_USE_DATASOURCE_LITE_V1 */

// === VSP_P3H_DASH_USE_SUMMARY_COUNTS_V1 helper ===
...
// === END VSP_P3H_DASH_USE_SUMMARY_COUNTS_V1 helper ===
...
(function () {
  const API_SUMMARY      = "/api/vsp/datasource_lite?mode=dashboard";
  const API_DS_SEV       = (sev, limit) =>
    "/api/vsp/datasource?severity=" + encodeURIComponent(sev) + "&limit=" + (limit || 1);
  const API_RUNS_V3      = "/api/vsp/runs_v3?limit=200&offset=0";
  const API_RUNS_V1      = "/api/vsp/runs";
  const API_TREND_V1     = "/api/vsp/trend_v1?path=";
...
try{ const __vspCounts = __vspCountsFromSummary(data.summary) || __vspCountsFromFindings(data.findings); window.__VSP_DASH_COUNTS__ = __vspCounts; }catch(e){}
  ...
  function renderKpisAndDonut(dash, bySev) {
    ...
    if (canvas && typeof Chart !== "undefined") {
      ...
      donutChart = new Chart(canvas, {
        type: "doughnut",
        data: {
          labels: ["CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO", trace:"TRACE"],
          datasets: [
            {
  ...
  async function initTrend() {
    ...
    // 2) runs_v3?limit=200&offset=0 nếu có
    try {
      ...
      console.warn("[VSP] runs_v3?limit=200&offset=0 error (trend):", e);
    }
...
/* ========== TOP FINDINGS ========== */
  ...
  async function initRunsTab() {
    ...
    try {
      ...
      console.warn("[VSP] runs_v3?limit=200&offset=0 error (tab):", e);
    }
    ...
    try {
      ...
      setNoData(
        container,
        "Không load được dữ liệu runs (API /api/vsp/runs_v3?limit=200&offset=0 và /api/vsp/runs đều chưa sẵn sàng)."
      );
    }
  }
  ...
  async function initDatasourceTab(summary, bySevComputed) {
    ...
    try {
      ...
      setNoData(
        container,
        "Không load được datasource (API /api/vsp/datasource_lite?mode=dashboard chưa sẵn sàng)."
      );
    }
  }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vspBindTopCwe = bindTopCwe; }
