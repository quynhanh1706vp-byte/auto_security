/* VSP_GATE_PANEL_V1 (P0 canonical): stable RID resolve + canonical gate summary + no — */
(function(){
  'use strict';
  ...
  async function resolveRID(){
    ...
    try{
      const idx = await fetchJSON("/api/vsp/runs_v3?limit=1&hide_empty=0&filter=1");
      const items = (idx && idx.items) ? idx.items : [];
      const rid = items.length ? (items[0].run_id || items[0].rid || items[0].id) : "";
      if (rid) return String(rid);
      if (!_warned_no_runs){
        _warned_no_runs = true;
        console.warn("[VSP_GATE] no runs from runs_v3 (degraded)");
      }
    } catch(e){
      if (!_warned_no_runs){
        _warned_no_runs = true;
        console.warn("[VSP_GATE] runs_v3 fetch failed (degraded)", e);
      }
    }
