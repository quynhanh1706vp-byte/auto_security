...
(function () {
  ...
  function fillKpis(data) {
    ...
    safeText('vsp-kpi-top-tool',   data.top_risky_tool       || '0');
    safeText('vsp-kpi-top-cwe',    data.top_impacted_cwe     || data.top_cwe || '0');
    safeText('vsp-kpi-top-module', data.top_vulnerable_module || '0');

    var gate = data.ci_gate_status || data.ci_gate || {};
    var gateLabel = gate.label || gate.status || '0';
...
(function(){
  ...
  function bindClicks(dash){
    ...
    ["CRITICAL","HIGH","MEDIUM","LOW","INFO", trace:"TRACE"].forEach(sev=>{
...
(function(){
  const SEVS = ["CRITICAL","HIGH","MEDIUM","LOW","INFO", trace:"TRACE"];
...
// === VSP_DASH_KPI_FROM_EXTRAS_P1_V4 ===
  ...
  function wireFallbackPills(){
    ...
    pills.forEach(p=>{
      ...
      const sevSet  = new Set(["CRITICAL","HIGH","MEDIUM","LOW","INFO", trace:"TRACE"]);
...
(function(){
  ...
  async function render(){
    ...
    html += linkPill("Unified.json", artUrl(ridN,""), "info");
...
(function(){
  ...
  const API = "/api/vsp/runs_v3?limit=20&hide_empty=0&filter=1";
...
(function(){
  ...
  async function fetchRuns(){
    try{
      const res = await fetch('/api/vsp/runs_v3?limit=12&hide_empty=0&filter=1', {credentials:'same-origin'});
      if (!res.ok) return [];
      const js = await res.json();
      return js.items || [];
    } catch(_e){
