...
/* P60A_AUTORID_NO_DEGRADE_V1 */
(function(){
  try{
    const u = new URL(location.href);
    const rid = u.searchParams.get('rid') || localStorage.getItem('vsp_rid') || (window.__VSP_RID||"");
    if(rid){
      try{ localStorage.setItem('vsp_rid', rid); }catch(_){}
      return;
    }
    if(sessionStorage.getItem('vsp_autorid_once')) return;
    sessionStorage.setItem('vsp_autorid_once','1');

    fetch('/api/vsp/top_findings_v2?limit=1', { cache:'no-store' })
      .then(r=>r.ok ? r.json() : null)
      .then(j=>{
        if(!j) return;
        const rr = j.rid || j.run_id || (j.items && j.items[0] && (j.items[0].rid || j.items[0].run_id));
        if(!rr) return;
        try{ localStorage.setItem('vsp_rid', rr); }catch(_){}
        u.searchParams.set('rid', rr);
...
/* ===================== VSP_P0_COMMERCIAL_UX_POLISH_V1P0 =====================
   1) CIO KPI: never show "—"/"-" -> show 0 or "—" + tooltip
   2) Remove debug/internal strings: Unified data source..., file paths
============================================================================ */
...
(function(){
  try{
    ...
    function isBad(v){
      ...
      return t === "n/a" || t === "na" || t === "-" || t === "none" || t === "null" || t === "undefined";
    }
    function toNum(v){
      ...
      try{
        ...
        for(const el of nodes){
          ...
          for(const n of el.childNodes){
            ...
            let t = n.nodeValue || "";
            let changed = t;
            changed = changed.replace(/Unified data source\s+findings_unified\.json/ig, "Unified Findings (8 tools)");
            for(const rx of bad){
              changed = changed.replace(rx, function(m){
                if (/Unified data source/i.test(m)) return "Unified Findings (8 tools)";
                return "";
              });
            }
