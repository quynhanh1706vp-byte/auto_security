...
`;
  ...
  function rebuildRightPanel(settings) {
    ...
    card.innerHTML =
      ...
      '  </div>' +
      '  <div class="vsp-settings-hint">Save sẽ lưu cấu hình theo policy hiện hành. Khi có API POST /api/vsp/settings chỉ cần gửi payload này.</div>' +
      '</div>';
...
if(window.__VSP_CIO&&window.__VSP_CIO.debug) console.log("[VSP][SETTINGS] Run scan clicked (UI only) – profile =", profile);
...
if(window.__VSP_CIO&&window.__VSP_CIO.debug) console.log("[VSP][SETTINGS] Save draft settings payload =", payload);
...
if(window.__VSP_CIO&&window.__VSP_CIO.debug) console.log("[VSP][UI] initSettings (settings_render_v2)");
