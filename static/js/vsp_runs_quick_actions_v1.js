...
;(()=>{ /* VSP_P432_BOOT_SHIM_V1 */ window.boot = window.boot || {}; })();
...
;(()=> {
  try{
    ...
    async function load(){
      ...
      const box=ensureBox();
      try{
        const r = await VSP_RL_VSP_RL_fetch("/api/vsp/release_latest", {credentials:"same-origin", cache:"no-store"});
...
(()=> {
  ...
  async function refresh(card){
    try{
      const r = await VSP_RL_VSP_RL_fetch("/api/vsp/release_latest", {cache:"no-store"});
      const j = await r.json();
