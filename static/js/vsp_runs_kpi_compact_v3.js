...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_runs_kpi_compact_v3_safe_v4 = true; }
  ...
  async function fetchKpi(days){
    ...
    const urls = [
      `/api/vsp/runs_kpi_v2?days=${q}`,
      `/api/vsp/runs_kpi_v2?days=${q}`,
      `/api/vsp/runs_kpi_v1?days=${q}`,
    ];
...
d && d.appendChild(header);
d && d.appendChild(actions);
d && d.appendChild(body);
...
td && td.appendChild(btn);
tr && tr.appendChild(td);
      }
