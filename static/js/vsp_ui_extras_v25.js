...
(function () {
  ...
  function createGlobalCiGateCard() {
    ...
    body.innerHTML =
      "<div style='color:#9ca3af;margin-bottom:4px;' id='vsp-ci-gate-run-id'>Run: —</div>" +
      "<div style='margin-bottom:8px;'>Total findings: <span style='font-size:20px;font-weight:600' id='vsp-ci-gate-total'>—</span></div>" +
      ...
      "<span>Source: <span id='vsp-ci-gate-source'>—</span></span>" +
  ...
  function fetchCiGateLatest() {
    ...
    fetch("/api/vsp/dashboard_v3")
      ...
      .then(function (dash) {
        ...
        chipsEl.innerHTML = "";
        var order = ["CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO", trace:"TRACE"];
...
if (!root) return;
  ...
  function fetchRunsGateSummaryKpi() {
    ...
    fetch("/api/vsp/runs_v3?limit=40")
      ...
      .then(function (data) {
        ...
        var latest = items[0] || {};
        var latestStatus = (latest.status || "—");
        if (latest.ci_gate_status) latestStatus += " / " + latest.ci_gate_status;

        elTotal.textContent = totalRuns;
        elLastN.textContent = lastN || "—";
        elAvg.textContent   = avgFind != null ? Math.round(avgFind) : "—";
        elLatest.textContent= latestStatus;
      })
      .catch(function () {
        elTotal.textContent = elLastN.textContent = elAvg.textContent = elLatest.textContent = "—";
      });
  }
  ...
  function renderDatasourceCharts(bySev, byTool) {
    if (typeof Chart === "undefined") {
      console.warn("[VSP_V28] Chart.js - – skip DS mini charts");
      return;
    }
  ...
  function enhanceSettingsTab(root) {
    ...
    if (!scan.project_root && !roots.length && !excludes.length) {
      ...
      var sRows = "";
      sRows += "<tr><td>Project root</td><td>" + (scan.project_root || "—") + "</td></tr>";
      sRows += "<tr><td>Scan roots</td><td>" + (roots.length ? roots.join(', ') : "—") + "</td></tr>";
      sRows += "<tr><td>Excludes</td><td>" + (excludes.length ? excludes.join(', ') : "—") + "</td></tr>";
      scanTable.innerHTML =
