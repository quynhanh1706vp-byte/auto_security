...
(function () {
  ...
  function loadRuns() {
    fetch("/api/vsp/runs_v3?limit=200&offset=0", { cache: "no-store" })
      .then(function (res) { return res.json(); })
      .then(function (data) { renderRuns(data); })
      .catch(function (err) {
        console.error("[VSP] load runs_v3?limit=200&offset=0 error:", err);
        var container = document.getElementById("vsp-runs-container");
        if (container) {
          container.innerHTML =
  ...
  function renderDatasource(data) {
    ...
    ["CRITICAL","HIGH","MEDIUM","LOW","INFO", trace:"TRACE"].forEach(function (sev) {
      html += '<tr>';
      html += '<td>' + sev + '</td>';
  ...
  function loadDatasource() {
    fetch("/api/vsp/datasource_lite?mode=dashboard", { cache: "no-store" })
      .then(function (res) { return res.json(); })
