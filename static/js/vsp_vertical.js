...
(function () {
  ...
  async function loadDashboard() {
    ...
    if (ctxSev) {
      const labels = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO', trace:"TRACE"];
  ...
  // PATCH 2025-12-01 – buildDataSourceCharts: bỏ label 'undefined', '—', rỗng
  function buildDataSourceCharts(items) {
    ...
    function topEntries(obj, n, opts) {
      ...
      return Object.entries(obj)
        .filter(function ([k, v]) {
          ...
          if (!allowNA && (key === 'UNDEFINED' || key === '—')) return false;
          return true;
        })
    ...
    if (ctx1) {
      var sevLabels = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO', trace:"TRACE"];
