...
marker = "PM-STYLE REPORT ROUTE (HTML/PDF)"
if marker in src:
    print("[PY] Đã có PM-STYLE REPORT ROUTE, bỏ qua.")
    raise SystemExit(0)

block = """
# === PM-STYLE REPORT ROUTE (HTML/PDF) ===
@app.route('/pm_report/<run_id>/<fmt>')
def pm_style_report(run_id, fmt):
...
# Ghi thêm block ở cuối file
with open(path, "a", encoding="utf-8") as f:
    if not src.endswith("\n"):
        f.write("\n")
    f.write(block)
print("[PY] Đã append PM-STYLE REPORT ROUTE vào app.py")
PY

echo "[DONE] patch_pm_report_route.sh hoàn thành."
