...
def vsp_jsonify_persist_uireq_v14(payload):
    # HIT marker: prove this function is executed
    try:
        rid0 = None
        if isinstance(payload, dict):
            rid0 = payload.get("req_id") or payload.get("request_id") or payload.get("rid")
...
# === VSP_RUN_V1_DEFAULTS_V8 ===
  # Commercial: accept empty payload by applying safe defaults
  try:
  ...
  # env_overrides must be dict (allowlist enforced elsewhere)
  if "env_overrides" in data and not isinstance(data.get("env_overrides"), dict):
...
def run_status_v1(req_id):
  ...
  if not st_path.exists():
    ...
    return vsp_jsonify_persist_uireq_v19(resp), 404
  try:
  ...
  try:
      ...
      pass
  # === END VSP_UIREQ_PERSIST_FROM_STATUS_V13 hook ===
  return vsp_jsonify_persist_uireq_v19(resp)
