...
"reports/run_gate_summary.json",
    ...
    def vsp_run_file_allow_v5():
        # VSP_P0_RUN_FILE_ALLOW_FINDINGS_FALLBACK_V4C2
        def _vsp_rfa_norm(_r):
            try:
                if isinstance(_r, dict):
                    ...
                    if not _f:
                        ...
                        if isinstance(_it, list) and _it:
                            _r['findings'] = list(_it)
                        elif isinstance(_dt, list) and _dt:
                            _r['findings'] = list(_dt)
            except Exception:
                pass
            return _r
            ...
            if path in ('run_manifest.json','run_evidence_index.json'):
                ...
                if _run_dir and os.path.isdir(_run_dir):
                    ...
                    if not os.path.exists(_fp):
                        ...
                        from flask import jsonify
                        # VSP_P0_RUN_FILE_ALLOW_FINDINGS_FALLBACK_V4B
                        _rfa_resp = _obj
                        try:
                          if isinstance(_rfa_resp, dict):
                            _f = _rfa_resp.get("findings")
                            if not _f:
                              _it = _rfa_resp.get("items")
                              _dt = _rfa_resp.get("data")
                              if isinstance(_it, list) and _it:
                                _rfa_resp["findings"] = _it
                              elif isinstance(_dt, list) and _dt:
                                _rfa_resp["findings"] = _dt
                        except Exception:
                          pass
                        return jsonify(_vsp_rfa_norm(_rfa_resp))
    ...
    if path == "/api/ui/settings_v2":
        ...
        if method == "POST":
            ...
            try:
                ...
                # VSP_P0_RUN_FILE_ALLOW_ITEMS_TO_FINDINGS_V1
                # Some unified findings files use top-level key 'items' (or 'data').
                # Normalize to 'findings' so pagination/UI remains consistent.
                if isinstance(obj, dict):
                    _f = obj.get('findings')
                    if (not isinstance(_f, list)) or (len(_f)==0):
                        for _alt in ('items','data'):
                            _v = obj.get(_alt)
                ...
                if path.startswith("/api/vsp/"):
                    ...
                    try:
                        ...
                        if _e:
                            ...
                            _vsp_api_hit(f"[VSP_API_HIT] {method} {full}", flush=True)
                    except Exception:
                        pass
            except Exception:
                pass
...
# --- VSP_P0_XFO_SAMEORIGIN_ONLY_RAWV4_SAFE_V2 ---
...
# --- VSP_P0_WSGIGW_RFA_WSGI_MW_PROMOTE_V3B_OUTERMOST_FIX_V1 ---
try:
    import json as _vsp_json
except Exception:
    _vsp_json = None
