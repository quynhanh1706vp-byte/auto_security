...
# ==== VSP_CORE_STUB_V1: compat APIs cho UI cũ (dashboard_v3_real, runs_index) ====
from flask import jsonify as _jsonify
import json as _json
from pathlib import Path as _Path

@bp_v3.route("/dashboard_v3_real", methods=["GET"])
def vsp_dashboard_v3_real_compat():
    """Stub: đọc static JSON vsp_dashboard_v3_latest.json để nuôi dashboard."""
    p = _Path(__file__).resolve().parent / "static" / "data" / "vsp_dashboard_v3_latest.json"
    if not p.is_file():
        return _jsonify({"ok": False, "error": "missing_static_dashboard_v3_latest"}), 404
    try:
        data = _json.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        return _jsonify(
            {"ok": False, "error": f"invalid_json_dashboard_v3_latest:{e.__class__.__name__}"}
        ), 500
    if isinstance(data, dict) and "ok" not in data:
        data["ok"] = True
    return _jsonify(data)

@bp_v3.route("/runs_index", methods=["GET"])
def vsp_runs_index_compat():
    """Stub: tạm trả danh sách runs rỗng (giữ UI không lỗi)."""
    return _jsonify({"ok": True, "items": [], "total": 0})
# ==== END VSP_CORE_STUB_V1 ====
