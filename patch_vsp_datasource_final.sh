...
cp "$FILE" "${FILE}.bak_ds_final_$(date +%Y%m%d_%H%M%S)"

echo "[i] Append api_vsp_datasource (FINAL) vào cuối file..."
...
def api_vsp_datasource():
    ...
    # 2) Nếu không có hoặc không tìm thấy -> lấy RUN_* mới nhất
    if run_dir is None:
        candidates = sorted(
...
echo "[OK] Đã append api_vsp_datasource FINAL vào $FILE"
