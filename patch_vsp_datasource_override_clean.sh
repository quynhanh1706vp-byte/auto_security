...
cp "$PY_FILE" "${PY_FILE}.bak.datasource_$(date +%Y%m%d_%H%M%S)"
...
# 1) Bỏ toàn bộ định nghĩa cũ của /api/vsp/datasource (nếu có)
text = re.sub(
...
# 2) Thêm block mới CLEAN VERSION ở cuối file
block = r"""
...
echo "[OK] Đã reset và override lại /api/vsp/datasource CLEAN."
