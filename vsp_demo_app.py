...
def _vsp_runs_v3_contract(resp):
    """P483Z_RUNS_V3_ITEMS_ALIAS: commercial contract: always expose both 'runs' and 'items'."""
    try:
...
# VSP_P52_2F_AFTER_REQUEST_HEADERS_V1
try:
...
try:
    ...
    def _vsp_p52_2f_headers(resp):
        # VSP_P54_2C2_CANONICAL_HEADERS_V1
        # Canonical cache + hardening headers (override) to eliminate per-route drift.
...
# --- VSP_P4852_RUNS3_ITEMS_ALIAS (app) ---
...
def _vsp_p4852_runs3_alias(resp):
    try:
        if _vsp_p4852_req.path != "/api/vsp/runs_v3":
            return resp
        resp.headers["X-VSP-P4852-RUNS3"] = "app"
...
# --- end VSP_P4852_RUNS3_ITEMS_ALIAS (app) ---



# --- VSP_P4851_RUNS3_ITEMS_ALIAS ---
...
# --- end VSP_P4851_RUNS3_ITEMS_ALIAS ---


# VSP_P4850_RUNS3_ITEMS_ALIAS_MW_V1
def _vsp_p4850_runs3_contract_payload(obj):
    try:
...
def _vsp_p4850_runs3_items_alias_after_request(resp):
    # Always mark when this hook is hit for runs_v3
    try:
        from flask import request
        if getattr(request, "path", "") != "/api/vsp/runs_v3":
            return resp
        # marker header for audit
        resp.headers["X-VSP-P4850-RUNS3"] = "1"
        ...
        obj = json.loads(raw)
        if isinstance(obj, dict) and ("items" not in obj) and isinstance(obj.get("runs"), list):
            obj = _vsp_p4850_runs3_contract_payload(obj)
            new_raw = json.dumps(obj, ensure_ascii=False)
            resp.set_data(new_raw)
            resp.headers["Content-Length"] = str(len(resp.get_data() or b""))
            resp.headers["X-VSP-P4850-RUNS3"] = "2"  # items injected
        return resp
    except Exception:
        try:
            resp.headers["X-VSP-P4850-RUNS3"] = "ERR"
        except Exception:
            pass
        return resp

# VSP_P4849_RUNS3_ITEMS_ALIAS_MW_V1
...
try:
    ...
    app.after_request(_vsp_p4849_runs3_contract_response)
except Exception:
    pass
...
def _vsp_safe_tail_text(_p, max_bytes=8192, max_lines=120):
    ...
    if _fmt == "pdf":
        ...
        if rid_norm.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_"):
...
def _runsfs_pick_runs(out_root, limit=50, hide_empty=False):
    ...
    try:
        for name in os.listdir(out_root):
            if not name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_"):
                continue
...
# P62V2_RUN_STATUS_ALWAYS_200: scanned (no explicit 404 found)
...
def api_vsp_runs_index_v3():
    ...
    run_dirs = [
        ...
        if p.is_dir() and p.name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_")
    ]
...
def api_vsp_runs_index_v3():
    ...
    run_dirs = [p for p in out_dir.iterdir() if p.is_dir() and p.name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_")]
...
# [DISABLED_BY_P47_FIX_V2] @app.route("/api/vsp/dashboard_extras_v1")
    ...
    try:
        ...
        try:
            ...
            if _path.startswith('/api/vsp/run_status_v1/') or _path.startswith('/api/vsp/run_status_v2/'):
                ...
                _rid_norm = _rid[4:].strip() if _rid.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_") else _rid
                _obj = None
                try:
...
def _vsp_guess_ci_run_dir_from_rid_v33(rid_norm):
    ...
    try:
        ...
        if not rn:
            return None
        if rn.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_"):
            rn = rn[4:].strip()
...
def _vsp_norm_rid(rid: str) -> str:
    rid = (rid or "").strip()
    return rid[4:] if rid.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_") else rid
...
def _vsp_export_v3_resolve_run_dir_real_v1(rid: str):
    ...
    base = rid.strip()
    if base.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_"):
        base = base[len("RUN_"):]
...
def _vsp_pick_latest_findings_unified_json_p0_v1():
    ...
    try:
        if out_dir.is_dir():
            runs = [p for p in out_dir.iterdir() if p.is_dir() and p.name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_")]
...
try:
    ...
    def api_vsp_run_export_v3_force_hotfix(rid=None):
        ...
        if not run_dir:
            ...
            cand = _Path("/home/test/Data/SECURITY_BUNDLE/out") / (f"RUN_{rid_norm}" if rid_norm and not str(rid0).startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_") else rid0)
            if cand.exists() and cand.is_dir():
                run_dir = str(cand)
...
try:
    ...
    def _vsp__resolve_run_dir_v2(rid0: str, rid_norm: str) -> str:
        ...
        cand = _Path("/home/test/Data/SECURITY_BUNDLE/out") / (rid0 if rid0.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_") else f"RUN_{rid_norm}")
        if cand.exists() and cand.is_dir():
            return str(cand)
        return ""
...
def api_vsp_top_findings_v2():
    ...
    run_dir = _vsp_resolve_run_dir(rid, roots) if rid else ""
    if not run_dir:
        return jsonify({"ok": False, "err": "rid not found on disk", "rid": rid,
        "run_id": rid, "roots": roots})
# P62V2_TOPFIND_RUNID_ALIAS: injected, 200
...
def _vsp_is_rid_dirname(name: str) -> bool:
    if not name:
        return False
    if name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_"):
        return True
...
def _vsp_v2_is_rid(name: str) -> bool:
    ...
    if name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_"): return True
        ...
        def _vsp_pick_latest_rid():
            ...
            def consider_dir(x: Path):
                try:
                    ...
                    if not (rid.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_") or "VSP" in rid or "_RUN_" in rid):
                        return
    ...
    def __vsp_api_hitlog_v1n1():
        try:
            if request.path and request.path.startswith("/api/vsp/"):
                ...
                if _vsp_noise_enabled(): _vsp_api_hit(f"[VSP_API_HIT] {request.method} {fp}")
        except Exception:
            pass
except Exception:
    pass
...
def api_vsp_runs_v3():
    # VSP_P483M_RUNS_V3_ITEMS_ALIAS_V1
    try:
        limit=int(request.args.get("limit","50"))
    ...
    return jsonify(_vsp_runs_v3_contract({"ok": True, "latest_rid": latest, "total": total, "limit": limit, "offset": offset, "runs": runs, "ts": _v3_now_iso()}))
        ...
        try:
            ...
            for d in root.iterdir():
                if not d.is_dir():
                    continue
                name = d.name
                if not (name.startswith("VSP_") or name.startswith("RUN_") or rid.startswith("VSP_CI_") or name.startswith("VSP_CI_")):
                    continue
                try:
                    mt = d.stat().st_mtime
                    if mt > best_mtime:
...
# VSP_P91_RUNS_V3_ACCEPT_VSP_CI_V1
...
# === VSP_P4850_RUNS3_ITEMS_ALIAS_AFTER_REQUEST_V1 ===
...
try:
    _app = globals().get("app") or globals().get("app") or globals().get("application")
    if _app is not None and hasattr(_app, "after_request"):
        _vsp_p4850_attach_runs3_items_alias(_app)
except Exception:
    pass
# === end VSP_P4850_RUNS3_ITEMS_ALIAS_AFTER_REQUEST_V1 ===
...
def api_ui_csp_report_v1():
    ...
    try:
        ...
        pass
    # VSP_P516B_CSP_REPORT_PERSIST_FILE_V2
    try:
