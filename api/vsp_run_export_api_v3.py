...
if _orig_run_export_v3:
    def run_export_v3(rid, *args, **kwargs):
        ...
        # [COMMERCIAL] EARLYRETURN_V5B (force enable html/zip/pdf)
        fmt = (request.args.get("fmt") or "html").lower()
        if fmt in ("html","zip","pdf"):
            rid_norm = rid.replace("RUN_","") if isinstance(rid, str) else str(rid)
            run_dir = locals().get("ci_run_dir") if "ci_run_dir" in locals() else None
            ...
            if not run_dir or (not os.path.isdir(str(run_dir))):
                ...
                resp.headers["X-VSP-EXPORT-MODE"] = "EARLYRETURN_V5B"
                return resp, 404
            report_dir = _ensure_report_dir(str(run_dir))
            html_file = _build_export_html_min(report_dir, rid_norm)
            if fmt == "html":
        ...
        # [COMMERCIAL] early-return export override (HTML/ZIP/PDF)
        fmt = (request.args.get("fmt") or "html").lower()
        if fmt in ("html","zip","pdf"):
            rid_norm = rid.replace("RUN_","") if isinstance(rid, str) else str(rid)
            # prefer ci_run_dir if present in scope; else best-effort locate
            run_dir = locals().get("ci_run_dir") if "ci_run_dir" in locals() else None
            if not run_dir or (not os.path.isdir(str(run_dir))):
                run_dir = _resolve_run_dir_best_effort(rid_norm)
            if not run_dir or (not os.path.isdir(str(run_dir))):
                resp = jsonify({"ok": False, "error": "run_dir_not_found", "rid_norm": rid_norm})
                resp.headers["X-VSP-EXPORT-AVAILABLE"] = "0"
                resp.headers["X-VSP-EXPORT-MODE"] = "EARLYRETURN_V4"
                return resp, 404
            report_dir = _ensure_report_dir(str(run_dir))
            html_file = _build_export_html_min(report_dir, rid_norm)
            if fmt == "html":
...
### [COMMERCIAL] EXPORT_V3_EARLYRETURN_V4 ###
def _nowz():
    return datetime.now(timezone.utc).isoformat(timespec="microseconds").replace("+00:00","Z")
...
### [COMMERCIAL] EXPORT_V3_EARLYRETURN_V5B ###
def _nowz():
    return datetime.now(timezone.utc).isoformat(timespec="microseconds").replace("+00:00","Z")
...
def _ensure_report_dir(run_dir: str):
    ...
    if os.path.isfile(root_json) and (not os.path.isfile(rep_json)):
        try:
            shutil.copy2(root_json, rep_json)
        except Exception:
            pass
    return report_dir
