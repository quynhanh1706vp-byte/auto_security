...
cp -f "$F" "${F}.bak_p575_${TS}"
echo "[OK] backup => ${F}.bak_p575_${TS}"
...
# Normalize tar list: strip leading "./" and CR
tar_list="$(tar -tzf "$code_tgz" | sed 's#^\./##' | tr -d '\r')"
...
for r in "${required[@]}"; do
  echo "$tar_list" | grep -qx "$r" || missing+=("$r")
done

if [ "${#missing[@]}" -gt 0 ]; then
  echo "MISSING_IN_CODE_TGZ:${missing[*]}"
  echo "[DBG] code_tgz=$code_tgz"
  echo "[DBG] sample tar_list (first 30):"
  echo "$tar_list" | head -n 30
  exit 5
fi

log "[OK] contract inside code tgz PASS: $(basename "$code_tgz")"
exit 0
# --- END P575_NORMALIZE_TARLIST_V1 ---
# --- END P570_CHECK_INSIDE_CODE_TGZ ---'''

s2 = s[:i] + new_block + s[j_end:]
p.write_text(s2, encoding="utf-8")
print("[OK] replaced P570 block with P575 normalize+exact matcher")
PY
