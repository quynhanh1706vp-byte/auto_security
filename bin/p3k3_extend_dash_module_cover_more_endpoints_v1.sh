...
[ -f "$MOD" ] && cp -f "$MOD" "${MOD}.bak_p3k3_${TS}" && echo "[BACKUP] ${MOD}.bak_p3k3_${TS}"
cp -f "$W" "${W}.bak_p3k3_${TS}" && echo "[BACKUP] ${W}.bak_p3k3_${TS}"
...
"$PY" -c "import wsgi_vsp_ui_gateway; print('GW_IMPORT_OK')" >/dev/null
...
echo "== [3] smoke key endpoints JS calls =="
...
for u in \
  ...
  "/api/vsp/rid_latest_v3" \
  "/api/vsp/latest_rid_v1" \
  "/api/vsp/runs_v3?limit=12&offset=0" \
  "/api/vsp/trend_v1" \
  "/api/vsp/top_findings_v3c?limit=50" \
  "/api/vsp/findings_page_v3?limit=1&offset=0" \
  "/api/vsp/dashboard_v3_extras_v1?rid=$RID"
do
  code="$(curl -s -o /dev/null -w '%{http_code}' "$BASE$u" || true)"
  echo "GET $u => $code"
done

echo "[DONE] p3k3_extend_dash_module_cover_more_endpoints_v1"
