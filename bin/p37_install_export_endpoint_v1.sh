...
cp -f "$W" "${W}.bak_p37_export_${TS}"
echo "[BACKUP] ${W}.bak_p37_export_${TS}"
...
MARK="VSP_P37_EXPORT_ENDPOINT_V1"
pat=re.compile(r"(?s)\n# --- "+re.escape(MARK)+r" ---.*?\n# --- /"+re.escape(MARK)+r" ---\n")
s, n = pat.subn("\n", s)
if n:
    print("[OK] removed old block x", n)

block = r"""
# --- VSP_P37_EXPORT_ENDPOINT_V1 ---
...
def __vsp_p37_resolve_findings_file(rid: str):
    # Reuse same roots as P36, but self-contained.
    import os
    if not rid:
        return None
    roots=[]
...
def __vsp_p37_make_pdf_minimal(rid: str, total: int) -> bytes:
    # Minimal valid PDF (no external libs). One page with 2 lines.
    # Note: Keep ASCII only in PDF content stream.
    txt1 = f"VSP Export"
...
for fmt in html pdf zip; do
  ...
  curl -sS -D /tmp/_e.hdr -o /tmp/_e.bin "$BASE/api/vsp/export?rid=$RID&fmt=$fmt" || true
