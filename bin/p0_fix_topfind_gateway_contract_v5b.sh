...
cp -f "$W" "${W}.bak_topfind_v5b_${TS}"
echo "[BACKUP] ${W}.bak_topfind_v5b_${TS}"
...
MARKER = "VSP_P0_TOPFIND_GATEWAY_V5B"

route_block = r'''
# VSP_P0_TOPFIND_GATEWAY_V5B
...
@app.route("/api/vsp/top_findings_v1", methods=["GET"], endpoint="vsp_top_findings_v1_gateway_v5b")
def vsp_top_findings_v1_gateway_v5b():
    ...
    def build_fail(reason: str, rid_used: str = ""):
        return jsonify({
            ...
            "items": [],
            "reason": reason,
            "err": reason,  # backward compatible
...
# END VSP_P0_TOPFIND_GATEWAY_V5B
'''

# If old V5B block exists, replace it; else disable any existing route decorators for same path then insert block.
if "VSP_P0_TOPFIND_GATEWAY_V5B" in s:
    s = re.sub(
        r'#\s*VSP_P0_TOPFIND_GATEWAY_V5B[\s\S]*?#\s*END\s*VSP_P0_TOPFIND_GATEWAY_V5B',
        route_block.strip(),
        s,
        flags=re.M
    )
    ...
    # insert before application= if present, else append
...
print("[OK] wrote V5B gateway block for top_findings_v1")
PY
...
print("ok=",j.get("ok"),"rid_req=",j.get("rid_requested"),"rid_used=",j.get("rid_used"),"total=",j.get("total"),"reason=",j.get("reason"))
