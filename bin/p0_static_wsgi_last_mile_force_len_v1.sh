...
try:
    {appv}.wsgi_app = _vsp_p0_wsgi_static_last_mile_force_len_v1({appv}.wsgi_app)
except Exception:
    pass
# --- /{MARK} ---
"""

# append at EOF so it is LAST assignment (strongest)
s2 = s.rstrip() + "\n\n" + block + "\n"
p.write_text(s2, encoding="utf-8")
print("[OK] appended WSGI last-mile static wrapper at EOF.")
PY
...
echo "== PROBE HEAD must have Content-Length > 0 =="
...
echo "== PROBE /vsp5 include gate story =="
curl -fsS "$BASE/vsp5" | grep -n "vsp_dashboard_gate_story_v1.js" | head -n 3 || true
echo "[DONE] WSGI last-mile static fix applied."
