...
echo "== [1] backup =="
cp -f "$JS" "${JS}.bak_rewrite_safe_${TS}"
echo "[BACKUP] ${JS}.bak_rewrite_safe_${TS}"

echo "== [2] rewrite JS (safe/compact) =="
python3 - <<'PY'
...
(()=> {
  ...
  function applyKpi(j, days){
    ...
    setText("vsp_runs_kpi_green_window", j.by_overall?.GREEN ?? "—");
    setText("vsp_runs_kpi_amber_window", j.by_overall?.AMBER ?? "—");
    setText("vsp_runs_kpi_red_window",   j.by_overall?.RED ?? "—");
    setText("vsp_runs_kpi_unknown_window", j.by_overall?.UNKNOWN ?? "—");
    setText("vsp_runs_kpi_has_findings_window", j.has_findings ?? "—");
    setText("vsp_runs_kpi_latest_rid", j.latest_rid ?? "—");
...
print("[OK] wrote safe compact KPI JS:", p)
PY
...
echo "== [4] sanity API v2 =="
curl -sS "$BASE/api/ui/runs_kpi_v2?days=30" | head -c 220; echo

echo
echo "[DONE] Now hard-reload /runs (Ctrl+Shift+R). KPI should fill without freezing layout."
