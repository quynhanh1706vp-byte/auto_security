...
need date; need python3; need curl; need bash; need head; need sed; need wc; need mktemp

F="bin/commercial_selfcheck_v1.sh"
TS="$(date +%Y%m%d_%H%M%S)"
[ -f "$F" ] && cp -f "$F" "${F}.bak_rewrite_${TS}" && echo "[BACKUP] ${F}.bak_rewrite_${TS}"
