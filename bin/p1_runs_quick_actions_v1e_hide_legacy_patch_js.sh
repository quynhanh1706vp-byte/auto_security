...
/* VSP_P1_RUNS_QUICK_ACTIONS_V1E_HIDE_LEGACY */
(()=> {
  if (window.__vsp_p1_runs_quick_actions_v1e) return;
  window.__vsp_p1_runs_quick_actions_v1e = true;

  const log = (...a)=>console.log("[RunsQuickV1E]", ...a);
  const warn = (...a)=>console.warn("[RunsQuickV1E]", ...a);
  ...
  // --------- HIDE LEGACY (the "VSP Runs & Reports" block) ----------
  function hideLegacyExcept(anchor){
    const hidden = [];

    function markHide(node){
      if (!node || node === document.body) return false;
      if (anchor && anchor.contains(node)) return false;
      node.classList.add("vsp-legacy-hidden");
      node.setAttribute("data-vsp-legacy-hidden", "1");
      hidden.push(node);
      return true;
    }

    // 1) try known ids/classes
    const selectors = [
      "#vspRunsReports", "#vspRunsReportsV1", "#runsReports", "#runs_reports",
      "#runs-root", "#runsRoot", "#vsp_runs_reports_root",
      ".vsp-runs-reports", ".vsp-runs-reports-root",
      "[data-vsp-runs-reports]", "[data-vsp-runs-root]",
    ];
    for (const sel of selectors){
      const n = qs(sel);
      if (n && (!anchor || !anchor.contains(n))) markHide(n);
    }

    // 2) heuristic by heading text “VSP Runs & Reports”
    ...
    for (const h of candidates){
      ...
      // climb up to find a reasonable container to hide
      let cur = h;
      for (let i=0;i<8;i++){
        if (!cur || cur === document.body) break;
        const tag = (cur.tagName||"").toLowerCase();
        const cls = (cur.className||"").toString().toLowerCase();
        if (tag === "section" || tag === "main") { markHide(cur); break; }
        if (cls.includes("container") || cls.includes("panel") || cls.includes("card") || cls.includes("content") || cls.includes("page")) { markHide(cur); break; }
        cur = cur.parentElement;
      }
    }

    // 3) if legacy is still visible, hide the largest table area not belonging to our quick actions
    const tables = Array.from(document.querySelectorAll("table")).filter(t => !(anchor && anchor.contains(t)));
    if (tables.length){
      // pick the biggest table by rows and hide its closest wrapper
      ...
      for (let i=0;i<6;i++){
        if (!cur || cur === document.body) break;
        if (cur.tagName && ["SECTION","MAIN","DIV"].includes(cur.tagName)){
          // avoid hiding body-level; pick a wrapper under body
          if (cur.parentElement && cur.parentElement !== document.body) { markHide(cur); break; }
        }
        cur = cur.parentElement;
      }
    }
  ...
  function mount(){
    ...
    // hide legacy block by default
    const legacyHidden = hideLegacyExcept(anchor);
    if (legacyHidden.length) log("legacy hidden nodes:", legacyHidden.length);
    ...
    const legacyBtn=el("button",{class:"vsp-runsqa-btn"},[legacyShown ? "Hide legacy" : "Show legacy"]);
    ...
    const wrap=el("div",{class:"vsp-runsqa-wrap"},[
      el("div",{class:"vsp-runsqa-mini"},["Quick Actions V1E: auto-hide legacy Runs & Reports block (toggle bằng Show legacy)."]),
      el("div",{class:"vsp-runsqa-toolbar"},[
        ...
        refreshBtn, legacyBtn, clearBtn, stat
      ])
    ]);
    ...
    function render(){
      ...
      stat.textContent = `Runs: ${items.length}/${runsCache.length} | legacy_hidden=${(window.__vsp_runs_legacy_hidden_nodes||[]).length}`;
    }
...
print("[OK] wrote V1E =>", js)
PY

echo "[DONE] JS patched to V1E. Mở /runs và Ctrl+Shift+R. Legacy block sẽ tự ẩn (có nút Show legacy để bật lại)."
