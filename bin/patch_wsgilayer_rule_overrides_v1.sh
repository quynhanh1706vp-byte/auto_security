...
[ -f "$F" ] || { echo "[ERR] missing $F (exportpdf_only module not found)"; exit 2; }

TS="$(date +%Y%m%d_%H%M%S)"
cp -f "$F" "$F.bak_rule_overrides_wsgilayer_${TS}"
echo "[BACKUP] $F.bak_rule_overrides_wsgilayer_${TS}"
...
TAG = "# === VSP_WSGI_RULE_OVERRIDES_LAYER_V1 ==="
if TAG in t:
    print("[OK] already patched, skip")
    raise SystemExit(0)

# Replace whole file with a deterministic commercial wrapper
new = r'''
...
print("[OK] replaced exportpdf_only module with commercial preempt (overrides+apply)")
PY
