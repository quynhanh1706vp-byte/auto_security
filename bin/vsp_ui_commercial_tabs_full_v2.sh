 ...
 *   + Badge severity / tool ở Data Source (MutationObserver để bắt async load).
...
(function () {
  ...
  function styleDataSourceRows(tbody) {
    ...
    rows.forEach(function (tr) {
      ...
      // Severity badge – nếu đã có .vsp-badge-sev-* thì bỏ qua
      if (sevCell) {
      ...
      // Tool badge – nếu đã có .vsp-badge-tool thì bỏ qua
      if (toolCell) {
      ...
      // Path short + tooltip – chỉ làm 1 lần
  ...
  function enhanceDataSourceTab() {
    ...
    if (!tbody) {
      console.log(
        LOG_PREFIX,
        "Không thấy tbody Data Source (#vsp-datasource-tbody / #vsp-ds-tbody) – bỏ qua polish Data (V2)."
      );
      return;
    }
    ...
    // Style lần đầu (nếu đã có data)
    styleDataSourceRows(tbody);

    // Dùng MutationObserver để bắt async load từ /api/vsp/datasource_v2
...
if changed:
    backup = tpl_path.with_suffix(tpl_path.suffix + ".bak_comm_tabs_v2")
...
echo "[VSP_UI_COMM] DONE – nhớ restart Flask + refresh (Ctrl+F5)."
