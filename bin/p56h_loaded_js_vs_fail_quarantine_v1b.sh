...
EVID="$OUT/p56h_loaded_js_${TS}"
mkdir -p "$EVID"

need(){ command -v "$1" >/dev/null 2>&1 || { echo "[ERR] missing: $1"; exit 2; }; }
need curl; need awk; need sed; need grep; need sort; need uniq; need ls; need find; need python3; need node; need wc; need head

echo "== [P56H v1b] detect LOADED JS (5 tabs) + compare with FAIL list ==" | tee "$EVID/summary.txt"
...
loaded = Path("$EVID/loaded_js.txt").read_text().splitlines() if Path("$EVID/loaded_js.txt").exists() else []
fails  = Path("$EVID/fail_js.txt").read_text().splitlines() if Path("$EVID/fail_js.txt").exists() else []
loaded=set(x.strip() for x in loaded if x.strip())
fails=set(x.strip() for x in fails if x.strip())
inter=sorted(loaded.intersection(fails))
Path("$EVID/intersection_loaded_fail.txt").write_text(("\n".join(inter)+"\n") if inter else "")
print("[OK] loaded_fail_count=", len(inter))
PY | tee -a "$EVID/summary.txt" >/dev/null

echo "== loaded∩fail ==" | tee "$EVID/loaded_fail.txt" >/dev/null
cat "$EVID/intersection_loaded_fail.txt" | tee -a "$EVID/loaded_fail.txt" >/dev/null || true

# 4) if loaded&fail => shim to stop UI crash
if [ -s "$EVID/intersection_loaded_fail.txt" ]; then
  echo "[WARN] FAIL JS are LOADED -> installing shims to stop UI crash..." | tee -a "$EVID/summary.txt"
...
JS
    if node --check "$f" >/dev/null 2>&1; then
      echo "[OK] shimmed $f" | tee -a "$EVID/summary.txt"
    else
      echo "[ERR] shim syntax fail $f" | tee -a "$EVID/summary.txt"
    fi
...
# 5) quarantine NOT-loaded fails: rename *.js -> *.disabled_p56h_TS.js
python3 - <<PY
from pathlib import Path
import time
ts=time.strftime("%Y%m%d_%H%M%S")
loaded=set(Path("$EVID/loaded_js.txt").read_text().splitlines()) if Path("$EVID/loaded_js.txt").exists() else set()
fails=set(Path("$EVID/fail_js.txt").read_text().splitlines()) if Path("$EVID/fail_js.txt").exists() else set()
actions=[]
...
Path("$EVID/quarantine_actions.txt").write_text(("\n".join(actions)+"\n") if actions else "")
print("[OK] quarantine_actions=", len(actions))
PY | tee -a "$EVID/summary.txt" >/dev/null

# 6) re-check syntax of LOADED JS only
echo "== [P56H] node --check LOADED JS ==" | tee -a "$EVID/summary.txt"
bad=0
: > "$EVID/loaded_js_check.txt"
while IFS= read -r url; do
  [ -n "$url" ] || continue
  f="${url#/}"
  [ -f "$f" ] || continue
...
if [ "$bad" = "0" ]; then
  ...
  echo "[WARN] some LOADED js still fail syntax -> $EVID/loaded_js_check.txt" | tee -a "$EVID/summary.txt"
fi
