...
log "== [P60 v3] Runtime gate (Playwright, strict evidence) =="
log "UI=$UI"
log "BASE=$BASE"
log "EVID=$EVID"
...
try:
  ...
  if not rid and isinstance(j.get("items"), list) and j["items"]:
    rid=j["items"][0].get("rid") or j["items"][0].get("run_id")
  print((rid or "").strip())
except Exception:
  print("")
PY2
...
node -e "require.resolve('playwright', {paths:[process.cwd()]});" >/dev/null 2>&1 || {
  log "[ERR] playwright not resolvable from UI. Install: cd $UI && npm i -D playwright && npx playwright install chromium"
