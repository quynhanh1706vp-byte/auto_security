...
cp -f "$APP" "$APP.bak_wsgi_bytes_gitleaks_v2_${TS}"
echo "[BACKUP] $APP.bak_wsgi_bytes_gitleaks_v2_${TS}"
...
# 1) Insert middleware class (near top)
insert_pos = 0
m = re.search(r"(?s)\A.*?\n\n", t)  # after first blank line block
if m:
    insert_pos = m.end()
...
t = t[:insert_pos] + middleware + t[insert_pos:]
print("[OK] inserted middleware class")

# 2) Install wrapper AFTER the preempt-installed print (indent-safe)
pm = re.search(r"(?m)^(?P<ind>\s*)print\(\s*['\"]\[VSP_WSGI_PREEMPT_V1\]\s*installed['\"]\s*\)\s*$", t)
if not pm:
    # fallback: any print containing VSP_WSGI_PREEMPT_V1 + installed
    pm = re.search(r"(?m)^(?P<ind>\s*)print\(\s*['\"].*VSP_WSGI_PREEMPT_V1.*installed.*['\"]\s*\)\s*$", t)
if not pm:
    raise SystemExit("[ERR] cannot find print('[VSP_WSGI_PREEMPT_V1] installed') anchor to attach wrapper")

ind = pm.group("ind")
eol = t.find("\n", pm.end())
if eol == -1:
    raise SystemExit("[ERR] anchor line has no EOL?")
...
install = "\n".join([
    ...
    f"{ind}    print('[VSP_WSGI_BYTES_POSTPROCESS_STATUSV2_GITLEAKS_V1][WARN] install failed:', _e)",
    ""
])

t = t[:eol+1] + install + t[eol+1:]
p.write_text(t, encoding="utf-8")
print("[OK] installed wrapper after preempt print anchor")
PY
