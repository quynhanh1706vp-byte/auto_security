...
cp -f "$W" "${W}.bak_p35_ro_wsgi_${TS}"
echo "[BACKUP] ${W}.bak_p35_ro_wsgi_${TS}"
...
MARK="VSP_P35_RULE_OVERRIDES_WSGI_CRUD_PERSIST_V2"
if MARK in s:
...
block=f"""
# --- {MARK} ---
# Commercial: implement Rule Overrides CRUD+persist at WSGI layer (works even if upstream routes are proxy/cached).
# Storage: /home/test/Data/SECURITY_BUNDLE/ui/out_ci/rule_overrides_v1/rule_overrides.json

__vsp_p35_ro_wrapped_v2 = globals().get("__vsp_p35_ro_wrapped_v2", False)
...
def __vsp_p35_ro_read_body(environ):
    ...
    if n <= 0:
        return b""
    try:
        return environ["wsgi.input"].read(n) or b""
    except Exception:
        return b""
...
def __vsp_p35_ro_handle(environ, start_response):
    ...
    def bad(reason):
        return __vsp_p35_ro_json(200, {{"ok": False, "reason": reason, "ts": int(time.time())}}, start_response)

    if method == "GET":
        return ok({{"items": items, "total": len(items), "path": FILE}})
    ...
    if method == "POST":
        ...
        if isinstance(body, dict) and isinstance(body.get("items"), list):
            ...
            return ok({{"items": merged, "total": len(merged), "path": FILE}})
        ...
        return ok({{"created": it, "total": len(out), "path": FILE}})
    ...
    if method == "PUT":
        ...
        for x in items:
            if isinstance(x, dict) and x.get("id")==it["id"]:
                ...
                out.append(x)
        if not found:
            return bad("id not found")
        out=[x for x in out if isinstance(x, dict)]
        out.sort(key=lambda z: (z.get("id") or ""))
        __vsp_p35_ro_write_atomic(FILE, out)
        return ok({{"updated": it, "total": len(out), "path": FILE}})
    ...
    if method == "DELETE":
        ...
        try:
            ...
            did = ""
        if not did and isinstance(body, dict):
        ...
        out.sort(key=lambda z: (z.get("id") or ""))
        __vsp_p35_ro_write_atomic(FILE, out)
        return ok({{"deleted": did, "total": len(out), "path": FILE}})

    return bad("method not supported")

try:
    if not __vsp_p35_ro_wrapped_v2:
        ...
        if callable(_orig_app):
            ...
            globals()["__vsp_p35_ro_wrapped_v2"] = True
except Exception:
    pass
# --- /{MARK} ---
"""
...
echo "== [P35 TEST] CRUD rule_overrides_v1 (WSGI) =="
...
echo "-- GET1 (must contain created) --"
curl -fsS "$BASE/api/vsp/rule_overrides_v1" | python3 - <<'PY'
import sys, json
j=json.load(sys.stdin)
