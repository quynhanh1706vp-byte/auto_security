...
echo "== [0] stop + reset-failed to stop restart storm =="
...
echo "== [2] write unit v4 (NO pidfile, capture-output to journal) =="
...
# Precheck: import module (quote-safe)
...
echo "== [4] journal last 220 lines (this will show REAL gunicorn error) =="
sudo journalctl -u "$SVC" -n 220 --no-pager || true

echo "== [5] verify listen :$PORT (sudo) =="
