...
MARK = "VSP_P1_PAGE_BOOT_V1"
JS_PATH = Path("static/js/vsp_p1_page_boot_v1.js")
...
# inject into templates (safe + idempotent)
candidates = [
...
SCRIPT_TAG = f'<script src="/static/js/vsp_p1_page_boot_v1.js?v={MARK}"></script>'

def inject_before_body_end(html: str) -> str:
  if MARK in html and "vsp_p1_page_boot_v1.js" in html:
    return html
  if SCRIPT_TAG in html:
    return html
  # before </body> if exists, else append
...
for f in candidates:
  ...
  s2 = inject_before_body_end(s)
  if s2 != s:
    bak = f"{f}.bak_p1boot_{int(time.time())}"
    Path(bak).write_text(s, encoding="utf-8")
    f.write_text(s2, encoding="utf-8")
    print("[OK] injected boot script into:", f.name, "(backup:", Path(bak).name, ")")
    patched += 1
  else:
    print("[OK] already/unchanged:", f.name)

print("[DONE] templates patched:", patched)
PY

echo "[OK] now restart 8910 to pick up template/js changes"
