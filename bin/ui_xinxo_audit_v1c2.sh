...
need curl; need grep; need sed; need sort; need head; need python3; need date
...
echo "VSP UI XINXO AUDIT v1c2  @ $(ts)"
echo "BASE=$BASE"
hr
...
echo "== [B] Per-tab scan + NOTES (strict, N/A uses fixed-string) =="
...
for P in "${PAGES[@]}"; do
  ...
  if grep -qF "N/A" "$scan"; then na=1; match="${match}N/A;"; fi
