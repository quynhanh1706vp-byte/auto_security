...
backup="$JS.bak_bind_v7_$(date +%Y%m%d_%H%M%S)"
cp "$JS" "$backup"
echo "[BACKUP] $JS -> $backup"

cat > "$JS" << 'JS'
/**
 * VSP Dashboard KPI binder V7
 * - Chỉ bind số trong panel "KPI ZONE – 10 KEY METRICS".
 * - Các block bên dưới (Trend, Compliance, Medium/Low/Info, ...) không bị ảnh hưởng.
 */
(function () {
  const LOG = "[VSP][KPI_V7]";
  ...
  function getKpiRoot() {
    ...
    for (const el of all) {
      ...
      if (!t) continue;
      if (t.includes("KPI ZONE") && t.includes("10 KEY METRICS")) {
        // Lấy div/section bao ngoài làm root
        const root =
          el.closest("section") || el.closest("div") || el.parentElement;
        if (root) return root;
      }
    }
    // fallback: cả tab (trường hợp không tìm thấy)
    return tab;
  }

  function findLabelNodes(upperMatch) {
    const root = getKpiRoot();
  ...
  function getCardContainer(labelEl) {
    ...
    // Giới hạn trong root KPI zone
    const kpiRoot = getKpiRoot();
    let container = labelEl.closest("div");
    while (container && container !== kpiRoot && !kpiRoot.contains(container)) {
      container = container.parentElement;
    }
    if (!container || !kpiRoot.contains(container)) {
      // nếu không thuộc KPI zone thì bỏ
      return null;
    }
    return container;
  }
...
echo "[DONE] patch_vsp_dashboard_kpi_bind_v7.sh hoàn tất."
