...
LOG_PREFIX="[VSP_ENABLE_RUNS_TAB_SIMPLE_V2]"
...
# 1) Ghi file JS v2
########################
cat > static/js/vsp_runs_tab_simple_v2.js << 'JS'
(function () {
  const LOG_PREFIX = "[VSP_RUNS_TAB_SIMPLE_V2]";
  ...
  function ensureContainer() {
    const runsTab = document.querySelector("#vsp-tab-runs");
    if (!runsTab) {
      log("Không tìm thấy #vsp-tab-runs – bỏ qua.");
      return null;
    }
    let container = runsTab.querySelector("#vsp-runs-overview");
    if (!container) {
      container = document.createElement("div");
      container.id = "vsp-runs-overview";
      container.className = "vsp-card vsp-card-runs-simple";
  ...
  function injectStyles() {
    if (document.getElementById("vsp-runs-tab-simple-v2-style")) return;
    ...
    style.id = "vsp-runs-tab-simple-v2-style";
  ...
  function detectBadge(item) {
    ...
    if (id.startsWith("RUN_VSP_CI_")) {
      return { label: "CI", className: "badge badge-ci" };
    }
  ...
  function renderTable(container, items) {
    ...
    title.textContent = "Runs history – last 20 runs";
  ...
  async function loadRunsSimpleV2() {
    ...
    try {
      ...
      container.textContent = "Failed to load runs (network error).";
    }
  }

  // Chỉ load khi DOM ready. Bảng nằm trong #vsp-tab-runs, nên chỉ hiển thị khi tab này active.
  document.addEventListener("DOMContentLoaded", loadRunsSimpleV2);

  // Cho phép gọi lại từ console / patch khác
  window.vspRunsSimpleReloadV2 = loadRunsSimpleV2;
  // Giữ alias cũ để không vỡ script đang dùng
  window.vspRunsSimpleReload = loadRunsSimpleV2;
})();
JS

echo "$LOG_PREFIX [OK] Đã ghi static/js/vsp_runs_tab_simple_v2.js"
...
for tpl in "${CANDIDATES[@]}"; do
  ...
  backup="${tpl}.bak_runs_tab_simple_v2_${ts}"
  cp "$tpl" "$backup"
...
# 2.1) Đảm bảo có container #vsp-runs-overview bên trong #vsp-tab-runs
if 'id="vsp-runs-overview"' not in txt:
    ...
    if idx != -1:
        ...
        if gt_idx != -1:
            ...
            print(LOG_PREFIX, "[OK] Inject <div id=\\"vsp-runs-overview\\"> vào", "$tpl")
            changed = True

# 2.2) Thay mọi tham chiếu v1 -> v2
if 'vsp_runs_tab_simple_v1.js' in txt:
    txt = txt.replace('vsp_runs_tab_simple_v1.js', 'vsp_runs_tab_simple_v2.js')
    print(LOG_PREFIX, "[OK] Thay v1 -> v2 trong", "$tpl")
    changed = True

# 2.3) Nếu chưa có script v2 thì thêm trước </body>
if 'vsp_runs_tab_simple_v2.js' not in txt:
    tag = '\\n    <script src="{{ url_for(\\'static\\', filename=\\'js/vsp_runs_tab_simple_v2.js\\') }}"></script>'
    body_idx = txt.lower().rfind("</body>")
    if body_idx != -1:
        txt = txt[:body_idx] + tag + "\\n" + txt[body_idx:]
        print(LOG_PREFIX, "[OK] Inject script v2 vào", "$tpl")
        changed = True
...
if [ "$patched" -eq 0 ]; then
  ...
  echo "$LOG_PREFIX [DONE] Hoàn tất enable Runs tab simple v2."
fi
