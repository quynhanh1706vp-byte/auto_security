...
if [ -f "$JS" ]; then
  BAK="$JS.bak_v26_$(date +%Y%m%d_%H%M%S)"
  cp "$JS" "$BAK"
...
cat > "$JS" << 'JS'
/**
 * VSP 2025 – UI Extras V2.7
 * - Shell + header cho 5 tab
 * - Dashboard extras:
 *     + Top 10 High/Critical
 *     + Delta giữa 2 run gần nhất
 *     + CI Gate – Latest Run (card nhỏ)
 * - Runs & Reports:
 *     + Filter Run ID / Status
 *     + CI/CD Gate summary (kéo từ runs_index_v3)
 *     + Badge màu cho cột cuối (STATUS / GATE)
 * - Data Source:
 *     + Filter severity / tool
 *     + Data Source summary (total, theo severity, theo tool)
 * - Settings:
 *     + Summary + 3 block Scan / Tools / CI Gate (khi settings có data)
 * - Rule Overrides:
 *     + Summary bảng rule overrides (nếu có), hoặc card “chưa có override”
 */

(function () {
  console.log("[VSP_V27] vsp_ui_extras_v25.js loaded (V2.7)");
  ...
  function safeParseJSON(text) {
    try {
      ...
      // Payload không phải JSON chuẩn thì bỏ qua, không log lỗi
      return null;
    }
  }
  ...
  function observeOnce(rootId, onReady) {
    ...
    // Gọi 1 lần khi hiện tại
    try { onReady(root); } catch (e) { console.warn("[VSP_V27] onReady initial error", e); }

    var obs = new MutationObserver(function () {
      try { onReady(root); } catch (e) { console.warn("[VSP_V27] onReady mutation error", e); }
    });
  ...
  function buildDashboardExtras() {
    ...
    // CI Gate – Latest Run (card nhỏ)
    if (!root.querySelector(".vsp-ci-gate-card")) {
      var gateCard = document.createElement("div");
      gateCard.className = "vsp-table-card vsp-ci-gate-card vsp-fadein vsp-fadein-delay-3";
      gateCard.style.marginTop = "18px";
      gateCard.style.maxWidth = "420px";
      ...
      gateCard.appendChild(header);

      var body = document.createElement("div");
      body.innerHTML =
        "<div style='font-size:12px;color:#9ca3af;margin-bottom:4px;' id='vsp-ci-gate-run-id'>Run: N/A</div>" +
        "<div style='font-size:13px;margin-bottom:8px;'>Total findings: <span style='font-size:20px;font-weight:600' id='vsp-ci-gate-total'>N/A</span></div>" +
        "<div style='display:flex;flex-wrap:wrap;gap:6px;font-size:11px;margin-bottom:6px;' id='vsp-ci-gate-sev-chips'></div>" +
        "<div style='font-size:11px;color:#6b7280;'>Source: <span id='vsp-ci-gate-source'>N/A</span></div>";
      gateCard.appendChild(body);
  ...
  // CI Gate – Latest Run (đang dùng dashboard_v3 + runs_index_v3)
  function fetchCiGateLatest() {
    ...
    // Lấy by_severity + default gate status từ dashboard_v3
    fetch("/api/vsp/dashboard_v3")
      .then(function (r) { return r.json(); })
      .then(function (dash) {
        ...
        // tính gate dựa trên score tạm thời
        var status = "OK";
  ...
  /* ------------ RUNS TAB: FILTER + SUMMARY + BADGES ------------ */
  ...
  function enhanceRunsTab(root) {
    ...
    // CI/CD Gate summary card – chèn TRƯỚC table, dùng parent của table để tránh NotFoundError
    if (!root.querySelector(".vsp-runs-gate-summary-card")) {
      ...
      sub.textContent = "Tổng quan từ /api/vsp/runs_index_v3 (limit=40).";
      ...
      sumTable.className = "vsp-table-compact";
      sumTable.innerHTML =
        "<thead><tr><th>Metric</th><th>Value</th></tr></thead>" +
        "<tbody id='vsp-runs-gate-summary-body'><tr><td colspan='2'>Đang tải...</td></tr></tbody>";
      card.appendChild(sumTable);

      // SỬA LỖI: dùng parentNode của table, không dùng root.insertBefore trực tiếp
  ...
  function enhanceDatasourceTab(root) {
    ...
    if (!root.querySelector(".vsp-ds-summary-card")) {
      ...
      var sub = document.createElement("div");
      sub.className = "vsp-table-card-sub";
      sub.textContent = "Thống kê nhanh từ /api/vsp/datasource_v2 (limit=500).";
      ...
      sTable.className = "vsp-table-compact";
      sTable.innerHTML =
        "<thead><tr><th>Metric</th><th>Value</th></tr></thead>" +
  ...
  function fetchDatasourceSummary() {
  ...
  function enhanceSettingsTab(root) {
    ...
    // Grid 3 card: Scan / Tools / CI Gate
  ...
  function bootstrap() {
    try {
      ...
      console.warn("[VSP_V27] dashboard bootstrap error", e);
    }
...
echo "$LOG_PREFIX [OK] Đã ghi JS V2.7 vào $JS"
echo "$LOG_PREFIX Hoàn tất patch UI V2.7 – Runs/DataSource ổn định + Dashboard CI Gate."
