...
(()=> {
  try {
    ...
    function _isLikelyRid(rid){
      ...
      // avoid weird template-ish strings
    ...
    window.fetch = function(input, init){
      try {
        ...
        if (url0 && url0.includes("/api/vsp/run_file_allow")) {
          ...
          if (!_isLikelyRid(rid)) {
            // return a safe JSON payload instead of hitting backend
...
# Where to inject: after first IIFE/opening if possible, else prepend
# Try inject after "(()=>{" or "(function(){"
...
echo "== [5] smoke: marker reachable from browser =="
# Try curl the JS directly (strip query in case)
...
curl -fsS "$BASE$JS_URL" | grep -n "VSP_RUNFILEALLOW_FETCH_GUARD_V1" >/dev/null \
  ...
  || echo "[WARN] cannot confirm marker via curl (maybe different JS is served, but file patched)"

echo "[DONE] Ctrl+F5 on /settings, /data_source, /rule_overrides. Open DevTools Console => should stop run_file_allow 404 spam."
