...
cp -f "$F" "$F.bak_earlyreturn_v5_${TS}"
echo "[BACKUP] $F.bak_earlyreturn_v5_${TS}"
...
marker="### [COMMERCIAL] EXPORT_V3_EARLYRETURN_V5 ###"
if marker in s:
    print("[OK] already has V5")
    raise SystemExit(0)

# ensure imports used in block
imports = [
  "import zipfile",
  "import tempfile",
  "import subprocess",
...
# insert immediately after def run_export_v3(...) line
m = re.search(r'(?m)^(\s*)def\s+run_export_v3\s*\(.*\)\s*:\s*$', s)
if not m:
    raise SystemExit("[ERR] cannot find def run_export_v3")

base = m.group(1)
indent = base + "    "
early = f"""
{indent}# [COMMERCIAL] EARLYRETURN_V5 (force enable html/zip/pdf)
...
{indent}        resp.headers["X-VSP-EXPORT-MODE"] = "EARLYRETURN_V5"
...
print("[OK] inserted EARLYRETURN_V5")
PY
