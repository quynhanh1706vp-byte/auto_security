...
cp -f "$W" "${W}.bak_tabs3_bundle_fix1_${TS}"
echo "[BACKUP] ${W}.bak_tabs3_bundle_fix1_${TS}"

mkdir -p static/js templates bin out_ci tools

# -------------------------
# 1) Blueprint v4 (APIs: runs/findings/settings/rule_overrides/apply)
# -------------------------
cat > vsp_tabs3_ui_bp_v4.py <<'PY'
# -*- coding: utf-8 -*-
...
PY
echo "[OK] wrote vsp_tabs3_ui_bp_v4.py"

# -------------------------
# 2) Register blueprint in gateway (append if missing)
# -------------------------
python3 - <<'PY'
from pathlib import Path
import time
...
marker="VSP_TABS3_UI_BP_REGISTER_V4"
if marker not in s:
    s += r'''
...
# -------------------------
# 3) JS v3 (nav + DataSource RID + Settings effective + RuleOverrides apply)
# -------------------------
cat > static/js/vsp_tabs3_common_v3.js <<'JS'
(() => {
...
cat > static/js/vsp_settings_tab_v3.js <<'JS'
(() => {
...
cat > static/js/vsp_rule_overrides_tab_v3.js <<'JS'
(() => {
...
echo "[OK] wrote JS v3 files"

# -------------------------
# 4) Minimal templates (no Jinja vars)
# -------------------------
cat > templates/vsp_data_source_2025.html <<'HTML'
...
<script src="/static/js/vsp_data_source_tab_v3.js?v=1"></script>
</body></html>
HTML

cat > templates/vsp_settings_2025.html <<'HTML'
<!doctype html><html lang="vi"><head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>
...
cat > templates/vsp_rule_overrides_2025.html <<'HTML'
<!doctype html><html lang="vi"><head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>
...
<div id="vsp_tab_root" data-vsp-tab="rule_overrides" style="padding:16px"><div style="color:#94a3b8;font-size:12px">Loading...</div></div>
<script src="/static/js/vsp_tabs3_common_v3.js?v=1"></script>
<script src="/static/js/vsp_rule_overrides_tab_v3.js?v=1"></script>
</body></html>
HTML
echo "[OK] rewrote minimal templates"

# -------------------------
# 5) Settings -> ENV export script (NO python triple quotes)
# -------------------------
cat > bin/vsp_settings_env_export_v1.sh <<'SH2'
#!/usr/bin/env bash
...
SH2
chmod +x bin/vsp_settings_env_export_v1.sh
echo "[OK] wrote bin/vsp_settings_env_export_v1.sh"

# Best-effort: patch runner scripts to source settings env (if found)
python3 - <<'PY'
from pathlib import Path
import time
root=Path("/home/test/Data/SECURITY_BUNDLE/ui/bin")
marker="VSP_SETTINGS_SOURCE_V1"
ins = """
...
for f in cands:
    ...
    if s.startswith("#!"):
        ...
        s2=ins+"\n"+s
    bak=f.with_name(f.name+f".bak_settings_fix1_{time.strftime('%Y%m%d_%H%M%S')}")
    bak.write_text(s, encoding="utf-8")
    f.write_text(s2, encoding="utf-8")
    print("[OK] patched runner:", f.name)
PY
...
echo "== verify APIs ok:true =="
curl -fsS "http://127.0.0.1:8910/api/ui/runs_v1?limit=2" | head -c 220; echo
curl -fsS "http://127.0.0.1:8910/api/ui/findings_v2?limit=1&offset=0" | head -c 220; echo
curl -fsS "http://127.0.0.1:8910/api/ui/settings_effective_v1" | head -c 220; echo
curl -fsS "http://127.0.0.1:8910/api/ui/rule_overrides_v2" | head -c 220; echo

echo "== verify JS bytes > 0 =="
