...
BAK="${PYF}.bak_ridlatest_reqfind_${TS}"
cp -f "$PYF" "$BAK"
echo "[BACKUP] $BAK"

export MARK="VSP_P0_RID_LATEST_REQUIRE_FINDINGS_V1B"
...
try:
    ...
    if _app is None:
        ...
        if eps:
            ...
            print("[{MARK}] OK added new rule endpoint=vsp_rid_latest_require_findings_v1b")
...
echo "[DONE] rid_latest now REQUIRES findings_unified (root or reports)."
