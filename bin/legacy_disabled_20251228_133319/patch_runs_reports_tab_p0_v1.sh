...
def _vsp_pick_html_report(run_dir: _VSP_Path):
    ...
    # fallback: first html under report*/reports*
    for sub in ("report","reports"):
        d = run_dir / sub
        if d.exists():
            for fp in sorted(d.rglob("*.html"))[:1]:
                try: return str(fp.relative_to(run_dir))
...
def vsp_api_runs():
    ...
    return jsonify({"ok": True, "who": "{MARK}", "root": str(root), "items": items, "items_len": len(items)})
...
print("[OK] injected /runs + /api/vsp/runs + /api/vsp/run_file")
PY

python3 -m py_compile "$APP"
echo "[OK] py_compile OK"
echo "[NEXT] restart 8910 then open:"
