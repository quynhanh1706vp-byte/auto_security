...
BK="$OUT/${APP}.bak_before_p4848_${TS}"
cp -f "$APP" "$BK"
...
MARK = "VSP_P4848_RUNS3_AFTER_REQUEST_ITEMS_ALIAS"
if MARK in s:
    print("[OK] already patched P4848")
    raise SystemExit(0)

# Find Flask app variable name:  <name> = Flask(...)
m = re.search(r'^\s*([A-Za-z_]\w*)\s*=\s*Flask\s*\(', s, flags=re.M)
if not m:
    raise SystemExit("[ERR] cannot find `<appvar> = Flask(` in vsp_demo_app.py (need manual locate).")
appvar = m.group(1)

# Insert block right AFTER the Flask() assignment line (safe, early)
# We insert after the end of that line only.
idx = m.end()
line_end = s.find("\n", idx)
if line_end < 0:
    line_end = idx
...
def _vsp_p4848_runs3_after_request(resp):
    try:
        ...
        if request.path.endswith("/api/vsp/runs_v3"):
            ...
            if "application/json" in ct:
                ...
                data = json.loads(raw)
                data = _vsp_p4848_runs3_contract_payload(data)
                out = json.dumps(data, ensure_ascii=False)
                resp.set_data(out)
                resp.headers["Content-Length"] = str(len(resp.get_data() or b""))
                resp.headers["X-VSP-P4848-RUNS3"] = "1"
    except Exception:
        pass
    return resp
"""
...
print(f"[P4848] appvar={appvar} injected after_request middleware OK")
PY

# py_compile gate
if ! python3 -m py_compile "$APP" 2> "$OUT/py_compile.err"; then
  echo "[ERR] py_compile failed; showing error:" | tee -a "$OUT/log.txt"
  sed -n '1,160p' "$OUT/py_compile.err" | tee -a "$OUT/log.txt"
...
echo "== hdr marker ==" | tee -a "$OUT/log.txt"
grep -i "X-VSP-P4848-RUNS3" -n "$OUT/hdr.txt" | tee -a "$OUT/log.txt" || true
...
echo "[OK] P4848 done. Close /c/runs tab, reopen then Ctrl+Shift+R" | tee -a "$OUT/log.txt"
