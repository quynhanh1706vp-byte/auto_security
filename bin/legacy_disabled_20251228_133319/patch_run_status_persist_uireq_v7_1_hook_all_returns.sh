...
cp -f "$F" "$F.bak_persist_uireq_v7_1_${TS}"
echo "[BACKUP] $F.bak_persist_uireq_v7_1_${TS}"
...
MARK = "VSP_RUN_STATUS_PERSIST_UIREQ_V7_1_HOOK_ALL_RETURNS"
if MARK in txt:
...
def inject(mm):
    ...
    block = "\n".join([
        ...
        f"{ind}      else:",
        f"{ind}        sp = _P(_VSP_UIREQ_DIR) / (_req + '.json')",
