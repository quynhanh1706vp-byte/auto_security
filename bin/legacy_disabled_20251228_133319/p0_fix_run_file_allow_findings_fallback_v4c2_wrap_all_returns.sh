...
cp -f "$WSGI" "${WSGI}.bak_rfallow_findingsfb_v4c2_${TS}"
echo "[BACKUP] ${WSGI}.bak_rfallow_findingsfb_v4c2_${TS}"
...
MARK="VSP_P0_RUN_FILE_ALLOW_FINDINGS_FALLBACK_V4C2"
if MARK in s:
...
for j in range(def_i+1, len(lines)):
    ...
    # end when back to same indentation and a new def/decorator starts
...
# insert helper right after def line
blk_lines=block.splitlines(True)
blk_lines.insert(1, helper)
block2="".join(blk_lines)

# wrap ALL return jsonify(...) and return flask.jsonify(...)
...
block3, wrapped = ret_pat.subn(repl, block2)
if wrapped == 0:
...
print("[OK] patched:", MARK, "wrapped_returns=", wrapped)
PY
