...
cp -f "$W" "${W}.bak_p3k2_safe_${TS}"
echo "[BACKUP] ${W}.bak_p3k2_safe_${TS}"
        ...
        try:
            for name in os.listdir(root):
                ...
                for rel in ("reports/findings_unified.json","report/findings_unified.json","findings_unified.json",
                            "reports/findings_unified_commercial.json","report/findings_unified_commercial.json","findings_unified_commercial.json"):
...
def _base_for(rid: str):
    ...
    src, items, mtime = _load_items(rid)
    ...
    _CACHE[rid] = {"ts": now, "mtime": mtime, "base": base}
    return base
...
echo "== [2] patch gateway tiny hook (try/except, never kills service) =="
"$PY" - <<'PY'
...
echo "[DONE] p3k2_safe_dash_mw_external_v1"
