...
cp -f "$JS" "${JS}.bak_row_actions_v2fix_${TS}"
echo "[BACKUP] ${JS}.bak_row_actions_v2fix_${TS}"
...
def strip_block(text: str, marker: str) -> str:
    # Remove from "/* <marker>" to the next "})(); // IIFE" (or end)
...
# remove any older injected versions (V1 or half V2)
for m in [
...
/* VSP5_RUNS_REPORTS_ROW_ACTIONS_P1_V2
   ...
   - Stop spam: intercept fetch("<URL>") so DevTools won't show Fetch failed GET "<URL>"
...
;(()=>{
  ...
  // ---- intercept fetch("<URL>") (this is the noisy one shown in vsp5:390)
  (function interceptBadFetch(){
    if (window.__vsp_fetch_intercept_p1v2) return;
    window.__vsp_fetch_intercept_p1v2 = true;
    ...
    window.fetch = (input, init)=>{
      try{
        ...
        if (u === "<URL>"){
          const body = JSON.stringify({ ok:false, note:"intercepted <URL> placeholder", marker: MARK });
  ...
  function patchRows(){
    ...
    // hide any old extra TD from V1 (nếu còn)
...
print("[OK] wrote:", "VSP5_RUNS_REPORTS_ROW_ACTIONS_P1_V2")
PY
...
echo "[DONE] patch applied: VSP5_RUNS_REPORTS_ROW_ACTIONS_P1_V2 (fix)"
