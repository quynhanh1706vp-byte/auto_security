...
JS_ENH="static/js/vsp_dash_topfind_modal_v1.js"
TPL_DIR="templates"

mkdir -p static/js
cp -f "$JS_ENH" "${JS_ENH}.bak_${TS}" 2>/dev/null || true
echo "[BACKUP] ${JS_ENH}.bak_${TS} (if existed)"

cat > "$JS_ENH" <<'JS'
/* VSP_P1_DASH_TOPFIND_MODAL_DS_V1
   - Non-invasive dashboard enhancer:
     * hooks fetch to capture findings_unified.json (and other run_file_allow JSON)
  ...
  if(_fetch){
    window.fetch = async (...args)=>{
      ...
      try{
        ...
        if(url.includes("findings_unified.json")){
          const c = res.clone();
          c.json().then(j=>{
...
needle = "vsp_dash_only_v1.js"
inject_src = "/static/js/vsp_dash_topfind_modal_v1.js"
marker = "VSP_P1_DASH_TOPFIND_MODAL_DS_V1"
...
for p in tpl_dir.rglob("*.html"):
    ...
    if inject_src in t:
        continue
    # insert right after dash_only script tag line
    # handle both with and without ?v=
    t2, n = re.subn(r'(<script[^>]+vsp_dash_only_v1\.js[^>]*></script>\s*)',
                    r'\1<script src="'+inject_src+'"></script>\n',
                    t, count=1, flags=re.I)
    if n:
        p.write_text(t2, encoding="utf-8")
        patched += 1
        print("[OK] patched template:", p.name)
print("[INFO] templates patched:", patched)
PY

echo "[INFO] Restart service: $SVC"
systemctl restart "$SVC" 2>/dev/null || true

echo "== smoke: template contains enhancer js? =="
grep -R --line-number "vsp_dash_topfind_modal_v1\.js" templates | head -n 20 || true
