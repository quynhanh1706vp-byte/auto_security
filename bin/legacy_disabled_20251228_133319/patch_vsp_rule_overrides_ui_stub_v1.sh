...
cp "$APP_FILE" "${APP_FILE}.bak_rules_ui_stub_${TS}"
echo "[BACKUP] $APP_FILE -> ${APP_FILE}.bak_rules_ui_stub_${TS}"
...
if "/api/vsp/rule_overrides_ui_v1" in text:
    print("[INFO] Đã có /api/vsp/rule_overrides_ui_v1 trong vsp_demo_app.py – bỏ qua patch.")
else:
    block = '''
...
def vsp_rule_overrides_ui_v1():
    """UI-only storage cho rule_overrides_v1 (không đụng core).

    File lưu tại: ../config/rule_overrides_v1.json (tính từ thư mục ui/).
    """
    root = _VSP_Path_UI(__file__).resolve().parent.parent  # .../SECURITY_BUNDLE
    cfg_dir = root / "config"
    ...
    if request.method == "GET":
        if cfg_file.exists():
            try:
                data = _vsp_json_ui.loads(cfg_file.read_text(encoding="utf-8"))
    ...
    try:
        cfg_file.write_text(
            _vsp_json_ui.dumps(to_save, indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
    except Exception as exc:  # pragma: no cover
...
'''
    ...
    print("[OK] ĐÃ APPEND stub /api/vsp/rule_overrides_ui_v1 vào vsp_demo_app.py")
PY
