  ...
  function renderChart() {
    try {
      ...
      items.forEach(item => {
        ...
        if (item.value > 0 && pct < 12) pct = 12; // chiều cao tối thiểu cho giá trị >0
...
needle = "patch_sb_severity_chart.js"

if needle in data:
    print("[INFO] templates/index.html đã include patch_sb_severity_chart.js – bỏ qua.")
else:
    line = "    <script src=\"{{ url_for('static', filename='patch_sb_severity_chart.js') }}\"></script>\\n"
    idx = data.lower().rfind("</body>")
    if idx == -1:
        idx = data.lower().rfind("</html>")
    if idx == -1:
        print("[WARN] Không tìm thấy </body> hoặc </html> trong templates/index.html – không chèn được script.")
    else:
        new_data = data[:idx] + line + data[idx:]
        path.write_text(new_data, encoding="utf-8")
        print("[OK] Đã chèn script patch_sb_severity_chart.js vào templates/index.html")
PY

echo "[DONE] enable_sb_severity_chart_index_v2.sh hoàn thành."
