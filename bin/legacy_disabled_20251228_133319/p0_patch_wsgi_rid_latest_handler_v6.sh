...
cp -f "$W" "${W}.bak_rid_latest_v6_${TS}"
echo "[BACKUP] ${W}.bak_rid_latest_v6_${TS}"
...
MARK = "VSP_P0_WSGI_RID_LATEST_PICK_ANY_DETAILS_V6_HANDLER"
ROUTE = "/api/vsp/rid_latest_gate_root"

# Prefer the real Flask decorator route handler
# e.g. @app_obj.get("/api/vsp/rid_latest_gate_root")
m = re.search(r'^\s*@\s*([A-Za-z_][A-Za-z0-9_\.]*)\s*\.get\(\s*["\']' + re.escape(ROUTE) + r'["\']\s*\)\s*$',
              s, flags=re.M)
if not m:
    raise SystemExit("[ERR] cannot find decorator @*.get('/api/vsp/rid_latest_gate_root') in wsgi")
...
if not mdef:
    raise SystemExit("[ERR] cannot find def right after decorator for rid_latest")
func_name = mdef.group(1)

def_start = dec_pos + mdef.start() + 1
tail2 = s[def_start:]
mend = re.search(r'\n(?=@\w|\ndef\s+)', tail2)
...
print("[OK] patched handler:", MARK, "func=", func_name)
PY

systemctl restart "$SVC" 2>/dev/null || true

echo "== verify rid_latest_gate_root (expect reason/why + rid change) =="
