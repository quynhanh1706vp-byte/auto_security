...
TARGET="$UI_ROOT/api/vsp_run_export_api_v3.py"

if [[ ! -f "$TARGET" ]]; then
  echo "[WARN] Không tìm thấy $TARGET, sẽ tạo mới."
fi

BACKUP="${TARGET}.bak_reset_$(date +%Y%m%d_%H%M%S)"
if [[ -f "$TARGET" ]]; then
  cp "$TARGET" "$BACKUP"
...
echo "[OK] Đã reset api/vsp_run_export_api_v3.py với blueprint run_export_v3 chuẩn."
