...
marker_end   = "BY TOOL / CONFIG (đọc từ tool_config.json)"

i = html.find(marker_start)
j = html.find(marker_end)

if i == -1 or j == -1 or j <= i:
    print("[ERR] Không tìm thấy đủ marker DASHBOARD / BY TOOL trong index.html")
    sys.exit(1)

before = html[:i]
after  = html[j:]   # giữ nguyên phần BY TOOL / CONFIG trở đi

# Block Dashboard đẹp (giống hiện tại, nhưng KHÔNG để lại phần TREND cũ bên dưới)
...
print("[OK] Đã thay toàn bộ block Dashboard, bỏ phần TREND cũ.")
PY
