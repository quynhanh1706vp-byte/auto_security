...
echo "VSP UI XINXO AUDIT v1b  @ $(ts)"
echo "BASE=$BASE"
hr
...
if ep.endswith("/api/vsp/rid_latest"):
  ...
  if "points" not in j: status="WARN"; msg.append("missing points[]")
  ...
  if not good: status="WARN"; msg.append("missing download/package link/path")
...
for P in "${PAGES[@]}"; do
  ...
  echo "-- $P --"
  echo "HTML bytes=$(wc -c <"$fn" | tr -d ' ') | JS=$(wc -l <"$js" | tr -d ' ') | CSS=$(wc -l <"$css" | tr -d ' ')"
done
hr

echo "== [C] Duplicate asset detection (global, FYI) =="
sort "$ALL_ASSETS" | sed '/^$/d' | uniq -c | awk '$1>1{print}' | head -n 80 || true
hr

echo "== [C2] Duplicate assets per-page (real duplication) =="
for P in "${PAGES[@]}"; do
  fn="$TMP/page_$(echo "$P" | tr '/' '_' ).html"
  [ -f "$fn" ] || continue
  dups="$( (grep -oE '/static/(js|css)/[^"'\'' >]+' "$fn" | sed 's/[?].*$//' | sort | uniq -c | awk '$1>1{print $2}' ) | head -n 5 | tr '\n' ',' | sed 's/,$//' )"
  if [ -n "${dups:-}" ]; then
    echo "[DUP] $P => $dups"
  else
    echo "[OK]  $P => no per-page dup"
  fi
done
hr
...
for P in "${PAGES[@]}"; do
  ...
  notes=()
  grep -qiE 'TODO|FIXME|DEBUG' "$scan" && notes+=("has TODO/FIXME/DEBUG") || true
  grep -qiE 'N/A' "$scan" && notes+=("has N/A") || true
  grep -qiE 'data-testid' "$scan" && notes+=("has data-testid") || true
