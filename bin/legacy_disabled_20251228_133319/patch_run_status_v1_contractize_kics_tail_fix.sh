...
cp -f "$F" "$F.bak_contractize_kics_tail_fix_${TS}"
echo "[BACKUP] $F.bak_contractize_kics_tail_fix_${TS}"

export VSP_DEMO_APP_FILE="$F"

python3 - <<'PY'
import os, re, json
from pathlib import Path

p = Path(os.environ["VSP_DEMO_APP_FILE"])
...
# 1) helper funcs (NO f-string; include local imports so it never breaks)
...
def patch_contractize_whitelist(t):
  ...
  def ensure(inner):
    for k in ("kics_tail","_handler"):
      # triple-quote regex avoids quote escaping issues
