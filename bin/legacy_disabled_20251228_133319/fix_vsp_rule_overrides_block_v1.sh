...
cp "$APP_FILE" "${APP_FILE}.bak_rule_fix_${TS}"
echo "[BACKUP] $APP_FILE -> ${APP_FILE}.bak_rule_fix_${TS}"
...
# Ưu tiên cắt từ marker stub V2, nếu không có thì cắt từ dòng route
marker = "# === VSP Rule Overrides API stub V2"
idx = text.find(marker)
if idx == -1:
    marker = "@app.route(\"/api/vsp/rule_overrides_v1\""
    idx = text.find(marker)
    if idx == -1:
        print("[ERR] Không tìm thấy block rule_overrides_v1_api để thay thế.")
        raise SystemExit(1)

# Giữ phần trước block cũ
head = text[:idx].rstrip()

block = '''# === VSP Rule Overrides API stub V2 (fixed) ===
...
print("[OK] Đã thay block rule_overrides_v1_api bằng bản fixed.")
PY
