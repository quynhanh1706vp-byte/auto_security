...
need ss; need awk; need sed; need pkill; need nohup; need python3; need curl; need tail
need flock || { echo "[ERR] missing flock (util-linux)"; exit 2; }
...
echo "== kill all listeners on :8910 =="
...
echo "== start gunicorn single-owner :8910 =="
...
echo "[DONE] 8910 up (single-owner)"
