...
# Find an anchor near existing rid-change logic, else fallback to whole file.
...
if not m:
  print("[WARN] no location.reload() found => skip attaching soft-refresh core (still continue with toggle/noise).")
  raise SystemExit(0)
