...
MARK="VSP_P2_KPI_V4_SKIP_BEFORE_PICK_V1"

cp -f "$F" "${F}.bak_kpiskip_${TS}"
echo "[BACKUP] ${F}.bak_kpiskip_${TS}"
...
for i,l in enumerate(lines):
    if "_app_v4 = _vsp_pick_flask_app_v4()" in l:
        ...
        break
if anchor is None:
    print("[ERR] cannot find _app_v4 = _vsp_pick_flask_app_v4()")
    sys.exit(2)

# find nearest try: above anchor (within 30 lines)
try_i = None
for j in range(anchor, max(-1, anchor-30), -1):
    if re.match(r'^[ \t]*try:\s*(#.*)?$', lines[j].rstrip("\n")):
        try_i = j
        break
if try_i is None:
    print("[ERR] cannot find try: above KPI_V4 mount")
    sys.exit(3)

# find end marker line containing "/VSP_P0_RUNS_KPI_V4_TREND_SERVER_SIDE_V1"
end_i = None
for k in range(anchor, min(len(lines), anchor+120)):
...
# replace [try_i, end_i) keep end marker line intact
out = "".join(lines[:try_i]) + "".join(new_block) + "".join(lines[end_i:])
p.write_text(out, encoding="utf-8")

py_compile.compile(str(p), doraise=True)
print("[OK] patched KPI_V4 mount guard + py_compile OK")
...
echo "== KPI_V4 log tail (should be 'skipped', not 'Working outside...') =="
