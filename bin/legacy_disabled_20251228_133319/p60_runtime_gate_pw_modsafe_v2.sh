...
log "== [P60 v2] Runtime gate (Playwright, strict evidence) =="
log "UI=$UI"
log "BASE=$BASE"
log "EVID=$EVID"
...
for p in /vsp5 /runs /data_source /settings /rule_overrides; do
  ...
  log "[HTTP] $p => $code"
  [ "$code" = "200" ] || { log "[ERR] tab not 200: $p"; exit 2; }
done

# --- playwright must resolve from UI ---
node -e "require.resolve('playwright', {paths:[process.cwd()]});" >/dev/null 2>&1 \
  || { log "[ERR] playwright not resolvable from UI (install in ui/ with npm i -D playwright)"; exit 2; }
...
# strict: must have verdict
if [ ! -s "$EVID/verdict.json" ]; then
  log "[FAIL] verdict.json missing"
