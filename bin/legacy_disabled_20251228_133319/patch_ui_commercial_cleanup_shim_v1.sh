...
JS_OUT="static/js/vsp_ui_commercial_cleanup_v1.js"
...
if [ -z "$pick_tpl" ]; then
  pick_tpl="$(grep -RIl --include='*.html' -E "VersaSecure Platform|vsp_tabs_hash_router|vsp_dashboard" templates 2>/dev/null | head -n 1 || true)"
fi

if [ -z "$pick_tpl" ]; then
  echo "[WARN] Cannot find template to patch. Add manually:"
  echo '  <script src="/static/js/vsp_ui_commercial_cleanup_v1.js" defer></script>'
  exit 0
fi

cp -f "$pick_tpl" "$pick_tpl.bak_uiclean_${TS}"
echo "[BACKUP] $pick_tpl.bak_uiclean_${TS}"
...
tag = '<script src="/static/js/vsp_ui_commercial_cleanup_v1.js" defer></script>'
if tag in txt:
    print("[OK] template already includes UI cleanup shim -> no change")
    raise SystemExit(0)
...
echo "[DONE] Commercial cleanup shim installed."
