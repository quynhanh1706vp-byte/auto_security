...
export MARK="VSP_P0_RUN_FILE_ALLOW_MULTIROOT_ALWAYS200_V1C3"
...
try:
    ...
    if _app is None:
        ...
        def _vf_guess_mime(rel: str) -> str:
            rel=(rel or "").lower()
        ...
        def vsp_run_file_allow_multiroot_always200_v1c3():
            try:
                ...
                if not fpath.is_file():
                    return jsonify({"ok": False, "err": "missing file", "rid": rid, "path": rel, "run_dir": str(run_dir), "marker": "{MARK}"}), 200

                # size guard
                try:
                    sz=fpath.stat().st_size
                    if sz > 250*1024*1024:
                        return jsonify({"ok": False, "err": "file too large", "rid": rid, "path": rel, "size": sz, "marker": "{MARK}"}), 200
                except Exception:
                    pass

                resp=make_response(send_file(str(fpath), mimetype=_vf_guess_mime(rel), as_attachment=False))
        ...
        # Force-bind by url_map (works even if original mounted elsewhere)
        eps=[]
        try:
        ...
        if eps:
            for ep in eps:
                _app.view_functions[ep] = vsp_run_file_allow_multiroot_always200_v1c3
            print("[{MARK}] OK rebound existing endpoints:", eps)
        else:
            _app.add_url_rule("/api/vsp/run_file_allow", "vsp_run_file_allow_multiroot_always200_v1c3",
                              vsp_run_file_allow_multiroot_always200_v1c3, methods=["GET"])
            print("[{MARK}] OK added new rule endpoint=vsp_run_file_allow_multiroot_always200_v1c3")
...
echo "[DONE] Patched run_file_allow. Now verify."
