...
need python3; need date; need ls; need head; need sort; need grep; need sed; need curl
...
GOOD=""
mapfile -t BAKS < <(ls -1t "${F}.bak_"* 2>/dev/null || true)
if [ "${#BAKS[@]}" -eq 0 ]; then
  echo "[ERR] no backups found: ${F}.bak_*"
  exit 2
fi

python3 - <<'PY' >/tmp/vsp_rescue_pick.txt
...
GOOD="$(cat /tmp/vsp_rescue_pick.txt | tr -d '\n' || true)"
if [ -z "$GOOD" ]; then
  echo "[ERR] cannot find any backup that py_compile OK"
  echo "      You may need to inspect recent edits manually."
  exit 2
fi
echo "[GOOD] $GOOD"
...
# (A) findings_v2 must always return WSGI-iterable JSON
run_if_exists "bin/p0_fix_findings_v2_wsgijson_hardlock_v1.sh"
# (B) /api/vsp/runs enrich contract (rid_latest_gate_root + gate_source)
run_if_exists "bin/p1_runs_enrich_wsgi_mw_v6.sh"
# (C) /api/vsp/run_file_allow smart gate selection (prefer CI non-UNKNOWN)
run_if_exists "bin/p1_backend_run_file_allow_gate_smart_v3_prefer_ci_nonunknown.sh"
# (D) /api/vsp/open stub + runsmeta helpers (safe)
...
curl -sS "$BASE/api/vsp/open" | head -c 300; echo
