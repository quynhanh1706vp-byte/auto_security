...
need python3; need curl; need grep; need sed; need head; need date
...
# check no literal "\n" at file header (the exact bug you hit)
...
# Ensure the expected commercial bundle + luxe present
...
echo
echo "== [3] API smoke: run_file_allow findings_unified (wrapper ok=true) =="
RID="$(curl -fsS "$BASE/api/vsp/rid_latest" | python3 -c 'import sys,json; print(json.load(sys.stdin).get("rid",""))')"
[ -n "$RID" ] || err "rid_latest empty"
echo "RID=$RID"

# Use small limit to avoid huge output
...
meta=j.get("meta") or {}
print("meta.keys=", sorted(list(meta.keys()))[:15])
f=j.get("findings") or []
...
echo
echo "== [4] Optional: restart service (if systemd present) =="
