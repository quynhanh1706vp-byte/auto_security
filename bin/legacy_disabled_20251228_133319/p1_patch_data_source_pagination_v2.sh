...
if tpldir.exists():
    for p in tpldir.glob("*.html"):
        if "data_source" in p.name.lower():
            cands.append(p)
# fallback: grep content
if not cands and tpldir.exists():
    for p in tpldir.glob("*.html"):
        try:
            s = p.read_text(encoding="utf-8", errors="replace").lower()
        except Exception:
            continue
        if ("data source" in s or "data_source" in s or "datasource" in s) and ("findings" in s):
            cands.append(p)
...
[ -f "$TPL" ] || { echo "[ERR] TPL not found: $TPL"; exit 2; }

echo "[INFO] TPL=$TPL"
cp -f "$TPL" "${TPL}.bak_pagination_${TS}"
echo "[BACKUP] ${TPL}.bak_pagination_${TS}"

# 2) write JS (new) for pagination overlay (idempotent-ish)
JS_NEW="static/js/vsp_data_source_pagination_v1.js"
mkdir -p static/js

if [ -f "$JS_NEW" ]; then
  cp -f "$JS_NEW" "${JS_NEW}.bak_${TS}" || true
  echo "[BACKUP] ${JS_NEW}.bak_${TS}"
fi
...
print("[OK] injected into:", tpl_path)
PY
