...
APP="app.py"
cp "$APP" "$APP.bak_backend_$(date +%Y%m%d_%H%M%S)" || true
...
marker = "# === _SB_HELPER_RUN_INDEX_V2 start ==="
if marker in data:
    print("[INFO] helper already present, skip append.")
    raise SystemExit(0)

block = r"""
# === _SB_HELPER_RUN_INDEX_V2 start ===
from pathlib import Path
import json
...
"""
idx = data.find('if __name__ == "__main__":')
if idx != -1:
    data = data[:idx] + block + "\n\n" + data[idx:]
else:
    data = data + block
...
print("[OK] appended helper + routes at end of app.py")
PY
