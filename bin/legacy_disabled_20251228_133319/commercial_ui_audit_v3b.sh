...
warm(){
  for i in $(seq 1 80); do
    if curl -g -fsS --connect-timeout 2 --max-time 4 "$BASE/api/vsp/selfcheck_p0" >/dev/null 2>&1; then
...
fetch_hdr(){
  ...
  rm -f "$hdr" "$body" || true
  # -g to disable [] glob/range issues
  curl -g -sS --connect-timeout 2 --max-time 12 -D "$hdr" -o "$body" "$url" >/dev/null 2>&1 || true
  [ -f "$hdr" ] || : > "$hdr"
  [ -f "$body" ] || : > "$body"
}
...
pick_rid(){
  ...
  for i in $(seq 1 80); do
    ...
    local ct bytes
    ct="$(ct_of /tmp/_a3_runs.hdr)"
    bytes="$(wc -c </tmp/_a3_runs.bin 2>/dev/null || echo 0)"
...
PY
      ...
      if [ -n "${rid:-}" ]; then
        log "Picked RID=$rid"
        echo "$rid"   # IMPORTANT: only RID, no extra text
        return 0
      fi
    fi
    sleep 0.2
...
echo "== [P38] commercial_ui_audit_v3b =="
echo "BASE=$BASE"
...
for t in "${tabs[@]}"; do
  ...
  st="$(status_of /tmp/_a3_tab.hdr)"; st="${st:-000}"
...
for a in "${apis[@]}"; do
  ...
  st="$(status_of /tmp/_a3_api.hdr)"; st="${st:-000}"
...
grep -q '"ok"' /tmp/_a3_ro0.bin && green "RO GET ok" || red "RO GET not json"

curl -g -sS --connect-timeout 2 --max-time 12 -D /tmp/_a3_post.hdr \
  -H "Content-Type: application/json" -X POST --data "$payload" \
  "$BASE/api/vsp/rule_overrides_v1/" -o /tmp/_a3_post.bin || true
grep -q "$RID2" /tmp/_a3_post.bin && green "RO POST created" || red "RO POST failed"
...
PY
grep -q 'True' /tmp/_a3_ro1_chk.txt && green "RO GET contains created" || red "RO GET missing created"

curl -g -sS --connect-timeout 2 --max-time 12 -D /tmp/_a3_del.hdr \
  -H "Content-Type: application/json" -X DELETE \
  --data "$(printf '{"id":"%s"}' "$RID2")" \
  "$BASE/api/vsp/rule_overrides_v1/" -o /tmp/_a3_del.bin || true
grep -q "$RID2" /tmp/_a3_del.bin && green "RO DELETE ok" || red "RO DELETE failed"
...
PY
grep -q 'True' /tmp/_a3_ro2_chk.txt && green "RO removed persisted" || red "RO still exists"
...
[ "$b0" -lt 200000 ] && green "findings paging small (bytes=$b0)" || amber "findings too big (bytes=$b0)"
...
PY
grep -q '(5, 5, False)' /tmp/_a3_fcmp.txt && green "paging offset differs" || amber "paging offset suspicious: $(cat /tmp/_a3_fcmp.txt 2>/dev/null || true)"

fetch_hdr "$BASE/api/vsp/datasource_v2?rid=RID_DOES_NOT_EXIST_123" /tmp/_a3_dsbad.hdr /tmp/_a3_dsbad.bin
grep -q '"ok":false' /tmp/_a3_dsbad.bin && green "datasource rid missing => ok:false" || amber "datasource rid missing not ok:false"
...
for fmt in html pdf zip; do
  ...
  st="$(status_of /tmp/_a3_e.hdr)"; st="${st:-000}"
