...
backup="$APP.bak_metrics_top_v2_$(date +%Y%m%d_%H%M%S)"
cp "$APP" "$backup"
...
def _vsp_extract_cwe_top(f: dict):
    ...
    # 2) Field dạng 'cwes' hoặc list tương tự
    ...
    # 4) Tìm trong tags/list string có chứa "CWE-"
...
def _vsp_extract_module_top(f: dict):
    # Các key phổ biến cho module/dependency
    for key in (
        "dependency",
        "package",
        "package_name",
    ...
    # Một số tool gói trong extra/metadata
    for key in ("extra", "metadata", "details"):
        sub = f.get(key)
        if isinstance(sub, dict):
            for kk in (
...
def _vsp_accumulate_top_metrics(findings, risky_only: bool):
    by_tool = Counter()
    by_cwe = Counter()
    by_module = Counter()
...
def _vsp_compute_top_metrics_top(latest_run_id: str) -> dict:
    ...
    # Nếu CWE/module không có gì thì fallback dùng tất cả severity
...
def vsp_metrics_after_request_top_v1(response):
    ...
    for key in ("top_r risky_tool", "top_impacted_cwe", "top_vulnerable_module"):
...
print("[OK] Đã ghi lại block VSP_METRICS_TOP_V1 (V2).")
PY

echo "[DONE] patch_vsp_demo_app_metrics_top_v2.sh hoàn tất."
