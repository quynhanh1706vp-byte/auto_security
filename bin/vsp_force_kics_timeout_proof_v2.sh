...
RID="$(echo "$BODY" | jq -r '.request_id // .rid // empty' 2>/dev/null || true)"
if [ -z "${RID:-}" ]; then
...
for i in $(seq 1 240); do
  ...
  echo "[PROOF] poll#$i stage='$STAGE' reason='$REASON' ci_run_dir='$CI_DIR' final=$FINAL"
...
for i in $(seq 1 240); do
  echo "---- tick#$i ----"
  if [ -f "$KLOG" ]; then
    echo "[KICS] last 10 lines:"
    tail -n 10 "$KLOG" | sed 's/\r/\n/g' || true
  else
