...
echo "[PATCH] 1) Ghi lại static/js/vsp_settings_simple_v1.js (phiên bản cuối)"

cat > static/js/vsp_settings_simple_v1.js << 'JS'
...
echo "[PATCH] 2) Sửa tất cả script tag dùng Jinja -> đường dẫn static thuần"
...
for p in files:
    ...
    before = txt
    # Sửa src="{{ url_for('static', filename='js/vsp_settings_simple_v1.js') }}"
    txt = txt.replace(
        "{{ url_for('static', filename='js/vsp_settings_simple_v1.js') }}",
        "/static/js/vsp_settings_simple_v1.js"
    )
    # Nếu chưa từng chèn, thì thêm script trước </body>
    if "/static/js/vsp_settings_simple_v1.js" not in txt:
        if "</body>" in txt:
            txt = txt.replace(
                "</body>",
                '    <script src="/static/js/vsp_settings_simple_v1.js"></script>\\n</body>'
            )
            print("[OK] Append script tag vào", p)
        else:
            print("[WARN] Không thấy </body> trong", p)
    elif before != txt:
        print("[OK] Đã sửa script tag (Jinja -> static) trong", p)
    p.write_text(txt, encoding="utf-8")
PY

echo "[PATCH] DONE. Hãy Ctrl+Shift+R trên browser rồi xem log [VSP_SETTINGS_SIMPLE]."
