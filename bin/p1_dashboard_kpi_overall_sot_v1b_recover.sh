...
echo "== [0] detect broken injection (bash expanded \${sev} -> empty) =="
if grep -qE 'data-kpi=""|data-sev=""|\[data-kpi=""\]' "$JS"; then
  echo "[WARN] looks broken (empty selectors). Will restore from latest backup."
  BAK="$(ls -1t "${JS}.bak_kpi_overall_"* 2>/dev/null | head -n 1 || true)"
  [ -n "${BAK:-}" ] || { echo "[ERR] no backup found: ${JS}.bak_kpi_overall_*"; exit 3; }
  cp -f "$BAK" "$JS"
  echo "[RESTORE] $BAK -> $JS"
else
  echo "[OK] no obvious empty-selector breakage detected"
fi

TS="$(date +%Y%m%d_%H%M%S)"
cp -f "$JS" "${JS}.bak_kpi_overall_recover_${TS}"
echo "[BACKUP] ${JS}.bak_kpi_overall_recover_${TS}"
...
if MARK in s:
    ...
    inject = r'''
/* VSP_P1_DASHBOARD_KPI_OVERALL_SOT_V1 (commercial SOT: KPI=run_gate_summary.counts_total, chip=run_gate_summary.overall) */
(function(){
 ...
 /* /VSP_P1_DASHBOARD_KPI_OVERALL_SOT_V1 */
'''
    s = s.rstrip() + "\n\n" + inject + "\n"
...
echo "[DONE] recovered + applied KPI/overall SOT patch safely (quoted heredoc)."
