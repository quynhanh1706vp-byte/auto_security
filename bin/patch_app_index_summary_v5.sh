...
# Tìm decorator route "/" (dạng " hoặc ')
route_idx = code.find('@app.route("/")')
if route_idx == -1:
    route_idx = code.find("@app.route('/')")
if route_idx == -1:
    route_idx = code.find('@app.route("/", methods=')
if route_idx == -1:
    print("[ERR] Không tìm thấy route '/' trong app.py (không có @app.route('/')).")
    sys.exit(1)

# Tìm def index sau decorator
def_idx = code.find("def index", route_idx)
if def_idx == -1:
    print("[ERR] Tìm được @app.route('/') nhưng không thấy def index sau đó.")
    sys.exit(1)

start = route_idx  # bắt đầu thay từ decorator

# Kết thúc: tới route tiếp theo hoặc if __name__ == "__main__"
next_route = code.find("\n@app.route", def_idx)
if next_route == -1:
    next_route = code.find('\nif __name__ == "__main__"', def_idx)
if next_route == -1:
    next_route = len(code)
...
print("[OK] Đã thay thế index() bằng phiên bản v5 – log [INFO][INDEX].")
PY
