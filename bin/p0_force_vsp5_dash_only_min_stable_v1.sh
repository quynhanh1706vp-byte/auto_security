...
cp -f "$JS" "${JS}.bak_MIN_STABLE_${TS}"
echo "[BACKUP] ${JS}.bak_MIN_STABLE_${TS}"
...
echo "== verify served JS marker =="
curl -fsS "$BASE/static/js/vsp_dash_only_v1.js?cb=$(date +%s)" | head -n 3
echo
echo "[DONE] Ctrl+Shift+R /vsp5 -> you must see floating panel 'MIN_STABLE'."
