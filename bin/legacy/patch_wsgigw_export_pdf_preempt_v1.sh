...
[ -f "$F" ] || { echo "[ERR] missing $F (gunicorn uses wsgi_vsp_ui_gateway:application)"; exit 2; }

TS="$(date +%Y%m%d_%H%M%S)"
cp -f "$F" "$F.bak_exportpdf_preempt_${TS}"
echo "[BACKUP] $F.bak_exportpdf_preempt_${TS}"
...
TAG = "# === VSP_WSGI_EXPORT_PDF_PREEMPT_V1 ==="
if TAG in t:
...
p.write_text(t + "\n\n" + block + "\n", encoding="utf-8")
print("[OK] appended WSGI preempt block")
PY
