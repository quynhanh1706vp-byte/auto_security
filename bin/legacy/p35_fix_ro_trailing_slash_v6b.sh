...
cp -f "$W" "${W}.bak_p35_ro_slash_v6b_${TS}"
echo "[BACKUP] ${W}.bak_p35_ro_slash_v6b_${TS}"
...
if MARK not in s:
    raise SystemExit("[ERR] V5 wrapper not found in wsgi_vsp_ui_gateway.py")

# Match the exact two-line pattern inside V5 wrapper and replace with normalized variant
pat = re.compile(
    r'(?m)^(?P<indent>[ \t]*)path=\(environ\.get\("PATH_INFO"\)\s*or\s*""\)\s*$\n'
    r'(?P=indent)if\s+path\s+in\s+\("/api/vsp/rule_overrides_v1","/api/vsp/rule_overrides_ui_v1"\)\s*:\s*$'
)

m = pat.search(s)
if not m:
    # try reversed tuple order (just in case)
    pat2 = re.compile(
        r'(?m)^(?P<indent>[ \t]*)path=\(environ\.get\("PATH_INFO"\)\s*or\s*""\)\s*$\n'
        r'(?P=indent)if\s+path\s+in\s+\("/api/vsp/rule_overrides_ui_v1","/api/vsp/rule_overrides_v1"\)\s*:\s*$'
    )
    m = pat2.search(s)
    if not m:
        raise SystemExit("[ERR] could not locate rule_overrides path check in V5 wrapper (pattern mismatch)")
...
print("[OK] patched V5: normalize trailing slash for rule_overrides")
PY
...
echo "== [CHECK] both endpoints must be ok JSON =="
