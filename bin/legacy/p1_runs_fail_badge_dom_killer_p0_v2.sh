...
MARK = "__VSP_P0_RUNS_FAIL_BADGE_DOM_KILLER_V2"
...
for pp in paths:
  ...
  if "VSP_P0_RUNS_FAIL_BADGE_DOM_KILLER_V2" in s:
    print("[OK] already patched:", pp)
    continue
  bak = pp.with_name(pp.name + f".bak_runs_domkill_{time.strftime('%Y%m%d_%H%M%S')}")
  bak.write_text(s, encoding="utf-8")
  pp.write_text(s + PATCH, encoding="utf-8")
