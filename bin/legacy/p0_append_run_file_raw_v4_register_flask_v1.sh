...
cp -f "$WSGI" "${WSGI}.bak_rawv4_${TS}"
echo "[BACKUP] ${WSGI}.bak_rawv4_${TS}"
...
marker="VSP_P0_RUN_FILE_RAW_V4_REGISTER_FLASK_V1"
if marker in s:
...
print("[OK] appended raw v4 + flask autodetect")
PY
...
echo "RID=$RID"
curl -i -sS "$BASE/api/vsp/run_file_raw_v4?rid=$RID&path=run_gate_summary.json" | head -n 28
