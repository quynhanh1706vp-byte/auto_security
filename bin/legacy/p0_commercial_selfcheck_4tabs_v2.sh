...
echo "== VSP commercial selfcheck 4 tabs =="
echo "[INFO] BASE=$BASE"
...
# 2) Runs API + latest RID
RID="$(curl -sS "$BASE/api/vsp/runs?limit=1" \
  | python3 - <<'PY'
import sys, json
j=json.load(sys.stdin)
it=(j.get("items") or [None])[0] or {}
print(it.get("run_id") or it.get("rid") or "")
PY
)"
if [ -n "${RID:-}" ]; then ok "latest RID => $RID"; else err "cannot resolve latest RID from /api/vsp/runs"; fi
