...
def api_runs():
    \\"\\"
    Trả về danh sách RUN_* trong out/ cho tab Run & Report.
    Bỏ qua các RUN_DEMO / RUN_gitleaks..., không chết nếu summary_unified.json lỗi.
    \\"\\"
    ...
    # Lấy các thư mục RUN_* đúng pattern
    entries = []
    ...
    for run_dir in entries:
            ...
            try:
                ...
                if isinstance(summary, dict):
                    ...
                    if isinstance(buckets, dict):
                        ...
                        total = int(summary.get("total_findings", crit + high + med + low) or 0)
                        has_counts = True
        ...
        if not has_counts:
            # Không ép buộc, nhưng vẫn trả về RUN với số liệu 0 để UI hiển thị được.
...
print("[OK] Đã viết lại API /api/runs (safe v2).")
PY
