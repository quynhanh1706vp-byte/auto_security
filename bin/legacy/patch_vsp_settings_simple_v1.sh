...
echo "[PATCH] 1) Tạo static/js/vsp_settings_simple_v1.js"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Dù đang đứng tab nào cũng load trước; UI Settings mở ra là có sẵn value
  vspSimpLoad();
});
JS

echo "[PATCH] 2) Thêm script tag vào template chính"

python - << 'PY'
from pathlib import Path

tag = "{{ url_for('static', filename='js/vsp_settings_simple_v1.js') }}"
files = [
...
for p in files:
    ...
    if "vsp_settings_simple_v1.js" in txt:
    ...
    print("[OK] Đã chèn script vào", p)
PY

echo "[PATCH] DONE. Ctrl+Shift+R trên trình duyệt để reload."
