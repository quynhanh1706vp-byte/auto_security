...
html, body{
  background: radial-gradient(1100px 700px at 25% 10%, rgba(50,120,255,0.18), transparent 55%),
...
# 2) patch templates: inject override css link + fix bootjs src EXACTLY 1 query
python3 - <<PY
...
for p in tps:
    ...
    if mark not in s and "</head>" in s:
        ins = f'\\n<!-- {mark} -->\\n<link rel="stylesheet" href="{ovr}">\\n'
        s = s.replace("</head>", ins + "</head>", 1)

    # force exactly: /static/js/vsp_p1_page_boot_v1.js?v=<TS>
    s = re.sub(r"(/static/js/vsp_p1_page_boot_v1\\.js)\\?v=[^\\\"']*", r"\\1?v="+ts, s)
    # kill accidental second ?v=... (if any)
...
# 4) verify from server side (avoid browser cache)
