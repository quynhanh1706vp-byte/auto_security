...
# VSP_P2_TABS5_BUNDLE_GATE_V1C
# - Gate UI tabs5+bundle (OK)
# - Gate contract APIs using curl->stdin JSON parsing (no temp argv issues)
...
RID="$(python3 - <<'PY' <<<"$RID_JSON"
import sys, json
s=sys.stdin.read().strip()
j=json.loads(s)
rid=j.get("rid","") if isinstance(j,dict) else ""
print(rid)
PY
)"
...
echo "== [PASS] VSP_P2_TABS5_BUNDLE_GATE_V1C =="
