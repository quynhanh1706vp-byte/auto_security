...
BAK="${F}.bak_fix_overall_runsv3_v3_${TS}"
cp -f "$F" "$BAK"
echo "[BACKUP] $BAK"
...
MARK = "VSP_P1_FIX_OVERALL_RUNS_V3_INDENT_SAFE_V3"
if MARK in s:
...
# ---- insert helper after last import (safe place)
helper = f"""
# {MARK}
...
# ---- find handler function name for runs_v3
src = "".join(lines)
...
if not fn_name:
    # decorator style: find line containing /api/ui/runs_v3 then nearest def below
    idx = None
    for i, ln in enumerate(lines):
        if "/api/ui/runs_v3" in ln and "route" in ln:
            idx = i
            break
    if idx is not None:
...
if not fn_name:
    # fallback: any def containing runs_v3
    for ln in lines:
...
if not fn_name:
    print("[ERR] cannot determine handler function for /api/ui/runs_v3")
    raise SystemExit(2)
...
for k in range(def_line+1, len(lines)):
    ...
    # end of function when indentation returns to <= def_indent and line starts new top-level block/decorator/def/class
...
# ---- patch first return of jsonify/__vsp__json within this function
patched = False
for i, ln in enumerate(body):
    mret = re.match(r'^(\s*)return\s+([A-Za-z_]\w*)\(\s*(.+?)\s*\)\s*$', ln)
    if not mret:
        continue
    indent, callee, expr = mret.group(1), mret.group(2), mret.group(3)
    if callee not in ("jsonify", "__vsp__json", "__vsp_json", "__json", "__vsp__ok"):
        continue
    # replace return line safely (evaluate expr once)
    hook = (
        f"{indent}# {MARK}_HOOK\n"
        f"{indent}try:\n"
        f"{indent}    __payload = {expr}\n"
        f"{indent}    if isinstance(__payload, dict) and isinstance(__payload.get('items'), list):\n"
        f"{indent}        _vsp_apply_overall_inference_on_payload(__payload)\n"
        f"{indent}except Exception:\n"
        f"{indent}    __payload = {expr}\n"
        f"{indent}return {callee}(__payload)\n"
    )
    body[i] = hook
    patched = True
    break
...
if not patched:
    print("[ERR] no suitable return jsonify/__vsp__json(...) found inside", fn_name)
    raise SystemExit(4)

# write back
lines[def_line:end] = body
p.write_text("".join(lines), encoding="utf-8")
print("[OK] patched file:", p)
PY

# transactional compile: if fail -> restore backup immediately
