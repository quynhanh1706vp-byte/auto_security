...
JS_OUT="static/js/vsp_runs_tab_resolved_v1.js"
mkdir -p "$(dirname "$JS_OUT")" bin

# --- 1) Write new Runs tab renderer (resolved endpoint) ---
TS="$(date +%Y%m%d_%H%M%S)"
if [ -f "$JS_OUT" ]; then
  cp -f "$JS_OUT" "$JS_OUT.bak_${TS}"
  echo "[BACKUP] $JS_OUT.bak_${TS}"
fi

cat > "$JS_OUT" <<'JS'
/* vsp_runs_tab_resolved_v1.js
 * Commercial wiring: Runs tab uses /api/vsp/runs_index_v3_fs_resolved
...
(function () {
  ...
  var ENDPOINT = '/api/vsp/runs_index_v3_fs_resolved?limit=50&hide_empty=0&filter=1';
...
echo "[OK] wrote $JS_OUT"

# --- 2) Patch template to include the new JS (defer) ---
# Prefer vsp_dashboard_2025.html, otherwise patch the first template that looks like the main VSP UI.
T_CANDIDATES=(
...
pick_tpl=""
for t in "${T_CANDIDATES[@]}"; do
  if [ -f "$t" ]; then pick_tpl="$t"; break; fi
done
if [ -z "$pick_tpl" ]; then
  # fallback: pick any template that mentions vsp_runs_tab
  pick_tpl="$(grep -RIl --include='*.html' -E "vsp_runs_tab|runs_index_v3_fs|Runs & Reports" templates 2>/dev/null | head -n 1 || true)"
fi

if [ -z "$pick_tpl" ]; then
  echo "[WARN] Cannot find template to patch under templates/. You must add this tag manually:"
  echo '  <script src="/static/js/vsp_runs_tab_resolved_v1.js" defer></script>'
  exit 0
fi

cp -f "$pick_tpl" "$pick_tpl.bak_runs_resolved_${TS}"
echo "[BACKUP] $pick_tpl.bak_runs_resolved_${TS}"
...
tag = '<script src="/static/js/vsp_runs_tab_resolved_v1.js" defer></script>'
if tag in txt:
    print("[OK] template already includes vsp_runs_tab_resolved_v1.js -> no change")
    raise SystemExit(0)

# Insert near other JS tags; prefer before </body>, else before </head>, else append.
if re.search(r"</body\s*>", txt, flags=re.I):
    txt = re.sub(r"(</body\s*>)", tag + r"\n\1", txt, flags=re.I, count=1)
elif re.search(r"</head\s*>", txt, flags=re.I):
    txt = re.sub(r"(</head\s*>)", tag + r"\n\1", txt, flags=re.I, count=1)
else:
    txt += "\n" + tag + "\n"
...
echo "[DONE] Runs tab resolved wiring installed."
echo "Next: restart UI service (or reload gunicorn) then open UI -> Runs tab."
