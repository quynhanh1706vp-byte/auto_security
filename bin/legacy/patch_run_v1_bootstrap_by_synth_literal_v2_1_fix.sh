...
cp -f "$F" "$F.bak_synth_literal_bootstrap_v2_1_${TS}"
echo "[BACKUP] $F.bak_synth_literal_bootstrap_v2_1_${TS}"
...
MARK = "VSP_BOOTSTRAP_BY_SYNTH_LITERAL_V2_1_FIX"
if MARK in txt:
...
# Find literal '"synthetic_req_id": True' inside run_v1
...
open_brace_pos = assign.end() - 1  # points to '{'

# Brace match that dict
s = fn
depth = 0
i = open_brace_pos
end_pos = -1
...
@@IND@@try:
@@IND@@  try:
@@IND@@    _req_payload = request.get_json(silent=True) or {}
@@IND@@  except Exception:
@@IND@@    _req_payload = {}
@@IND@@  _rid = ""
@@IND@@  try:
@@IND@@    _rid = str((@@VAR@@ or {}).get("request_id") or "")
@@IND@@  except Exception:
@@IND@@    _rid = ""
@@IND@@  if _rid:
@@IND@@    _vsp_write_uireq_state_v1(_rid, _req_payload)
@@IND@@except Exception:
@@IND@@  pass
@@IND@@# === END @@MARK@@ ===
'''
inject = (inject_tpl
          .replace("@@IND@@", var_indent)
