...
BACKUP="${TARGET}.bak_run_export_$(date +%Y%m%d_%H%M%S)"
cp "$TARGET" "$BACKUP"
...
new_txt = before + block + "\n\n" + after
target.write_text(new_txt, encoding="utf-8")
print("[PATCH] Đã chèn route /api/vsp/run_export_v3 vào", target)
PY

echo "[OK] Đã patch xong vsp_demo_app.py"
