...
[ -f "$SHIM" ] || { echo "[ERR] missing $SHIM (run v15 first)"; exit 2; }

TS="$(date +%Y%m%d_%H%M%S)"
cp -f "$SHIM" "${SHIM}.bak_timing_${TS}"
echo "[BACKUP] ${SHIM}.bak_timing_${TS}"

python3 - <<'PY'
from pathlib import Path
import re, textwrap

p = Path("static/js/vsp_p0_fetch_shim_v1.js")
s = p.read_text(encoding="utf-8", errors="replace")

# Replace whole file with v2 (keep same name to avoid touching WSGI)
out = textwrap.dedent(r"""
...
print("[OK] wrote shim V2")
PY

systemctl restart "$SVC" >/dev/null 2>&1 || true
echo "== smoke /vsp5 has shim =="
curl -fsS "$BASE/vsp5" | grep -n "vsp_p0_fetch_shim_v1.js" | head -n 5 || true
echo "[DONE] Ctrl+Shift+R /vsp5 and look for overlay top-right + console: VSP_P0_FETCH_SHIM_V2"
