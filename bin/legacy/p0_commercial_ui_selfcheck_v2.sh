...
need curl; need python3; need grep; need sed; need wc; need head; need date
...
echo
echo "== [2] Marker checks in key JS =="
check_js_marker(){
  local file="$1" pat="$2"
  if curl -sS "$BASE/$file?cb=$(date +%s)" | grep -qE "$pat"; then ok "$file has $pat"
  else err "$file missing $pat"; fi
}

# common markers (tabs3)
