...
export VSP_CIO_VER="cio_${TS}"

need(){ command -v "$1" >/dev/null 2>&1 || { echo "[ERR] missing: $1"; exit 2; }; }
need date; need python3; need curl; need head

[ -f "$WSGI" ] || { echo "[ERR] missing $WSGI"; exit 2; }

cp -f "$WSGI" "${WSGI}.bak_inject_cio_vsp5_mw_${TS}"
echo "[BACKUP] ${WSGI}.bak_inject_cio_vsp5_mw_${TS}"

"$PY" - <<'PY'
from pathlib import Path
import os, re, time, py_compile

ver = os.environ.get("VSP_CIO_VER","cio")
p = Path("wsgi_vsp_ui_gateway.py")
s = p.read_text(encoding="utf-8", errors="replace")

# --- remove previous auto blocks (v1/v2) to keep file clean ---
for m1,m2 in [
  ("### === CIO INJECT SHELL VSP5 ONLY (AUTO) ===", "### === END CIO INJECT SHELL VSP5 ONLY (AUTO) ==="),
]:
    if m1 in s and m2 in s:
        s = re.sub(rf"{re.escape(m1)}.*?{re.escape(m2)}\n?", "", s, flags=re.S)

MARK="### === CIO INJECT SHELL VSP5 ONLY (MW V3) ==="
END ="### === END CIO INJECT SHELL VSP5 ONLY (MW V3) ==="
...
def _cio_inject_shell_into_vsp5_html(app):
    import re
    css = b'<link rel="stylesheet" href="/static/css/vsp_cio_shell_v1.css?v={ver}"/>'
    js  = b'<script defer src="/static/js/vsp_cio_shell_apply_v1.js?v={ver}"></script>'
...
# Attach to Flask app middleware (gunicorn usually serves module:app)
def _cio_attach_mw():
    try:
        g=globals()
        if "app" in g and hasattr(g["app"], "wsgi_app"):
            g["app"].wsgi_app = _cio_inject_shell_into_vsp5_html(g["app"].wsgi_app)
    except Exception:
        pass
    try:
        g=globals()
        if "application" in g and callable(g["application"]):
            g["application"] = _cio_inject_shell_into_vsp5_html(g["application"])
    except Exception:
        pass
...
bak = p.with_name(p.name + f".bak_injectcio_mw_{time.strftime('%Y%m%d_%H%M%S')}")
bak.write_text(p.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
p.write_text(s, encoding="utf-8")

py_compile.compile(str(p), doraise=True)
print("[OK] patched middleware attach; ver=", ver, "backup:", bak.name)
PY
