...
need python3; need mkdir; need date; need curl

BASE="${VSP_UI_BASE:-http://127.0.0.1:8910}"

echo "== [1] install synth script =="
cat > bin/vsp_report_synth_v1.py <<'PY'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
PY
chmod +x bin/vsp_report_synth_v1.py
python3 -m py_compile bin/vsp_report_synth_v1.py
echo "[OK] synth script ready"

echo "== [2] detect latest RID + run_dir via manifest =="
...
[ -n "$RUN_DIR" ] || { echo "[ERR] cannot resolve run_dir from manifest"; exit 2; }

echo "== [3] synth reports (create missing) =="
python3 bin/vsp_report_synth_v1.py --run-dir "$RUN_DIR" --title "VSP Findings Report • $RID" | python3 -c 'import sys,json; j=json.load(sys.stdin); print("created=",len(j.get("created") or []),"degraded=",j.get("degraded") or [])'

echo "== [4] re-check manifest (lite) =="
...
echo "[DONE] Reload /runs to see badge improve."
