...
cp -f "$W" "${W}.bak_before_rewrite_v2b_${TS}"
echo "[SNAPSHOT] ${W}.bak_before_rewrite_v2b_${TS}"
...
# 0) if current broken, restore newest compiling backup
if not compiles(W):
    ...
    for p in baks:
        if compiles(p):
            ...
            break
    if not good:
        raise SystemExit("[FATAL] No compiling backup found")
    ...
    print("[OK] gateway already compiles")
...
if START not in s or END not in s:
    raise SystemExit("[FATAL] injector START/END markers missing")

# 1) rewrite the whole block (NO f-string here; safe braces)
new_block = textwrap.dedent("""
{START}
# NOTE: rewritten V2B (compile-safe, no Jinja remnants). Inject autorid ONLY for 4 tabs, never dashboard, never /reports (json).
...
if _VSP_GATEWAY_APP and _vsp_request:
    ...
    def _vsp_gateway_after_request_tabs4_inject_autorid(resp):
        ...
        # Sanitize any broken src like ?v={{ asset_v|default(...) }} or truncated ?v={ asset_v|default(
        try:
        ...
        # If already present, still set_data (in case sanitized)
        ...
        return resp
{END}
""").strip().replace("{START}", START).replace("{END}", END) + "\n"

pattern = re.compile(re.escape(START) + r".*?" + re.escape(END), re.S)
s2, n = pattern.subn(new_block, s)
if n != 1:
    raise SystemExit(f"[FATAL] rewrite failed, replacements={n}")

W.write_text(s2, encoding="utf-8")
py_compile.compile(str(W), doraise=True)
print("[OK] gateway injector rewritten V2B + compiles")

# 2) clean templates: remove any autorid tag that uses braces in query v=...
tpl_dir = Path("templates")
pat = re.compile(r'(?is)\s*<!--\s*VSP_P1_TABS4_AUTORID_NODASH_V1\s*-->\s*<script[^>]*vsp_tabs4_autorid_v1\\.js\\?v=[^">]*\\{[^">]*[^>]*></script>\s*')
cleaned = 0
...
print("[INFO] total template cleaned blocks:", cleaned)
PY

echo "[INFO] Restart service: $SVC"
systemctl restart "$SVC" 2>/dev/null || true

echo "== verify /runs autorid src (must be numeric v, no braces) =="
