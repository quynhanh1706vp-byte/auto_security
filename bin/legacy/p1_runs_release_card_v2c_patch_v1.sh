...
cp -f "$JS" "${JS}.bak_release_v2c_${TS}"
echo "[BACKUP] ${JS}.bak_release_v2c_${TS}"
...
old_marker = "VSP_P1_RELEASE_CARD_V2_RUNS_ONLY_V2"
new_marker = "VSP_P1_RELEASE_CARD_V2C_RUNS_ONLY_V1"
...
if idx != -1:
    ...
    if end == -1:
        raise SystemExit("[ERR] cannot locate end of old release card block")
    end2 = s.find("\n", end)
    if end2 == -1:
        end2 = end + len("})();")
    else:
        end2 = end2 + 1
    s2 = s[:idx] + new_block.strip() + "\n" + s[end2:]
    js.write_text(s2, encoding="utf-8")
    print("[OK] replaced old block:", old_marker, "=>", new_marker)
else:
    # append new if old not found
    js.write_text(s.rstrip() + "\n\n" + new_block.strip() + "\n", encoding="utf-8")
    print("[OK] appended new block:", new_marker)
PY
...
echo "[DONE] release card v2c patched"
