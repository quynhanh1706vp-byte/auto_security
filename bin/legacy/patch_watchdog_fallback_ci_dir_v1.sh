...
cp -f "$WD" "$WD.bak_ci_fallback_$(date +%Y%m%d_%H%M%S)"
echo "[BACKUP] $WD.bak_ci_fallback_*"
...
def _pick_latest_ci_dir(out_ci: Path, start_ts: int) -> str:
    cand=[]
    if not out_ci.exists(): return ""
    for d in out_ci.glob("VSP_CI_*"):
        try:
            mt=int(d.stat().st_mtime)
            if mt >= start_ts - 3600:  # within 60m
...
def _fallback_scan_home_test_data(start_ts: int) -> str:
    """
    Commercial fallback when target is empty and /proc/pid/cwd not usable:
    scan /home/test/Data/*/out_ci/VSP_CI_* and pick newest within 60m.
    """
    ...
    for sub in base.iterdir():
        try:
            ...
            if not out_ci.exists():
                continue
            d = _pick_latest_ci_dir(out_ci, start_ts)
            if not d:
                continue
...
def main():
    ...
    while True:
        ...
        # Always record watchdog pid
        st["watchdog_pid"] = int(os.getpid())

        # Infer CI root & CI dir
        if not st.get("ci_run_dir"):
            root = _infer_root_from_pid(pid)
            st["ci_root_from_pid"] = root or ""
            if root:
                st["ci_run_dir"] = _pick_latest_ci_dir(Path(root)/"out_ci", start_ts) or ""
            if not st.get("ci_run_dir"):
                # fallback: use target if present
                target = str(st.get("target") or "")
                if target:
                    st["ci_run_dir"] = _pick_latest_ci_dir(Path(target)/"out_ci", start_ts) or ""
            if not st.get("ci_run_dir"):
                # last resort: scan /home/test/Data/*/out_ci
...
echo "[OK] py_compile watchdog OK"
