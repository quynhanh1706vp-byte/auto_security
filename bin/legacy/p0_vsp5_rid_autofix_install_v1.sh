...
JSFIX="static/js/vsp_rid_autofix_v1.js"

[ -f "$WSGI" ] || { echo "[ERR] missing $WSGI"; exit 2; }

cp -f "$WSGI" "${WSGI}.bak_ridautofix_${TS}"
echo "[BACKUP] ${WSGI}.bak_ridautofix_${TS}"
...
if [ "$node_ok" = "1" ]; then
  node --check "$JSFIX" >/dev/null && echo "[OK] node --check $JSFIX"
else
...
# find gate story include line (keep whatever placeholder it uses)
m = re.search(r'(<script\s+src="\/static\/js\/vsp_dashboard_gate_story_v1\.js\?v=([^"]+)"><\/script>)', s)
if not m:
    raise SystemExit("[ERR] cannot find gate_story include in WSGI")

gate_line = m.group(1)
placeholder = m.group(2)  # e.g. {asset_v} or {{ asset_v }} or timestamp
fix_line = f'  <script src="/static/js/vsp_rid_autofix_v1.js?v={placeholder}"></script>'

if "vsp_rid_autofix_v1.js" in s:
    print("[OK] WSGI already includes rid autofix")
else:
    s = s.replace(gate_line, fix_line + "\n  " + gate_line, 1)
    w.write_text(s, encoding="utf-8")
    print("[OK] inserted rid autofix before gate_story include")
PY

python3 -m py_compile "$WSGI" >/dev/null && echo "[OK] py_compile WSGI OK"

echo
echo "[DONE] Now do:"
echo "  1) systemctl restart vsp-ui-8910.service"
echo "  2) Ctrl+Shift+R  http://127.0.0.1:8910/vsp5"
echo
