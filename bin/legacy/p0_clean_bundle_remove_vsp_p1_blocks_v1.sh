...
cp -f "$JS" "${JS}.bak_clean_${TS}"
echo "[BACKUP] ${JS}.bak_clean_${TS}"
...
# remove ONLY blocks that we injected in this conversation (safe list)
markers = [
...
for m in markers:
    # /* ==== m ==== */ ... /* ==== /m ==== */
    pat = re.compile(r"/\*\s*=+\s*"+re.escape(m)+r"\s*=+.*?\*/\s*(?:.|\n)*?/\*\s*=+\s*/"+re.escape(m)+r"\s*=+.*?\*/\s*", re.MULTILINE)
    s2, n = pat.subn("", s)
    if n:
        n_total += n
        s = s2
