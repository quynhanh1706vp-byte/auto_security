...
# find a data_source template to patch
TPL=""
if [ -d "$TPL_DIR" ]; then
  # strongest signal: template contains '/data_source'
...
if [ -z "${TPL:-}" ]; then
  ...
  echo "[HINT] If your template name is known, patch manually by adding chart strip + script include."
  exit 0
fi

cp -f "$TPL" "${TPL}.bak_p2_ds_strip_${TS}"
echo "[BACKUP] ${TPL}.bak_p2_ds_strip_${TS}"
...
if "VSP_P2_DATA_SOURCE_STRIP_V1" not in s:
    ...
    # Insert near top (after <main> if exists)
    if "<main" in s:
...
if "vsp_data_source_charts_v1.js" not in s:
    s = re.sub(r"(?is)</body>", r'\n<script src="/static/js/vsp_data_source_charts_v1.js"></script>\n</body>', s, count=1) or (s + '\n<script src="/static/js/vsp_data_source_charts_v1.js"></script>\n')
tpl_path.write_text(s, encoding="utf-8")
print("[OK] patched template:", tpl_path)
else:
print("[OK] template already patched:", tpl_path)
PY

node --check "$JS_NEW" && echo "[OK] node --check OK"
echo "[DONE] p2_data_source_charts_strip_v1"
