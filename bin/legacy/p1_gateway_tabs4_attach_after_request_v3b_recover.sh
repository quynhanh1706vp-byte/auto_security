...
# 1) restore newest compiling backup
...
print("[OK] restored compiling gateway from:", good.name)

s = W.read_text(encoding="utf-8", errors="replace")

MARK = "VSP_P1_GATEWAY_TABS4_ATTACH_AFTER_REQUEST_V3B"
if MARK in s:
...
echo "== verify: header + src for 4 tabs =="
