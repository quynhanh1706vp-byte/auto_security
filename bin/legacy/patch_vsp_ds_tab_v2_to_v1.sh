...
if [ -f "$FILE_V1" ]; then
  BAK="${FILE_V1}.bak_use_v2_$(date +%Y%m%d_%H%M%S)"
  cp "$FILE_V1" "$BAK"
  echo "[PATCH_DS_V2] Backup $FILE_V1 -> $BAK"
fi
...
echo "[PATCH_DS_V2] Applied V2 datasource logic to vsp_datasource_tab_simple_v1.js"
JS
