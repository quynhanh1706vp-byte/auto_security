...
/* VSP_P1_RUNS_QUICK_ACTIONS_V1F (KPI + Pagination + Overall Resolve + Hide Legacy) */
(()=> {
  if (window.__vsp_p1_runs_quick_actions_v1f) return;
  window.__vsp_p1_runs_quick_actions_v1f = true;

  const log = (...a)=>console.log("[RunsQuickV1F]", ...a);
  const warn = (...a)=>console.warn("[RunsQuickV1F]", ...a);
  ...
  // ---- Overall cache resolved from run_gate.json ----
  ...
  function hideLegacyExcept(anchor){
    ...
    // find any element whose text matches legacy header, then hide its container + following siblings
    ...
    for (const h of nodes){
      if (anchor && anchor.contains(h)) continue;
      // climb to a container that has a table inside
      let cur = h;
      let picked = null;
      for (let i=0;i<10;i++){
        if (!cur || cur===document.body) break;
        const hasTable = cur.querySelector && cur.querySelector("table");
        if (hasTable){ picked = cur; break; }
        cur = cur.parentElement;
      }
      picked = picked || h.parentElement || h;
      hideNode(picked);

      // also hide next siblings (legacy block often spans multiple siblings)
      let sib = picked.nextElementSibling;
      for (let j=0;j<6 && sib; j++){
        if (anchor && anchor.contains(sib)) break;
        // stop if we hit our quick-actions table
        if (sib.querySelector && sib.querySelector("#vspRunsQuickActionsV1")) break;
        // stop if sib is empty
        hideNode(sib);
        sib = sib.nextElementSibling;
      }
    }

    // fallback: hide biggest table not inside anchor
    const tables = Array.from(document.querySelectorAll("table")).filter(t => !(anchor && anchor.contains(t)));
    if (tables.length){
      let best = tables[0], bestScore = 0;
      for (const t of tables){
        const score = (t.querySelectorAll("tr").length || 0);
        if (score > bestScore){ bestScore = score; best = t; }
      }
      let cur = best;
      for (let i=0;i<6;i++){
        if (!cur || cur===document.body) break;
        if (cur.parentElement && cur.parentElement!==document.body){
          hideNode(cur);
          break;
        }
        cur = cur.parentElement;
  ...
  function mount(){
    ...
    const hidden = hideLegacyExcept(anchor);
    log("legacy hidden nodes:", hidden.length);
    ...
    // KPI strip
    const kTotal=el("span",{class:"v"} ,["0"]);
    const kGreen=el("span",{class:"v"} ,["0"]);
    const kAmber=el("span",{class:"v"} ,["0"]);
    const kRed=el("span",{class:"v"} ,["0"]);
    const kUnk=el("span",{class:"v"} ,["0"]);
    const kDeg=el("span",{class:"v"} ,["0"]);

    const kpis = el("div",{class:"vsp-kpis"},[
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["Total"]), kTotal]),
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["GREEN"]), kGreen]),
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["AMBER"]), kAmber]),
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["RED"]), kRed]),
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["UNKNOWN"]), kUnk]),
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["DEGRADED"]), kDeg]),
      el("div",{class:"vsp-kpi"},[el("span",{class:"k"},["Cache"]), el("span",{class:"v", id:"vspRunsCacheN"},["0"])]),
    ]);
    ...
    const wrap=el("div",{class:"vsp-runsqa-wrap"},[
      el("div",{class:"vsp-runsqa-mini"},["Runs & Reports Quick Actions V1F: KPI + pagination + resolve Overall via run_gate.json (safe, cached)."]),
      kpis,
    ...
    function recomputeKpi(allItems){
      ...
      kTotal.textContent = String(allItems.length);
      kGreen.textContent = String(g);
      kAmber.textContent = String(a);
      kRed.textContent   = String(r);
      kUnk.textContent   = String(u);
      kDeg.textContent   = String(d);
      const cacheN = qs("#vspRunsCacheN");
      if (cacheN) cacheN.textContent = String(overallCache.size);
    }
    ...
    function render(){
      ...
      // resolve UNKNOWN for visible page (background)
      for (const run of items){
        const rid = pickRid(run);
        if (!rid) continue;
        if (String(pickOverall(run)).toUpperCase() !== "UNKNOWN") continue;
        scheduleResolve(rid, ()=>{ recomputeKpi(itemsAll); render(); });
      }
      ...
      stat.textContent = `Showing ${items.length}/${itemsAll.length} (total runs=${runsCache.length}) | cache=${overallCache.size}`;
...
print("[OK] wrote V1F =>", js)
PY

echo "[DONE] Patched JS to V1F. Open /runs and Ctrl+Shift+R."
