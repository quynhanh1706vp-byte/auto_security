...
need python3; need ls; need head; need date; need curl
...
MARK = "VSP_P1_RULE_OVERRIDES_PERSIST_AUDIT_RO_MODE_V3"
if MARK in s:
...
def vsp_rule_overrides_v1():
    ...
    if request.method == "GET":
        data = _vsp_rule_ovr_load_v3()
        _vsp_rule_ovr_audit_v3(request, "get_rule_overrides", {"items_len": len(data.get("items") or [])})
        return jsonify({"ok": True, "ro_mode": _vsp_ro_mode_v3(), "data": data})

    if _vsp_ro_mode_v3():
        _vsp_rule_ovr_audit_v3(request, "put_rule_overrides_denied_ro_mode")
        return jsonify({"ok": False, "error": "ro_mode", "msg": "Read-only mode enabled"}), 403

    payload = request.get_json(silent=True)
    ok, err = _vsp_rule_ovr_validate_v3(payload)
    if not ok:
        _vsp_rule_ovr_audit_v3(request, "put_rule_overrides_rejected", {"reason": err})
        return jsonify({"ok": False, "error": "invalid_payload", "reason": err}), 400

    _vsp_rule_ovr_save_v3(payload)
    _vsp_rule_ovr_audit_v3(request, "put_rule_overrides_ok", {"items_len": len((payload or {}).get("items") or [])})
    return jsonify({"ok": True, "saved": True, "ro_mode": _vsp_ro_mode_v3()})
# =================== /VSP_P1_RULE_OVERRIDES_PERSIST_AUDIT_RO_MODE_V3 ======================
'''.strip("\n")

def end_of_flask_call(text: str, line_start: int) -> int | None:
    # find "Flask(" after this line_start
    m = re.search(r'Flask\s*\(', text[line_start:])
    if not m:
        return None
    i = line_start + m.end() - 1  # at '('
    depth = 0
    in_str = None
    esc = False
    while i < len(text):
...
def patch_after_app_flask(full: str, region_start: int = 0) -> str | None:
    # find "app = Flask(" from region_start
    m = re.search(r'(?m)^[ \t]*app\s*=\s*Flask\s*\(', full[region_start:])
    if not m:
        return None
    app_line_start = region_start + m.start()
    # indent = leading spaces of the app= line
    indent = re.match(r'(?m)^([ \t]*)app\s*=\s*Flask', full[app_line_start:]).group(1)
    endpos = end_of_flask_call(full, app_line_start)
    if endpos is None:
        return None
    blk = indent_block(BLOCK, indent)
    return full[:endpos] + blk + full[endpos:]

patched = None

# Prefer create_app scope
m_create = re.search(r'(?m)^([ \t]*)def\s+create_app\s*\(', s)
if m_create:
    patched = patch_after_app_flask(s, region_start=m_create.end())

# Fallback global
if patched is None:
    patched = patch_after_app_flask(s, region_start=0)

if patched is None:
    raise SystemExit("[ERR] cannot locate a complete 'app = Flask(...)' call to insert after. Abort safely.")
