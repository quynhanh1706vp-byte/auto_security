...
cp -f "$W" "${W}.bak_rfa_wsgimw_v3_${TS}"
echo "[BACKUP] ${W}.bak_rfa_wsgimw_v3_${TS}"
...
TAG="VSP_P0_WSGIGW_RFA_WSGI_MW_PROMOTE_V3_OUTERMOST_FIX_NAMEERR_V1"
if TAG in s:
...
class __VspRfaPromoteWSGIMW_V3:
    \"""
    Outermost WSGI promoter for /api/vsp/run_file_allow:
      ...
      - Always rewrites JSON when promotion happens
    \"""
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            if changed:
                ...
                # remove length/te then set length
...
# Wrap as OUTERMOST
try:
    application = __VspRfaPromoteWSGIMW_V3(application)
except Exception:
    try:
        app = __VspRfaPromoteWSGIMW_V3(app)
    except Exception:
        pass
# --- /%s ---
""" % (TAG, TAG)
...
print("[OK] appended V3 outermost MW (force promote + err detail)")
PY
