...
cp -f "$WD" "$WD.bak_pidinfer_$(date +%Y%m%d_%H%M%S)"
echo "[BACKUP] $WD.bak_pidinfer_*"
...
def _guess_ci_dir_from_root(root: str, start_ts: int) -> str:
    if not root: return ""
    out_ci = Path(root) / "out_ci"
    if not out_ci.exists(): return ""
    cand=[]
    for d in out_ci.glob("VSP_CI_*"):
        try:
            mt=int(d.stat().st_mtime)
            if mt >= start_ts - 1800:  # within 30m
...
def _pick_runner_log(ci_dir: str) -> str:
    ...
    # fallback: scan depth<=3 file log/txt/summary, ưu tiên có marker
...
def main():
    ...
    while True:
        ...
        # === COMMERCIAL KEY: infer ci_run_dir from PID (cwd) even if target blank ===
        if not st.get("ci_run_dir"):
            root = _infer_root_from_pid(pid)
            st["ci_root_from_pid"] = root or ""
            st["ci_run_dir"] = _guess_ci_dir_from_root(root, start_ts) or ""
...
echo "[OK] watchdog patched + py_compile OK"
