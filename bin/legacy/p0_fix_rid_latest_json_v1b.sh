...
need python3; need date; command -v systemctl >/dev/null 2>&1 || true
...
cp -f "$WSGI" "${WSGI}.bak_ridlatest_v1b_${TS}"
echo "[BACKUP] ${WSGI}.bak_ridlatest_v1b_${TS}"
...
MARK = "VSP_P0_RID_LATEST_JSON_V1B"
if MARK in s:
...
# Append patch block once
s = s + "\n\n" + patch_block + "\n"

# Replace existing rid_latest route if present; else add it.
route_pat = re.compile(
    r'@app\.route\(\s*[\'"]/api/vsp/rid_latest[\'"]\s*\)\s*\n'
    r'def\s+\w+\s*\([^)]*\)\s*:\s*\n'
    r'(?:[ \t].*\n)+',
    re.M
)

new_handler = textwrap.dedent('''
...
systemctl is-active "$SVC" 2>/dev/null && echo "[OK] $SVC active" || echo "[WARN] could not confirm service active"

echo "== [verify] rid_latest json =="
