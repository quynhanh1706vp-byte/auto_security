...
# put right after <head> (earliest)
s2 = re.sub(r'(<head[^>]*>\s*)', r'\1\n'+inject+'\n', s, count=1, flags=re.I)
...
(()=> {
  ...
  function findNumberNearLabel(label){
    try{
      ...
      // label + number somewhere after
      const rx = new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + r'\s*([0-9][0-9,]*)', 'i');
      const m = t.match(rx);
  ...
  function apply(){
    try{
      ...
      // detect sample from existing "xxxx total" label if present
      ...
      if (!ov){
        ...
        // attach to chart area (prefer canvas parent)
      ...
      // hide old "xxxx total" label if it exists
  ...
  // try multiple times (charts load async)
...
echo "DONE. Ctrl+F5 /vsp5. Expect: console clean + warning gone + donut shows 'sample / total'."
