...
# 1) Write dashboard renderer JS
JS="static/js/vsp_dashboard_kpi_toolstrip_v1.js"
mkdir -p "$(dirname "$JS")"
cat > "$JS" <<'JS'
...
# 2) Inject JS include into templates that serve /vsp5 shell
python3 - <<'PY'
...
tag = '<script src="/static/js/vsp_dashboard_kpi_toolstrip_v1.js?v={{ asset_v }}"></script>'
...
for p in targets:
  ...
  bak = p.with_name(p.name + f".bak_dash_kpi_{ts}")
  bak.write_text(s, encoding="utf-8")
  if "vsp_dashboard_kpi_toolstrip_v1.js" in s:
    print("[OK] already included:", p)
    continue
  if "</body>" in s:
  ...
  print("[OK] injected include:", p, "backup:", bak)
PY

echo "[DONE] dashboard KPI/tool strip patch applied."
