...
if marker in html:
    print("[OK] index.html đã có PATCH_TOOL_NOTES_INLINE, bỏ qua.")
else:
    js = r"""(function () {
  function log(msg) {
  ...
  function applyNotesOnce() {
    ...
    if (changed) {
      log('Đã áp dụng ghi chú cố định cho các tool (inline).');
    }
    return changed;
  }
  ...
  if (document.readyState === 'loading') {
    ...
    if "</body>" not in html:
        ...
        print("[OK] Đã chèn inline script PATCH_TOOL_NOTES_INLINE trước </body>.")
PY

echo "[DONE] patch_tool_notes_inline.sh hoàn thành."
