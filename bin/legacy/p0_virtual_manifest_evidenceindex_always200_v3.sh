...
cp -f "$WSGI" "${WSGI}.bak_vmanifest_v3_${TS}"
echo "[BACKUP] ${WSGI}.bak_vmanifest_v3_${TS}"
...
MARK = "VSP_P0_VIRTUAL_MANIFEST_EVIDENCEINDEX_ALWAYS200_V3"
INJECT_MARK = "VSP_P0_VIRTUAL_MANIFEST_EVIDENCEINDEX_ALWAYS200_V3_INJECT"

# --- helper block: NO outer f-string, so braces inside remain literal ---
helper_tpl = r"""
# ===================== __MARK__ =====================
import os, json, time
...
if MARK not in s:
    # insert after initial imports if possible
    m = re.search(r'^(?:from[^\n]+\n|import[^\n]+\n)+', s, flags=re.M)
    if m:
        s = s[:m.end()] + "\n" + helper_tpl + "\n\n" + s[m.end():]
    else:
...
# find handler for /api/vsp/run_file_allow
...
# check within next ~2000 chars after def
window = s[dm.end(): dm.end() + 2000]
if INJECT_MARK not in window:
    # choose indent by peeking next indented line, fallback to 4 spaces
    after = s[dm.end(): dm.end() + 400]
    m_ind = re.search(r'\n([ \t]+)\S', after)
...
echo "[DONE] Expect both 200. If degraded=true -> set VSP_RUNS_ROOT(S) later."
