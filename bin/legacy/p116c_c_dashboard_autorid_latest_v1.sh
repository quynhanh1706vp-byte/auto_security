...
cp -f "$F" "${F}.bak_p116c_${TS}"
echo "[OK] backup: ${F}.bak_p116c_${TS}"

cat > "$F" <<'JS'
/* VSP_P116C_C_DASHBOARD_AUTORID_LATEST_V1
 * If rid missing/empty => auto-resolve latest rid from /api/ui/runs_v3 then redirect.
 * Keeps lightweight SVG trend-mini + TopCWE bars. No external libs.
 */
(() => {
  ...
  function detectRidFromDom(){
    // Some templates put RID as an element id like id="VSP_CI_..."
    const el = $$('[id^="VSP_"]').find(x => x.id && x.id.includes("_"));
    return el?.id || "";
  }

  async function resolveRidOrRedirect(){
    let rid = qparam("rid");
    if (rid !== null) rid = (rid || "").trim();

    // If rid exists and not empty => use it
    if (rid) return rid;
    ...
    // 2) dom hint
    const domRid = (detectRidFromDom() || "").trim();
    if (domRid) {
      const u = new URL(location.href);
      u.searchParams.set("rid", domRid);
      location.replace(u.toString());
      return null;
    }

    // 3) latest from API (best)
    const runs = await fetchJson(`/api/ui/runs_v3?limit=1&include_ci=1`);
    const latest = runs?.json?.items?.[0]?.rid || "";
    if (latest) {
      const u = new URL(location.href);
  ...
  function renderTrendMini(points){
    ...
    if (Array.isArray(points)) {
      for (const p of points) {
        ...
        let xLabel = (p.label ?? p.x ?? p.ts ?? p.time ?? "");
        if (y === null) continue;
  ...
  async function main(){
    // Auto RID (redirect if needed)
    const rid = await resolveRidOrRedirect();
    ...
    const urls = {
      kpis: `/api/vsp/dashboard_kpis_v4${rid ? `?rid=${encodeURIComponent(rid)}` : ""}`,
      trend: `/api/vsp/trend_v1${rid ? `?rid=${encodeURIComponent(rid)}` : ""}`,
      topcwe: `/api/vsp/topcwe_v1${rid ? `?rid=${encodeURIComponent(rid)}` : ""}`,
      top: `/api/vsp/top_findings_v2?limit=5${rid ? `&rid=${encodeURIComponent(rid)}` : ""}`,
    };
...
echo "[OK] wrote $F"
echo "[NEXT] Open: http://127.0.0.1:8910/c/dashboard   (even with rid empty, it will auto-pick latest)"
echo "[TIP] Hard refresh: Ctrl+Shift+R"
