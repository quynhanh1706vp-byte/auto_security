...
cp -f "$WSGI" "${WSGI}.bak_fixdedupe_v3_${TS}"
echo "[BACKUP] ${WSGI}.bak_fixdedupe_v3_${TS}"
...
# Match the old v1 block if it exists
block_re = re.compile(
    r"(?s)# ===================== VSP_P0_VSP5_HTML_DEDUPE_GATE_STORY_MW_V1 =====================.*?"
    r"# ===================== /VSP_P0_VSP5_HTML_DEDUPE_GATE_STORY_MW_V1 ====================="
)
...
if block_re.search(s):
    # CRITICAL: use lambda replacement so backslashes like \s are not parsed by re.sub template engine
    s = block_re.sub(lambda m: mw_v2, s)
else:
    if "VSP_P0_VSP5_HTML_DEDUPE_GATE_STORY_MW_V2" not in s:
        s = s + "\n\n" + mw_v2 + "\n"

p.write_text(s, encoding="utf-8")
print("[OK] MW patched to V2 (lambda replacement; single start_response)")
PY
...
[ "$ok" = "1" ] || { echo "[ERR] /vsp5 not reachable"; exit 2; }
