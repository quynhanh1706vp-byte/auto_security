...
echo "== [P491] BASE=$BASE ==" | tee "$OUT/headers_report.txt"
...
for p in "${tabs[@]}"; do
  ...
  if [ -n "$TO" ]; then
    $TO 6s curl -sS -D - -o /dev/null "$BASE$p" | awk 'BEGIN{IGNORECASE=1} {sub("\r","")} $0 ~ /'"$want"'/ {print}' | tee -a "$OUT/headers_report.txt"
  else
    curl -sS -D - -o /dev/null "$BASE$p" | awk 'BEGIN{IGNORECASE=1} {sub("\r","")} $0 ~ /'"$want"'/ {print}' | tee -a "$OUT/headers_report.txt"
  fi
done
