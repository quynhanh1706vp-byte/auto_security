...
MARK = "VSP_P2_GLOBAL_RID_PICKER_PIN_COPY_V1"
if MARK in s:
...
/* ===================== {MARK} ===================== */
(()=> {{
  if (window.__vsp_p2_global_rid_picker_v1) return;
  window.__vsp_p2_global_rid_picker_v1 = true;
  ...
  function mount(){{
    // avoid double mount
    if (document.getElementById("vsp_rid_picker_v1")) return;

    // Prefer to place near topbar controls; fallback fixed top-right (under follow toggle)
    const host =
    ...
    wrap.innerHTML = `
      ...
      <select id="vsp_rid_sel_v1" style="max-width:280px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:#d8ecff;border-radius:10px;padding:6px 10px;outline:none"></select>
      <button id="vsp_rid_pin_v1" style="padding:6px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:rgba(80,60,20,.35);color:#ffe7b7;cursor:pointer">Pin</button>
      <button id="vsp_rid_copy_v1" style="padding:6px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:rgba(30,60,110,.35);color:#d8ecff;cursor:pointer">Copy</button>
    `;

    // insert
    if (!fixed){{
      // try append to host last
      host.appendChild(wrap);
    }} else {{
      document.body.appendChild(wrap);
    }}

    const sel = document.getElementById("vsp_rid_sel_v1");
    const btnPin = document.getElementById("vsp_rid_pin_v1");
    const btnCopy = document.getElementById("vsp_rid_copy_v1");

    function getPinned(){{
      try {{ return localStorage.getItem(LS_PIN); }} catch(e) {{ return null; }}
    }}
    function setPinned(rid){{
      try {{ localStorage.setItem(LS_PIN, rid); }} catch(e) {{}}
    }}
    ...
    async function populate(){{
      ...
      // Build options: Latest, Pinned, then recent list
      const opts = [];
      ...
      for (const it of runs){{
        ...
        if (opts.length >= 22) break; // keep short
      }}
      ...
      // Choose current rid: if follow ON => latest, else pinned if any, else keep current selection
    ...
    sel.addEventListener("change", ()=> {{
      const rid = sel.value;
      if (!rid) return;
      // User manually selects => follow OFF (commercial expectation)
      setFollow(false);
      dispatchRid(rid);
    }});
    ...
    btnPin.addEventListener("click", async ()=> {{
      ...
      if (pin){{
        clearPinned();
        // if unpin: follow ON and jump to latest
        setFollow(true);
        const latest = await getLatestRid();
    ...
    // Keep UI coherent if follow toggle changes elsewhere
...
p.write_text(s + "\n\n" + addon + "\n", encoding="utf-8")
print("[OK] appended P2 global rid picker v1")
PY
...
echo "[DONE] P2 global RID picker applied"
