...
cp "$APP" "${APP}.bak_settings_v6_$(date +%Y%m%d_%H%M%S)" || true
...
if "SETTINGS_LATEST_INSERT_V6" in data:
    print("[INFO] Đã có block SETTINGS_LATEST_INSERT_V6, bỏ qua.")
else:
    block = '''
# ==== SETTINGS_LATEST_INSERT_V6 ====
...
# ==== END SETTINGS_LATEST_INSERT_V6 ====

'''

    data = data[:idx] + block + "\n" + data[idx:]
    path.write_text(data, encoding="utf-8")
    print("[OK] Đã chèn block SETTINGS_LATEST_INSERT_V6 trước if __main__.")
PY

echo "[DONE] patch_settings_insert_before_main_v6.sh hoàn thành."
