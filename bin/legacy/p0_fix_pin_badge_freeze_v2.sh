...
mkdir -p static/js
cp -f "$JS" "${JS}.bak_freeze_${TS}" 2>/dev/null || true
...
(function(){
  ...
  async function probeSmall(){
    try{
      ...
      const badge=document.getElementById("vsp-data-source-badge");
      if (badge){ badge.textContent="DATA SOURCE: (ERR)"; badge.title=String(e||""); }
    }
  }
...
echo "[OK] patched freeze-safe badge JS: $JS"
echo "[NEXT] Ctrl+F5 in browser."
