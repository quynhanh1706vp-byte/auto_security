...
echo "[VSP_UI_COMM] Wrote CSS -> $CSS"

##################################
# 2) JS polish V2 – tabs thương mại #
##################################
...
echo "[VSP_UI_COMM] Wrote JS -> $JS"

#################################
# 3) Patch mọi template chính   #
#################################

python - << 'PY'
import pathlib, shutil

ui_root = pathlib.Path(".").resolve()
templates = [
    ui_root / "templates" / "index.html",
    ui_root / "templates" / "vsp_dashboard_2025.html",
    ui_root / "templates" / "vsp_index.html",
]

for tpl_path in templates:
    if not tpl_path.exists():
        print("[TPL] Skip (không tồn tại):", tpl_path)
        continue
    ...
    if "vsp_ui_commercial_v1.css" not in txt:
        if "</head>" in txt:
            ...
            changed = True
            print("[TPL]", tpl_path.name, "-> Inject CSS link")
    ...
    if "vsp_ui_commercial_polish_v1.js" not in txt:
        if "</body>" in txt:
            ...
            changed = True
            print("[TPL]", tpl_path.name, "-> Inject JS link")

    if changed:
        backup = tpl_path.with_suffix(tpl_path.suffix + ".bak_comm_tabs_v3")
        backup.write_text(original, encoding="utf-8")
        tpl_path.write_text(txt, encoding="utf-8")
        print("[TPL]", tpl_path.name, "updated, backup ->", backup)
    else:
        print("[TPL]", tpl_path.name, "đã có CSS/JS, bỏ qua.")
PY

echo "[VSP_UI_COMM] DONE – restart Flask + refresh (Ctrl+F5)."
