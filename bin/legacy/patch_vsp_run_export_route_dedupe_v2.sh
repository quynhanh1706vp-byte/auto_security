...
BACKUP="${TARGET}.bak_run_export_dedupe_v2_$(date +%Y%m%d_%H%M%S)"
cp "$TARGET" "$BACKUP"
...
# 1) Bỏ mọi block @app.route(... run_export_v3 ...)
pattern_route = re.compile(
    r'@app\.route\([^\\n]*run_export_v3[^\\n]*\\)[\\s\\S]+?(?=^@app\\.route\\(|^if __name__ == "__main__":|\\Z)',
    re.MULTILINE,
)

txt2, n1 = pattern_route.subn("", txt)
print(f"[INFO] Removed {n1} route block(s) for run_export_v3")

# 2) Bỏ mọi def vsp_run_export_v3(...) còn sót (nếu có)
pattern_def = re.compile(
    r'^def\\s+vsp_run_export_v3\\([^)]*\\):[\\s\\S]+?(?=^@app\\.route\\(|^if __name__ == "__main__":|\\Z)',
    re.MULTILINE,
)

txt3, n2 = pattern_def.subn("", txt2)
print(f"[INFO] Removed {n2} def block(s) for vsp_run_export_v3")

marker = 'if __name__ == "__main__":'
idx = txt3.find(marker)
if idx == -1:
