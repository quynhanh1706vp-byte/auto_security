...
cp -f "$JS" "${JS}.bak_p2_5_fixsyntax_v2_${TS}"
echo "[BACKUP] ${JS}.bak_p2_5_fixsyntax_v2_${TS}"
...
PATCH = """/* ===================== VSP_P2_5_RUNS_AUTOFILTER_RID_V1 ===================== */
(function(){
  try{
...
# IMPORTANT: use lambda replacement to avoid re interpreting backslashes like \n
s2, n = block_re.subn(lambda m: PATCH, s, count=1)
if n != 1:
    print(f"[ERR] failed to replace marker block (n={n})", file=sys.stderr)
    sys.exit(2)

p.write_text(s2, encoding="utf-8")
print("[OK] replaced marker block with clean JS (v2)")
PY
...
echo
echo "[NEXT] Test:"
echo "  Ctrl+Shift+R  /runs?rid=VSP_CI_...  -> highlight + scroll + hide non-matching rows"
echo "  Optional      /runs?rid=VSP_CI_...&open=1"
