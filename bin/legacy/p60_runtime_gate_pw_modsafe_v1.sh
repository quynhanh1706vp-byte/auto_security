...
log "== [P60] Runtime gate (Playwright module-safe, strict) =="
log "UI=$UI"
log "BASE=$BASE"
log "EVID=$EVID"

# --- get RID (robust, retry) ---
RID="${RID:-}"
if [ -z "${RID}" ]; then
...
try:
  ...
  print((j.get("rid") or j.get("run_id") or "").strip())
except Exception:
  print("")
PY
    )" || true
...
log "RID=${RID:-}"
[ -n "${RID:-}" ] || { log "[ERR] cannot get RID"; exit 2; }

# --- sanity tabs (retry /vsp5) ---
tab_ok=1
...
[ "$tab_ok" -eq 1 ] || { log "[ERR] some tabs not 200"; exit 2; }

# --- playwright resolvable in UI ---
node -e "require.resolve('playwright', {paths:[process.cwd()]}); console.log('OK')" >/dev/null 2>&1 \
  || { log "[ERR] playwright not resolvable from UI. (Did you install in ui/?)"; exit 2; }
log "[OK] playwright resolvable from UI"
...
if [ ! -s "$EVID/verdict.json" ]; then
  log "[FAIL] verdict.json missing (gate did not run correctly)"
  log "---- pw_gate.err tail ----"
  tail -n 80 "$EVID/pw_gate.err" | tee -a "$EVID/summary.txt" >/dev/null || true
  exit 2
fi
