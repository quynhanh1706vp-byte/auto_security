...
JSF="static/js/vsp_tool_pills_verdict_from_gate_p0_v1.js"
TPL="templates/vsp_dashboard_2025.html"
[ -f "$TPL" ] || { echo "[ERR] missing $TPL"; exit 2; }

TS="$(date +%Y%m%d_%H%M%S)"
cp -f "$TPL" "$TPL.bak_toolpill_${TS}" && echo "[BACKUP] $TPL.bak_toolpill_${TS}"
...
if "vsp_tool_pills_verdict_from_gate_p0_v1.js" not in t:
  stamp=datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
  tag=f'<script src="/static/js/vsp_tool_pills_verdict_from_gate_p0_v1.js?v={stamp}" defer></script>'
  t=re.sub(r"</body>", tag+"\n</body>", t, count=1, flags=re.I)
  tpl.write_text(t, encoding="utf-8")
  print("[OK] injected tool pills verdict shim")
else:
  print("[OK] tool pills verdict shim already injected")
PY

node --check "$JSF" >/dev/null && echo "[OK] node --check"
echo "[OK] patched tool pills verdict shim"
