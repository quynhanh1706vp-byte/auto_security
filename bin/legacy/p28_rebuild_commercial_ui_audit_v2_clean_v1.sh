...
[ -f "$F" ] && cp -f "$F" "${F}.bak_broken_${TS}" && echo "[BACKUP] ${F}.bak_broken_${TS}"
...
check_api_json "dashboard_extras_v1" "/api/vsp/dashboard_extras_v1" 12 || true

echo "== [4] Per-RID APIs (best-effort) =="
if [ -n "${RID:-}" ]; then
  check_api_json "findings_unified_v1" "/api/vsp/findings_unified_v1/$RID" 12 || true
