...
cp -f "$W" "${W}.bak_endwrap_v3_${TS}"
echo "[BACKUP] ${W}.bak_endwrap_v3_${TS}"
...
MARK = "VSP_P0_RID_LATEST_ENDWRAP_V3"
if MARK in s:
...
echo "== verify (must see X-VSP-RIDPICK header + reason/why keys) =="
curl -isS "$BASE/api/vsp/rid_latest_gate_root" | sed -n '1,35p'
echo
