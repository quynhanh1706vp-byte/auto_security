...
cp -f "$F" "$F.bak_forcefs_export_v1b_${TS}"
echo "[BACKUP] $F.bak_forcefs_export_v1b_${TS}"
...
marker = "### [COMMERCIAL] FORCE_FS_EXPORT_V1B ###"
if marker not in s:
    helpers = """
### [COMMERCIAL] FORCE_FS_EXPORT_V1B ###
def _nowz_v1b():
    return datetime.now(timezone.utc).isoformat(timespec="microseconds").replace("+00:00","Z")

def _find_run_dir_v1b(rid_norm: str):
    cands = []
...
def _ensure_report_v1b(run_dir: str):
    ...
    if not (os.path.isfile(html_path) and os.path.getsize(html_path) > 0):
        ...
        html = "<!doctype html><html><head><meta charset='utf-8'/>" \\
               ...
               f"<h2>VSP Export v3</h2><p>Generated at: {_nowz_v1b()}</p><p><b>Total findings:</b> {total}</p>" \\
...
def _zip_report_v1b(report_dir: str):
...
def _pdf_from_html_wk_v1b(html_file: str, timeout_sec: int = 180):
...
{ind}    run_dir = _find_run_dir_v1b(rid_norm)
...
{ind}        resp.headers["X-VSP-EXPORT-MODE"] = "FORCE_FS_V1B"
{ind}        return resp, 404

{ind}    report_dir = _ensure_report_v1b(run_dir)
...
{ind}    if fmt == "zip":
{ind}        z = _zip_report_v1b(report_dir)
...
{ind}    if fmt == "pdf":
{ind}        pdf_path, err = _pdf_from_html_wk_v1b(html_file, timeout_sec=180)
{ind}        if pdf_path:
