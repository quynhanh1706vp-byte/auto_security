...
cp -f "$JS" "${JS}.bak_fix_deg_${TS}"
echo "[BACKUP] ${JS}.bak_fix_deg_${TS}"
...
#vsp_degraded_v1 a:hover{border-bottom-color:rgba(255,255,255,.55)}
    ...
    const ensurePanel=()=>{
      ...
      p.innerHTML = `
        ...
        <div class="muted" id="vsp_deg_hint_v1" style="margin-top:6px">
          Only shows tools when a run is explicitly DEGRADED / MISSING (avoid false alarms).
        </div>
    ...
    const toolStatus=(o)=>{
      // Commercial-safe classification:
      // - Only mark degraded/missing if explicitly flagged.
      // - Otherwise treat as OK unless explicit error hints exist.
      ...
      // if it contains counts or findings => accept as OK (common in summaries)
      const hasCounts = !!(o.counts_total || o.counts_by_severity || (o.meta && o.meta.counts_by_severity));
      const hasFindingsCount = (o.findings_count != null) || (o.total_findings != null) || (o.total != null);
      if (hasCounts or hasFindingsCount):
        # (python-like 'or' can't be used in JS; keep placeholder to be replaced below
        pass

      return {cls:"ok", st:"OK", why:""};
    };

    // patch the accidental python-ish line above to real JS at runtime (safer than regex edits)
    const toolStatusSafe = (o)=>{
      ...
      const hasCounts = !!(o.counts_total || o.counts_by_severity || (o.meta && o.meta.counts_by_severity));
      const hasFindingsCount = (o.findings_count != null) || (o.total_findings != null) || (o.total != null);
      if (hasCounts || hasFindingsCount) return {cls:"ok", st:"OK", why:""};
      // explicit error hints
    ...
    const render = async ()=>{
      ...
      for (const tool of TOOLS){
        ...
        // Commercial-safe: only list if explicit missing/degraded; or if run itself is degraded and tool is unknown/warn.
      ...
      if (!rows.length && runDegraded){
        setPill("warn","Degraded (no tool detail)");
      ...
      const htmlRows = rows.map(r=>{
        ...
        return `
          <tr>
            <td><span class="pill ${r.st.cls==="err"?"err":(r.st.cls==="warn"?"warn":"warn")}"><span class="d"></span><span>${r.tool}</span></span></td>
...
# fix the accidental python-ish stub line by removing it entirely (already superseded by toolStatusSafe)
fixed = fixed.replace("if (hasCounts or hasFindingsCount):", "// (stub removed)").replace("pass", "// (stub removed)")

s2 = s[:i] + fixed + s[j2:]
p.write_text(s2, encoding="utf-8")
print("[OK] replaced degraded panel with commercial-safe version (RID pinned-first + fewer false WARN/UNKNOWN)")
PY
...
echo "[DONE] Hard refresh /vsp5 => Degraded panel uses pinned RID + avoids false alarms."
