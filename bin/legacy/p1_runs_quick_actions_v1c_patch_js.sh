...
cp -f "$JS" "${JS}.bak_v1c_${TS}"
echo "[BACKUP] ${JS}.bak_v1c_${TS}"
...
/* VSP_P1_RUNS_QUICK_ACTIONS_V1C */
(()=> {
  if (window.__vsp_p1_runs_quick_actions_v1c) return;
  window.__vsp_p1_runs_quick_actions_v1c = true;

  const log = (...a)=>console.log("[RunsQuickV1C]", ...a);
  const warn = (...a)=>console.warn("[RunsQuickV1C]", ...a);
  ...
  function injectStyles(){
    const css = `
      ...
      .vsp-toast{position:fixed;right:16px;bottom:16px;background:#0b1220;border:1px solid #24314f;color:#e5e7eb;padding:10px 12px;border-radius:12px;max-width:520px;box-shadow:0 8px 28px rgba(0,0,0,.35);z-index:99999}
    `;
  ...
  // ---- robust pickers (don't assume API shape) ----
  ...
  function pickDegraded(run){
    ...
    // deep scan boolean true under keys containing "degrad"
    function scan(o, depth=0){
      if (o == null || depth>4) return false;
  ...
  // ---- actions: always enabled; toast if 404/failed ----
  function openUrl(u){ window.open(u,"_blank","noopener"); }

  function ridParam(rid){ return encodeURIComponent(String(rid||"")); }

  function urlCsv(rid){
    const x=ridParam(rid);
    return `${API.exportCsv}?rid=${x}`;
  }
  function urlTgz(rid){
    const x=ridParam(rid);
    return `${API.exportTgz}?rid=${x}`;
  }
  function urlRunFile(rid, path){
    const x=ridParam(rid), p=encodeURIComponent(path);
    return `${API.runFile}?rid=${x}&path=${p}`;
  }
  ...
  async function go(u, label){
    ...
    if (st !== "OK") toast(`${label}: ${st} (vẫn mở tab để xem)`); // demo-friendly
    openUrl(u);
  }
  ...
  function mount(){
    ...
    const wrap=el("div",{class:"vsp-runsqa-wrap"},[
      el("div",{class:"vsp-runsqa-mini"},["Quick Actions V1C: luôn bật action, tự dò Overall/Degraded từ item; nếu endpoint thiếu sẽ toast HTTP status."]),
    ...
    function passes(run){
      ...
      if(o){
        const ok = (x)=>String(x||"").toUpperCase();
        const ov = ok(overall);
        if(o==="PASS" && !(ov.includes("PASS")||ov.includes("OK")||ov.includes("GREEN"))) return false;
        if(o==="FAIL" && !(ov.includes("FAIL")||ov.includes("ERROR")||ov.includes("RED"))) return false;
...
print("[OK] wrote V1C =>", js)
PY

echo "[DONE] Patched JS to V1C. Mở /runs và Ctrl+Shift+R (hard refresh)."
