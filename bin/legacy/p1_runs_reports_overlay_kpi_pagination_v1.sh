...
cp -f "$TPL" "${TPL}.bak_overlay_${TS}"
echo "[BACKUP] ${TPL}.bak_overlay_${TS}"
...
needle = "vsp_runs_reports_overlay_v1.js"
tag = '<script src="/static/js/vsp_runs_reports_overlay_v1.js?v={{ asset_v }}"></script>'

if needle in s:
    print("[OK] overlay already referenced in template")
else:
    # insert before </body>
    if "</body>" not in s:
        raise SystemExit("[ERR] template has no </body> anchor")
    s = s.replace("</body>", tag + "\n</body>")
    tpl.write_text(s, encoding="utf-8")
    print("[OK] injected overlay script tag into template")
PY
...
if [ -x "bin/p1_force_restart_8910_unlock_v1.sh" ]; then
  ...
  echo "[WARN] missing bin/p1_force_restart_8910_unlock_v1.sh (skip restart)"
fi

echo "== quick verify =="
BASE="${VSP_UI_BASE:-http://127.0.0.1:8910}"
curl -sS "$BASE/api/ui/runs_kpi_v2" | head -c 220; echo
curl -sS "$BASE/api/ui/runs_v3?limit=3&offset=0" | head -c 400; echo

echo "[DONE] Open /runs and hard-refresh (Ctrl+Shift+R)."
