...
# Overwrite runs JS with a single commercial-grade implementation (no DEMO, API-only).
cat > "$F" <<'JS'
...
if [ "$HAS_NODE" = "1" ]; then
  node --check "$F" || { echo "[ERR] node --check failed" | tee -a "$OUT/log.txt"; exit 2; }
  echo "[OK] node --check ok"
else
  echo "[WARN] node not found; skipped node --check" | tee -a "$OUT/log.txt"
fi

echo "[INFO] restart $SVC"
...
echo "[OK] P483z done."
echo "Next:"
echo "  1) Close ALL tabs of http://127.0.0.1:8910/c/runs"
echo "  2) Reopen /c/runs"
echo "  3) Ctrl+Shift+R (hard reload)"
