...
cp -f "$F" "${F}.bak_runs_contract_eof_v6_${TS}"
echo "[BACKUP] ${F}.bak_runs_contract_eof_v6_${TS}"
...
MARK="VSP_P1_RUNS_CONTRACT_EOF_V6"
if MARK in s:
...
def _vsp_runs_contract_after_request_v6(resp):
    try:
        ...
        resp.headers["X-VSP-RUNS-CONTRACT"] = "P1_V6"
        return resp
    except Exception:
        return resp

try:
    application.after_request(_vsp_runs_contract_after_request_v6)
except Exception:
    pass
# ==== /{MARK} ====
"""
