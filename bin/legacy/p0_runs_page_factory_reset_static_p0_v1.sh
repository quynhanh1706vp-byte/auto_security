...
need date; need ss; need curl; need awk; need sed; need grep
...
cp -f "$TPL" "${TPL}.bak_factory_reset_${TS}"
echo "[BACKUP] ${TPL}.bak_factory_reset_${TS}"
...
echo "[OK] wrote $TPL (static, no bundle JS)"
...
[ -n "${PIDS// }" ] && echo "[INFO] killing :8910 pids: $PIDS" && kill -9 $PIDS || true

bin/p1_ui_8910_single_owner_start_v2.sh || true

echo "== verify marker =="
curl -sS http://127.0.0.1:8910/runs | grep -n "VSP_RUNS_PAGE_FACTORY_RESET_STATIC_P0_V1" || true
