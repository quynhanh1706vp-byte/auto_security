...
# Nếu mình đã từng patch rồi thì thôi
if "SB_FINAL_ROUTES_V1" in data:
    print("[INFO] SB_FINAL_ROUTES_V1 đã có trong app.py – bỏ qua.")
    sys.exit(0)
...
@app.route("/scan_one")
def sb_scan_one():
    \\"""Trang RUN ONE PROJECT – dùng cùng context với Dashboard nếu có.\\\"""
    ctx = {}
    try:
        # Nếu đã có hàm build context dashboard thì reuse
        if '_sb_build_dashboard_data_ctx' in globals():
            ctx = _sb_build_dashboard_data_ctx()
    except Exception as e:
        print("[SB][scan_one] Lỗi build ctx:", e)
    return render_template("scan_one.html", **ctx)
# ==== END SB_FINAL_ROUTES_V1 ====
""")

data = data + block
path.write_text(data, encoding="utf-8")
print("[OK] Đã chèn SB_FINAL_ROUTES_V1 vào", path)
PY

echo "[DONE] patch_fix_routes_final.sh hoàn thành."
