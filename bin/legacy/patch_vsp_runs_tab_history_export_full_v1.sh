...
BACKUP="${TARGET}.bak_history_export_full_$(date +%Y%m%d_%H%M%S)"
cp "$TARGET" "$BACKUP"
echo "[BACKUP] $TARGET -> $BACKUP"

cat > "$TARGET" << 'JS'
// vsp_runs_tab_v1.js
// [VSP_RUNS_TAB] Commercial Runs & Reports - history + export V1
...
(function () {
  ...
  function renderRunsTable(items) {
    const tbody = document.querySelector('#vsp-runs-table-body');
    if (!tbody) {
      log('Không thấy #vsp-runs-table-body – skip render');
      return;
    }
  ...
  async function loadRunsHistory() {
    const tbody = document.querySelector('#vsp-runs-table-body');
    if (!tbody) {
      log('Không có DOM Runs tab – skip init history');
      return;
    }
  ...
  function init() {
    ...
    log('initialized (history + export V1)');
    loadRunsHistory();
    ensureExportToolbar();
  }
...
echo "[PATCH] Đã ghi JS mới (history + export FULL) vào $TARGET"
