...
need grep; need awk; need sed; need date; need python3

echo "== find candidate JS containing runs/run_file API =="
CANDS="$(grep -RIl --line-number -E '/api/vsp/(runs|run_file)' static/js 2>/dev/null || true)"
if [ -z "${CANDS:-}" ]; then
  echo "[ERR] cannot find JS under static/js containing /api/vsp/runs or /api/vsp/run_file"
  echo "Hint: list static/js:"
  ls -la static/js | head
  exit 2
fi

# pick the best candidate: prefer file that mentions run_file, else first
...
MARK = "VSP_RUNS_EXPORT_ZIP_DOM_P0_V1"
if MARK in s:
    print("[OK] already patched"); raise SystemExit(0)

patch = r'''
;(()=>{ // VSP_RUNS_EXPORT_ZIP_DOM_P0_V1
  try{
    if (window.__VSP_RUNS_EXPORT_ZIP_DOM_P0_V1) return;
    window.__VSP_RUNS_EXPORT_ZIP_DOM_P0_V1 = 1;
    ...
    // aggressive retry because /runs table is async-rendered
    let n=0;
    const t = setInterval(()=>{
      n++;
      patchOnce();
...
echo "[NEXT] restart 8910 then open /runs in browser (hard refresh)"
