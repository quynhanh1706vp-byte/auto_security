...
cp -f "$W" "${W}.bak_p37_export_v2_${TS}"
echo "[BACKUP] ${W}.bak_p37_export_v2_${TS}"
...
block = r'''
# --- VSP_P37_EXPORT_ENDPOINT_V2_FIXED ---
# Commercial P37: provide /api/vsp/export?rid=...&fmt=html|pdf|zip (and trailing slash) with correct content-type + bytes>0.
...
def __vsp_p37_resolve_findings_file(rid: str):
    import os
    if not rid:
        return None
    roots=[]
...
def __vsp_p37_make_html(rid: str, fp: str, items: list) -> bytes:
    # use triple-double-quotes here safely (outer block is triple-single-quotes)
...
code{{color:#93c5fd}}
    ...
    meta = f"<h1>VSP Export</h1><small>RID: <code>{__vsp_p37_escape(rid)}</code> | source: <code>{__vsp_p37_escape(fp)}</code> | total: {len(items)}</small>"
    rows=[]
    ...
    foot = "</body></html>"
    return (head + meta + table + foot).encode("utf-8","replace")

def __vsp_p37_make_pdf_minimal(rid: str, total: int) -> bytes:
    # Minimal valid PDF, 1 page, 2 lines (ASCII only).
    txt1 = "VSP Export"
    txt2 = f"RID: {rid}   TOTAL: {total}"
    def esc(t): return t.replace("\\","\\\\").replace("(","\\(").replace(")","\\)")
    txt1, txt2 = esc(txt1), esc(txt2)
...
def __vsp_p37_make_zip(rid: str, fp: str, html: bytes) -> bytes:
            ...
            def application(environ, start_response):
                try:
                    ...
                    if p == "/api/vsp/export" and method == "GET":
                        ...
                        fp = __vsp_p37_resolve_findings_file(rid)
                        if not fp:
                            return __vsp_p37_json(start_response, {"ok": False, "rid": rid, "reason": "run_dir_not_found"})

                        items = __vsp_p37_load_items(fp)
                        html = __vsp_p37_make_html(rid, fp, items)

                        if fmt == "html":
                            return __vsp_p37_bytes(start_response, html, "text/html; charset=utf-8", f"vsp_{rid}.html")
                        if fmt == "pdf":
                            pdf = __vsp_p37_make_pdf_minimal(rid, len(items))
                            return __vsp_p37_bytes(start_response, pdf, "application/pdf", f"vsp_{rid}.pdf")
                        if fmt == "zip":
                            z = __vsp_p37_make_zip(rid, fp, html)
...
print("[OK] appended: VSP_P37_EXPORT_ENDPOINT_V2_FIXED")
PY
...
echo "== [pick RID] =="
RID="$(curl -sS --connect-timeout 2 --max-time 8 "$BASE/api/vsp/runs?limit=1&offset=0" | python3 - <<'PY' 2>/dev/null || true
import sys,json
...
for fmt in html pdf zip; do
  ...
  curl -sS --connect-timeout 2 --max-time 20 -D /tmp/_e.hdr -o /tmp/_e.bin "$BASE/api/vsp/export?rid=$RID&fmt=$fmt" || true
  awk 'BEGIN{IGNORECASE=1} /^HTTP\/|^Content-Type:|^Content-Disposition:|^Content-Length:/{print}' /tmp/_e.hdr
  echo "bytes=$(wc -c </tmp/_e.bin)"
