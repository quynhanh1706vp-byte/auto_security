...
# Insert CIO link near html/zip/pdf group (best effort)
if "run_export_cio_v1" not in s:
  s = s.replace(
    'href="${esc(EXPORT_BASE + "/" + encodeURIComponent(rid) + "?fmt=html")}"',
    'href="${esc(EXPORT_BASE + "/" + encodeURIComponent(rid) + "?fmt=html")}"'
  )
  # add a CIO action button before html (works for V3 we wrote)
  s = re.sub(
...
print("[OK] injected CIO button link into Runs actions")
PY

node --check "$F" >/dev/null && echo "[OK] runs JS syntax OK"
echo "[DONE] Added CIO button. Hard refresh Ctrl+Shift+R."
