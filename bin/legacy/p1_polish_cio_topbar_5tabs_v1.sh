...
NEW="static/js/vsp_cio_topbar_badge_v1.js"

need(){ command -v "$1" >/dev/null 2>&1 || { echo "[ERR] missing: $1"; exit 2; }; }
need date; need python3; need cp; need grep
command -v node >/dev/null 2>&1 || { echo "[ERR] missing node (need node --check)"; exit 2; }

# 1) Write topbar badge JS (idempotent)
cat > "$NEW" <<'JS'
(function(){
  "use strict";
...
# 2) Inject loader into main tab bundles (idempotent)
python3 - <<'PY'
from pathlib import Path
import re, sys

targets = ["static/js/vsp_bundle_tabs5_v1.js", "static/js/vsp_tabs4_autorid_v1.js"]
inject = 'if(!document.querySelector(\'script[data-vsp-cio-topbar]\')){var s=document.createElement("script");s.src="/static/js/vsp_cio_topbar_badge_v1.js?v=cio_topbar_v1";s.defer=true;s.setAttribute("data-vsp-cio-topbar","1");document.head.appendChild(s);}'
...
def patch_file(fn):
    ...
    if "vsp_cio_topbar_badge_v1.js" in s:
        return False, "already"
    # best place: after DOMContentLoaded or first IIFE top
    if re.search(r'document\.addEventListener\(\s*["\']DOMContentLoaded["\']', s):
        s2=re.sub(r'(document\.addEventListener\(\s*["\']DOMContentLoaded["\']\s*,\s*function\s*\(\)\s*\{)',
                  r'\1\n  try{'+inject+r'}catch(_e){}', s, count=1)
    else:
        s2='try{'+inject+r'}catch(_e){};\n'+s
    p.write_text(s2, encoding="utf-8")
    return True, "patched"

for fn in targets:
    changed, st = patch_file(fn)
    print("[PATCH]", fn, st)
PY

# 3) Syntax check the bundles we touched (best effort)
for f in "$J1" "$J2"; do
  if [ -f "$f" ]; then
    node --check "$f" >/dev/null && echo "[OK] node --check $f" || { echo "[ERR] syntax fail: $f"; exit 2; }
  fi
done

echo "[DONE] CIO Topbar injected. Ctrl+F5 once."
