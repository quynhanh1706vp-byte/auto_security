...
cp "$F" "$F.bak_dedupe_runapi_bp_${TS}"
echo "[BACKUP] $F.bak_dedupe_runapi_bp_${TS}"
...
# Wrap any: app.register_blueprint(bp_vsp_run_api_v1 ...) with "if bp already registered -> skip"
...
if n == 0:
    raise SystemExit("[ERR] cannot find app.register_blueprint(bp_vsp_run_api_v1 ...) in vsp_demo_app.py")

p.write_text(new_txt, encoding="utf-8")
print(f"[OK] patched {n} register_blueprint() call(s) with dedupe guard")
PY
