...
echo "== [0] backup current file =="
cp -f "$APP" "${APP}.bak_p3k26_v11b_before_${TS}"
echo "[BACKUP] ${APP}.bak_p3k26_v11b_before_${TS}"

echo "== [1] find a backup that py_compile passes and restore it =="
GOOD=""
for f in $(ls -1t ${APP}.bak_* 2>/dev/null || true); do
  cp -f "$f" "$APP"
  if python3 -m py_compile "$APP" >/dev/null 2>&1; then
    GOOD="$f"
    echo "[OK] using good backup: $GOOD"
    break
  fi
done
[ -n "$GOOD" ] || { echo "[ERR] no backup compiles"; python3 -m py_compile "$APP" || true; tail -n 80 "$APP" || true; exit 2; }

echo "== [2] insert /vsp5 HTML route safely (AST) =="
python3 - <<'PY'
...
# Remove previously injected blocks (v8/v9/v10)
...
TAG="P3K26_VSP5_HTML_ROUTE_V11B"
if TAG in s:
        ...
        try:
            if any(isinstance(t, ast.Name) and t.id == "app" for t in node.targets):
                if isinstance(node.value, ast.Call):
                    ...
                    if fn_name == "Flask":
                        holder["line"] = getattr(node, "end_lineno", node.lineno)
        except Exception:
            pass
...
if holder["line"]:
    idx = holder["line"]  # insert after this 1-based line
    lines.insert(idx, route_block + "\n")
    out = "".join(lines)
    print(f"[OK] inserted /vsp5 after app=Flask at line {holder['line']}")
else:
    m = re.search(r'(?m)^\s*@app\.route\(', s)
    if m:
        pos = m.start()
        out = s[:pos] + route_block + "\n\n" + s[pos:]
        print("[OK] inserted /vsp5 before first @app.route (fallback)")
    else:
        out = s + "\n\n" + route_block + "\n"
        print("[WARN] appended /vsp5 at end (no @app.route found)")
...
echo "[DONE] p3k26_rescue_vsp_demo_app_restore_good_and_insert_vsp5_v11b"
