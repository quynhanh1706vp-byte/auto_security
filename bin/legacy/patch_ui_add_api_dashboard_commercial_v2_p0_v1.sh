...
if MARK in s or '"/api/vsp/dashboard_commercial_v2"' in s:
    print("[OK] already patched:", MARK)
    p.write_text(s, encoding="utf-8")
    raise SystemExit(0)

# Remove any previous incomplete blocks with same marker (just in case)
s = re.sub(
...
def vsp_api_dashboard_commercial_v2():
    \"\"\"Commercial dashboard API (v2). Returns a stable model derived from findings_unified.json.
    This keeps UI 'commercial' even when unify is summary-only/placeholder.
    \"\"\"
    try:
        from pathlib import Path
        import json
        ...
        # compute totals (prefer explicit 'total', else sum KICS counts, else sum severities)
        ...
        model = {{
            ...
            # Keep a compact list for tool pills / banners
            "items": items,
        }}
...
echo "[NEXT] restart UI 8910 then Ctrl+Shift+R (console should be clean)"
