...
JS_OUT="static/js/vsp_gate_panel_v1.js"
...
(function () {
  ...
  var EP_RUNS = '/api/vsp/runs_index_v3_fs_resolved?limit=1&hide_empty=0&filter=1';
  ...
  function loadLatestAndRender(force){
    ...
    fetch(EP_RUNS, { credentials:'same-origin' })
      .then(function(r){ return r.json(); })
      .then(function(j){
        var it = (j && j.items && j.items[0]) ? j.items[0] : null;
        if (!it) throw new Error('no runs from runs_index_v3_fs_resolved');
        ...
        return fetch('/api/vsp/run_status_v2/' + encodeURIComponent(rid), { credentials:'same-origin' })
          .then(function(r){ return r.json(); })
...
if [ -z "$pick_tpl" ]; then
  pick_tpl="$(grep -RIl --include='*.html' -E "vsp_dashboard|tabs_router|VersaSecure Platform" templates 2>/dev/null | head -n 1 || true)"
fi

if [ -z "$pick_tpl" ]; then
  echo "[WARN] Cannot find template to patch. Add manually:"
  echo '  <script src="/static/js/vsp_gate_panel_v1.js" defer></script>'
  exit 0
fi

cp -f "$pick_tpl" "$pick_tpl.bak_gatepanel_${TS}"
echo "[BACKUP] $pick_tpl.bak_gatepanel_${TS}"
...
tag = '<script src="/static/js/vsp_gate_panel_v1.js" defer></script>'

if tag in txt:
    print("[OK] template already includes gate panel js -> no change")
    raise SystemExit(0)
...
echo "[DONE] Gate panel installed (dashboard)."
echo "Next: restart UI service (if needed) then open Dashboard to see CI/CD Gate."
