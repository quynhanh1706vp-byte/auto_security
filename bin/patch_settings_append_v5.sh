...
cp "$APP" "${APP}.bak_settings_append_v5_$(date +%Y%m%d_%H%M%S)" || true
...
# Nếu đã từng append settings_latest thì thôi
if "def settings_latest(" in data:
    print("[INFO] settings_latest đã tồn tại, không append lần nữa.")
else:
    block = '''
...
'''
    ...
    print("[OK] Đã append settings_latest vào cuối app.py.")
PY

echo "[DONE] patch_settings_append_v5.sh hoàn thành."
