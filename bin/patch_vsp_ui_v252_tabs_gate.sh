...
if [ -f "$JS" ]; then
  BAK_JS="$JS.bak_v251_$(date +%Y%m%d_%H%M%S)"
  cp "$JS" "$BAK_JS"
...
echo "$LOG_PREFIX [OK] Đã ghi JS V2.5.2 vào $JS"
echo "$LOG_PREFIX Hoàn tất patch UI V2.5.2 – Runs/DataSource ổn định + Settings/Rules + CI/CD Gate badges."
