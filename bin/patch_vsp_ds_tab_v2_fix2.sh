...
FILE_V1="$JS_DIR/vsp_datasource_tab_simple_v1.js"

if [ -f "$FILE_V1" ]; then
  BAK="${FILE_V1}.bak_use_v2_fix2_$(date +%Y%m%d_%H%M%S)"
  cp "$FILE_V1" "$BAK"
  echo "[PATCH_DS_V2_FIX2] Backup $FILE_V1 -> $BAK"
fi
  ...
  function applyFilters() {
    ...
    if (!ALL_ITEMS || !ALL_ITEMS.length) {
      tbody.innerHTML =
        ...
        'Không có findings nào trong datasource_v2.' +
        '</td></tr>';
      return;
    }
    ...
    if (!filtered.length) {
      tbody.innerHTML =
        '<tr><td colspan="6" class="vsp-table-empty">' +
        'Không có findings nào khớp bộ lọc.' +
        '</td></tr>';
      return;
    }
    ...
    for (var j = 0; j < filtered.length; j++) {
      ...
      html += '<tr>' +
        '<td>' + idx + '</td>' +
        '<td><span class="' + severityClass(sev2) + '">' +
          severityLabel(sev2) + '</span></td>' +
        '<td>' + tool2 + '</td>' +
      ...
      '<div class="vsp-section-header">' +
        '<div>' +
          ...
          '<p class="vsp-section-subtitle">' +
            'Bảng unified findings (tối đa 500 dòng, từ datasource_v2)' +
          '</p>' +
        '</div>' +
      ...
      '<div class="vsp-card">' +
        '<div class="vsp-ds-filters">' +
          ...
          '<div style="flex:1; min-width:200px;">' +
            ...
            '<input id="vsp-ds-filter-search" class="vsp-input" type="text" ' +
                   'placeholder="Tìm theo rule hoặc file...">' +
          '</div>' +
        '</div>' +
...
echo "[PATCH_DS_V2_FIX2] Wrote V2 datasource logic into vsp_datasource_tab_simple_v1.js"
