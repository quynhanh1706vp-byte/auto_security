...
cp -f "$F" "$F.bak_kics_tail_v9_fixed_${TS}"
echo "[BACKUP] $F.bak_kics_tail_v9_fixed_${TS}"
...
# nếu đã là V9 thì thôi
if "# === VSP_STATUS_TAIL_OVERRIDE_KICS_V9 ===" in t:
    print("[OK] already V9")
    raise SystemExit(0)

# tìm block V8 để replace
pat = re.compile(
...
L = []
L.append(f"{ind}# === VSP_STATUS_TAIL_OVERRIDE_KICS_V9 ===")
L.append(f"{ind}try:")
...
L.append(f"{ind}{step}{step}{step}{step}j = json.loads(txt) if txt.strip() else {{}}")
...
L.append(f"{ind}{step}{step}{step}raw = rawb.decode('utf-8', errors='ignore').replace('\\r','\\n')")
...
L.append(f"{ind}{step}{step}{step}ktail = '\\n'.join(clean[-25:])")
L.append(f"{ind}{step}{step}{step}if hb and (hb not in ktail):")
L.append(f"{ind}{step}{step}{step}{step}ktail = hb + '\\n' + ktail")
...
L.append(f"{ind}# === END VSP_STATUS_TAIL_OVERRIDE_KICS_V9 ===")
block = "\n".join(L) + "\n"

t2 = pat.sub(block, t, count=1)
p.write_text(t2, encoding="utf-8")
print("[OK] upgraded V8 -> V9_FIXED (statefile fallback for ci_run_dir)")
PY
