...
# ensure SAFE entry exists (overwrite for determinism)
cat > "$JS" <<'JSFILE'
...
# patch template: disable ALL static/js/*.js, then add safe entry with fresh cachebust
python3 - <<'PY'
...
print("[OK] patched template -> SAFE MODE (legacy static/js disabled), v=", ts)
PY
