...
JSF="static/js/vsp_runs_overall_from_gate_shim_p0_v3.js"
TPL="templates/vsp_dashboard_2025.html"
[ -f "$TPL" ] || { echo "[ERR] missing $TPL"; exit 2; }

TS="$(date +%Y%m%d_%H%M%S)"
cp -f "$TPL" "$TPL.bak_overallshimv3_${TS}" && echo "[BACKUP] $TPL.bak_overallshimv3_${TS}"
...
# inject v3 if missing
if "vsp_runs_overall_from_gate_shim_p0_v3.js" not in t:
  stamp=datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
  tag=f'<script src="/static/js/vsp_runs_overall_from_gate_shim_p0_v3.js?v={stamp}" defer></script>'
  t=re.sub(r"</body>", tag+"\n</body>", t, count=1, flags=re.I)
# (optional) leave old v2 in place; harmless
tpl.write_text(t, encoding="utf-8")
print("[OK] injected overall shim v3")
PY

node --check "$JSF" >/dev/null && echo "[OK] node --check"
echo "[OK] patched overall shim v3"
