...
[ -f "$F" ] && cp -f "$F" "$F.bak_clean_${TS}" && echo "[BACKUP] $F.bak_clean_${TS}"

python3 - <<'PY'
from pathlib import Path
p=Path("static/js/vsp_rule_overrides_tab_v1.js")
p.write_text(r"""/* VSP_RULE_OVERRIDES_TAB_V1: clean + stable */
(function(){
...
# sanity: refuse if corrupted marker appears
if grep -q 'echo "\[OK\]' "$F"; then
  echo "[ERR] JS still contains shell echo noise. abort." >&2
  exit 3
fi

echo "[OK] rules js clean"
