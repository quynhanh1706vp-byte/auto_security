...
LIST_FILE="/tmp/vsp_jslist_settings_${TS}.txt"
printf "%s\n" "$JS_LIST" > "$LIST_FILE"
echo "[OK] list saved: $LIST_FILE"

echo "== [1] backup all local files =="
BK="/tmp/vsp_fetchguard_v3c_${TS}"
mkdir -p "$BK"
while read -r web; do
  [ -z "$web" ] && continue
...
echo "== [2] inject guard into each JS (idempotent) =="
export LIST_FILE
python3 - <<'PY'
from pathlib import Path
import os, re

marker = "VSP_RUNFILEALLOW_FETCH_GUARD_V3C"
...
lst = Path(os.environ["LIST_FILE"])
webs = [ln.strip() for ln in lst.read_text().splitlines() if ln.strip().startswith("/static/js/")]
...
print("[DONE] changed:", changed, "/", len(webs))
PY
...
while read -r web; do
  [ -z "$web" ] && continue
  if curl -fsS "$BASE$web?cb=$TS" | grep -q "VSP_RUNFILEALLOW_FETCH_GUARD_V3C"; then
...
echo "[DONE] served markers ok=$OK warn=$WARN"
