...
cp -f "$F" "$F.bak_rewrite_v4_${TS}" && echo "[BACKUP] $F.bak_rewrite_v4_${TS}"
...
node --check "$F" >/dev/null && echo "[OK] node --check controller v4" || { echo "[ERR] controller v4 syntax failed"; node --check "$F"; exit 3; }

# cachebust controller in template (best-effort)
if [ -f "$TPL" ]; then
  cp -f "$TPL" "$TPL.bak_cb_ctrl_${TS}" && echo "[BACKUP] $TPL.bak_cb_ctrl_${TS}"
  python3 - <<PY
from pathlib import Path
import re
p=Path("$TPL")
s=p.read_text(encoding="utf-8", errors="ignore")
s=re.sub(r'(vsp_commercial_layout_controller_v1\.js)(\?[^"\']*)?', r'\1?v=$TS', s)
p.write_text(s, encoding="utf-8")
print("[OK] cachebusted controller in template")
PY
fi
