...
backup="$JS.bak_bind_v4_$(date +%Y%m%d_%H%M%S)"
cp "$JS" "$backup"
echo "[BACKUP] $JS -> $backup"

cat > "$JS" << 'JS'
/**
 * VSP Dashboard KPI binder V4
 * Cực kỳ "brute-force" nhưng an toàn:
 * - Quét mọi span/div/p/strong trong #vsp-tab-dashboard.
 * - Tìm node có text chứa "TOTAL FINDINGS", "CRITICAL", ...
 * - Gần label đó, tìm ô value (đang là "-" hoặc số) và gán.
 */
(function () {
  const LOG = "[VSP][KPI_V4]";
  ...
  function findLabelNodes(upperMatch) {
    ...
    const all = root.querySelectorAll("span,div,p,strong");
    const out = [];
    all.forEach((el) => {
  ...
  function findValueNearLabel(labelEl) {
    ...
    // Lấy container DIV gần nhất (card)
...
echo "[DONE] patch_vsp_dashboard_kpi_bind_v4.sh hoàn tất."
