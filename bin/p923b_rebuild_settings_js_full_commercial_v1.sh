...
restore(){
  echo "[ROLLBACK] restoring backup => $BK" | tee -a "$OUT/log.txt"
  if [ -f "$BK" ]; then cp -f "$BK" "$F"; fi
  sudo systemctl restart "$SVC" >/dev/null 2>&1 || true
}
trap 'echo "[ERR] p923b failed"; restore; exit 2' ERR

python3 - <<'PY'
from pathlib import Path

p = Path("static/js/vsp_c_settings_v1.js")
js = r"""// P923B_SETTINGS_FULL_COMMERCIAL_V1 (syntax-safe, commercial-grade)
// Goals:
...
echo "== [P923B] node --check ==" | tee -a "$OUT/log.txt"
node --check "$F" 2>&1 | tee -a "$OUT/log.txt"
echo "[OK] js syntax OK" | tee -a "$OUT/log.txt"

echo "== [P923B] restart ==" | tee -a "$OUT/log.txt"
sudo systemctl restart "$SVC" >/dev/null 2>&1 || true

echo "== [P923B] wait ready ==" | tee -a "$OUT/log.txt"
ok=0
...
[ "$ok" = "1" ] || { echo "[FAIL] not ready"; exit 2; }

echo "== [P923B] smoke ==" | tee -a "$OUT/log.txt"
bash bin/p918_p0_smoke_no_error_v1.sh | tee -a "$OUT/log.txt"

echo "[OK] P923B done. Open: $BASE/c/settings (Ctrl+Shift+R). Evidence: $OUT" | tee -a "$OUT/log.txt"
