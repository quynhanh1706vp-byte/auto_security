...
cp -f "$F" "${F}.bak_vsp5_safe_${TS}"
echo "[BACKUP] ${F}.bak_vsp5_safe_${TS}"
...
def _vsp_p1_vsp5_safe_mode_gate_story_only_v1(resp):
    try:
        ...
        if request.path != "/vsp5":
            return resp
        # Always serve a minimal, stable page (no legacy dash JS).
        import time
...
echo "== PROBE page 200 =="
curl -fsS -I "$BASE/vsp5" | sed -n '1,10p'
echo "[DONE] VSP5 SAFE MODE applied."
