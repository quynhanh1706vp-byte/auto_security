...
cp -f "$W" "${W}.bak_before_rewrite_v2c_${TS}"
echo "[SNAPSHOT] ${W}.bak_before_rewrite_v2c_${TS}"
...
# Restore newest compiling backup if current is broken
if not compiles(W):
...
if i < 0 or j < 0 or j <= i:
    raise SystemExit("[FATAL] START/END markers not found (or malformed)")
...
print("[OK] gateway injector rewritten (splice) + compiles")

# Clean templates: remove any autorid injected tag that contains braces in v=
...
echo "== verify autorid src in 4 tabs (must be numeric v, no braces) =="
