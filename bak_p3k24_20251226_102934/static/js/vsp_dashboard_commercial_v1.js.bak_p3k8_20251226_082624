...
/* VSP_P1_DASHBOARD_COMMERCIAL_V1 (Single Renderer; Strict Contract; CIO-level layout)
   ...
   - /api/vsp/run_file?rid=...&name=gate_summary
   - /api/vsp/run_file?rid=...&name=findings_unified
   NO schema guessing: mismatch => "Data contract mismatch"
*/
(() => {
  if (window.__vsp_dash_commercial_v1_loaded) return;
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_dash_commercial_v1_loaded = true; }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_dash_filter_sev = sevKey; }
    ...
    // Trend (best effort): /api/vsp/rid_latest_v30
    (async () => {
      const note = root.querySelector("#vspTrendNote");
      const bars = root.querySelector("#vspTrendBars");
      try{
        const runs = await fetchJSON("/api/vsp/rid_latest_v30");
  ...
  async function boot(){
    ...
    // Fetch truth files
    const gateUrl = "/api/vsp/run_file?rid="+encodeURIComponent(rid)+"&name=gate_summary";
    const findUrl = "/api/vsp/run_file?rid="+encodeURIComponent(rid)+"&name=findings_unified";
    try{
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p1_settings_soft_refresh_lastrun_v1 = true; }
  function followOn(){
  ...
  async function refreshLastRunExplicit(rid){
    ...
    try {
      const sum = await jget(`/api/vsp/run_file?rid=${encodeURIComponent(rid)}&name=gate_summary`);
      if (sum && sum.ok) {
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p1_rule_overrides_soft_preview_v1 = true; }
  function followOn(){
    try { return (localStorage.getItem("vsp_follow_latest") ?? "on") !== "off"; }
    catch(e) { return true; }
  }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p1_rule_overrides_sync_dropdown_v1b = true; }
  function followOn(){
    try { return (localStorage.getItem("vsp_follow_latest") ?? "on") !== "off"; }
    catch(e) { return true; }
  }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p2_global_rid_picker_v1b = true; }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_rid_prev = prev; }
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_rid_latest = rid; }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p2_rid_picker_force_visible_v1c = true; }
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p1_ui_ok_badge_v1 = true; }
  async function ping(){
    try{
...
if(window.__VSP_CIO && window.__VSP_CIO.debug){ window.__vsp_p1_ui_ok_badge_v2 = true; }
  async function ping(){
    try{
      const r = await fetch("/api/vsp/rid_latest_gate_root", {credentials:"same-origin"});
      if(!r.ok) return {ok:false, status:r.status};
