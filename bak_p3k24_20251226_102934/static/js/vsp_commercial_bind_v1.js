...
(function () {
  const API = {
    runsIndex: "/api/vsp/runs_v3?limit=20&hide_empty=0&filter=1",
  ...
  function verdictBadge(verdict, label) {
    ...
    let cls = "vsp-badge-muted", dot = "vsp-dot-muted", text = v || "—";
  ...
  function updateKpis(s) {
    ...
    const overall = s.overall_verdict || s.overall || (s.run_gate_summary && s.run_gate_summary.overall) || "—";
    const gateOverall = (s.run_gate_summary && (s.run_gate_summary.overall_verdict || s.run_gate_summary.overall)) || overall || "—";
  ...
  async function enhanceRunsTable() {
    ...
    for (const tr of rows) {
      ...
      tdOverall.innerHTML = verdictBadge("—","loading...");
      ...
      try {
        ...
        const overall = s.overall_verdict || (s.run_gate_summary && s.run_gate_summary.overall) || "—";
        ...
        tdOverall.innerHTML = verdictBadge("—", String(e));
        tdDeg.textContent = "-";
      }
    }
  }
